...
- **狀態**：**DONE**（Opus REVIEW-005a PASS，2026-05-17；詳見 `CLAUDE_REVIEW_LOG.md` → REVIEW-005a）
- **Owner**：Codex
- **預估**：S（0.5–1 天）
...
Paper execution 前置條件完整清單（更新於 2026-05-17）：
- TASK-007b DONE ✅（2026-05-17）
- TASK-005 DONE ✅（2026-05-17）
- TASK-005a DONE ✅（2026-05-17，Opus REVIEW-005a PASS）
- Rick 手動 --test-send 驗證 ✅（2026-05-17，Discord channel SENT，proof: `outputs/monitor/test_send/20260517_discord_proof.txt`）
...
## TASK-009 — Forward Record Runner

- **狀態**：**DONE**（Opus REVIEW-009 PASS，2026-05-17；詳見 `CLAUDE_REVIEW_LOG.md` → REVIEW-009）
- **Owner**：Codex
- **預估**：M（3–4 天）
- **依賴**：TASK-005 ✓ DONE；TASK-005a ✓ DONE；TASK-006 ✓ DONE
- **任務目的**：建立 `apps/forward_record/` 模組，實作每日無人值守的 forward record runner，輸出 primary（combined_paper_safe_variant）與 shadow（A_roll12_share20_exclude）兩條 track 的 positions / pnl / forward_stats，並執行 W/S gate 檢查與 safety scan。
- **核心產出**：`apps/forward_record/`（primary.py, shadow.py, gate_checker.py, safety.py, market_data.py, stats_updater.py, report_writer.py, config.py, signal_loader.py, pnl_calculator.py）；`scripts/run_forward_record.py`；15+ unit tests。
- **REVIEW-009 核心發現**：
  - 11/11 unit tests PASS；CLI dry-run REVIEW_READY；endpoint_scan PASS；FORBIDDEN flags 全部存在
  - W-1（CAVEAT）：`api_key_request=NOT_ATTEMPTED` substring 誤判 false positive → 歸 TASK-009a
  - W-2（CAVEAT）：dry-run 日 primary = shadow weights（alpha cap 未觸發）→ 歸 TASK-009a 加 `alpha_cap_triggered_today`
  - W-3（CAVEAT）：既有 uncommitted diff → Rick forward clock 前需 clean working tree

---

## TASK-009a — Forward Record 小修補（Secret Scan 白名單 + alpha_cap_triggered_today）

- **狀態**：**TODO**
- **Owner**：Codex
- **預估**：XS（< 1 天）
- **依賴**：TASK-009 ✓ DONE
- **任務目的**：處理 REVIEW-009 W-1/W-2 caveat：(1) secret scan 白名單排除 `api_key_request=NOT_ATTEMPTED` false positive；(2) `forward_stats.json` 加入 `alpha_cap_triggered_today` 欄位以明確標記 alpha cap 是否觸發，讓 REVIEW-009b W-2 類問題可直接從輸出判讀。
- **範圍**：`apps/forward_record/safety.py`（掃描範圍精準化或白名單機制）；`apps/forward_record/primary.py` 或 `stats_updater.py`（加 `alpha_cap_triggered_today` 欄位）；相關 unit test 更新。
- **禁止**：不可修改策略訊號；不可重跑 baseline；不可連接 Bybit；不可啟動 forward clock。

---

## TASK-009b — Forward Monitor Alerting

- **狀態**：**DONE**（Opus REVIEW-009b PASS，2026-05-18；詳見 `CLAUDE_REVIEW_LOG.md` → REVIEW-009b）
- **Owner**：Codex
- **預估**：S（2–3 天）
- **依賴**：TASK-009 ✓ DONE；TASK-005a ✓ DONE
- **任務目的**：在 `apps/forward_record/alerting.py` 實作 alerting 模組，整合 `apps/monitor/channels/discord.py`，在 runner 靜默失敗、gate 命中、或 review_006b 觸發條件達成時主動發 Discord 通知。
- **核心產出**：`apps/forward_record/alerting.py`（197 行）；`apps/forward_record/alert_conditions.py`（294 行）；`tests/forward_record/test_alerting.py`（267 行）；`scripts/run_forward_record.py` 末端整合。
- **Alert Conditions（A-1~A-7）**：runner missing rows 連續 2 天、stop gate 命中、warning gate 連續 3 天、primary/shadow alpha gap > 0.05、data source failure、review_006b_trigger_ready（不重複通知）、FORBIDDEN field violation。
- **REVIEW-009b 核心發現**：
  - 10/10 fail gates PASS；15/15 tests PASS；dry_run 三重 gate；Discord external_post_attempted=false
  - W-1（CAVEAT）：A-5 `CacheMarketDataProvider` marker false positive → 歸 TASK-009c
  - W-2（CAVEAT）：`_extract_yyyymmdd()` path parsing 脆弱 → 歸 TASK-009c
  - W-3（CAVEAT）：`REVIEW_NUMBERS_PATH` 依賴 review artifact → 歸 TASK-009c

---

## TASK-009c — Forward Record 技術債修補（W-1/W-2/W-3 Caveat 收斂）

- **狀態**：**DONE**（Opus REVIEW-009c PASS，2026-05-18；詳見 `CLAUDE_REVIEW_LOG.md` → REVIEW-009c）
- **Owner**：Codex
- **預估**：XS（< 1 天）
- **依賴**：TASK-009b ✓ DONE
- **任務目的**：修補 REVIEW-009b 與 REVIEW-009d 共六項 caveat（合併範圍）：
  - **REVIEW-009b W-1**：`check_data_source_failure()` 中的 `CacheMarketDataProvider` log marker 改為精準字串（如 `data_source_error=True`），或加白名單機制，避免正常運行時誤觸 A-5。
  - **REVIEW-009b W-2**：`_extract_yyyymmdd()` 加邊界測試（路徑含多個 8 位數字序列時行為正確）；長期改為 pathlib 解析 stem 或用 `{date}` 佔位符。
  - **REVIEW-009b W-3**：output path 模板從 `REVIEW-009_NUMBERS.json`（review artifact）搬至 runtime config（如 `configs/prev3y_crypto.yaml` 或 `configs/forward_record.yaml`），讓 alerting 不依賴 review 產物。
  - **REVIEW-009d W-1**：`_message_preview()` 在 content checks 前 inject `scenario_id/condition_id/Date/Action` 欄位，使 `has_date`、`has_condition_id`、`has_action` 常 True，無法直接驗證 `condition.message` 原文。建議新增 raw content check 或將 inject 欄位剝離後再做 generic checks。
  - **REVIEW-009d W-2**：`_sanitize_text("None" → "n/a")` 在 preview 生成前替換，使 `no_placeholder` 對 Python `None` 混入的防護形骸化。建議在 `condition.message` 構建時加 None guard，或在 `_sanitize_text` 之前先做 raw string check。
  - **REVIEW-009d W-3**：A-5 `CacheMarketDataProvider` false positive 的 negative drill scenario（S-A5c）在 TASK-009d drill 中未含。TASK-009c 修正 A-5 marker 後，應補入 S-A5c 驗證正常 log 不誤射。
- **範圍**：`apps/forward_record/alert_conditions.py`；`apps/forward_record/alerting.py`（REVIEW_NUMBERS_PATH）；`scripts/drill_forward_alerts.py`（_message_preview, _sanitize_text, S-A5c）；configs 新增/修改；相關 tests 更新。
- **禁止**：不可修改策略訊號；不可重跑任何 runner；不可連接 Bybit；不可啟動 forward clock；不可送實際 Discord alert。
- **REVIEW-009c 核心發現**：
  - C-1~C-6 全部 PASS；Windows 110 tests PASS；drill 13/13 PASS（含 S-A5c）
  - Linux sandbox stale .pyc / truncated source（4 monitor test failures）= infrastructure noise，非 code regression（Opus 裁定）
  - NOTE-1（non-blocking）：`REVIEW_NUMBERS_PATH` legacy constant 留 fallback；可 cleanup sprint 移除
  - NOTE-2（non-blocking）：`drill_report.json` `"task": "TASK-009d"` tag 未更新；可 cleanup sprint 修正
- **Codex implementation output（2026-05-18）**：
  - Added `configs/forward_record.yaml` runtime output path templates.
  - Updated `apps/forward_record/alert_conditions.py` for A-5 marker narrowing, `{date}` template support, stem-first date parsing, A-4 `n/a` display, and `AlertConditionResult` None guard.
  - Updated `apps/forward_record/alerting.py` with `FORWARD_RECORD_CONFIG_PATH`, `resolve_forward_output_paths_from_config()`, and review-numbers fallback compatibility.
  - Updated `scripts/drill_forward_alerts.py` with raw content checks, `no_placeholder_raw`, S-A5c negative scenario, and 13-scenario report output.
  - Updated `tests/forward_record/test_alerting.py` and `tests/forward_record/test_alert_e2e_drill.py`.
  - Generated `docs/research/review_packets/REVIEW-009c_PACKET.md` and `docs/research/review_packets/REVIEW-009c_NUMBERS.json`.
  - Refreshed `outputs/forward_record/drill/20260517_drill_report.json`; result `PASS`, `scenario_count=13`, `external_post_attempted=false`.
  - Validation: `tests.forward_record.test_alerting` PASS (22), `tests.forward_record.test_alert_e2e_drill` PASS (21), `tests.forward_record` PASS (54), `tests.monitor.test_channels` PASS (13), py_compile PASS.
  - Did not rerun the forward record runner; did not connect Bybit; did not send real Discord alert; 30-day clock remains NOT_STARTED; paper/live remain FORBIDDEN.

---

## TASK-009d — Alert Delivery 端對端演練（E2E Drill）

- **狀態**：**DONE**（Opus REVIEW-009d PASS，2026-05-18；詳見 `CLAUDE_REVIEW_LOG.md` → REVIEW-009d）
- **Owner**：Codex
- **預估**：S
- **依賴**：TASK-009b ✓ DONE；最新 TASK-009d 工單、NEXT_ACTION、Rick 指示均要求 **dry-run/mock only**。
- **範圍更新（2026-05-18）**：舊 VPS/live drill 描述不適用；本任務不得使用 `--live-alerts`，不得把 Discord `dry_run` 改成 false，不得送真實 Discord alert，不得連 Bybit，不得要求或讀取 credential material，不得啟動 30-day forward clock。
- **任務目的**：用 mock fixtures 執行 A-1~A-7 alert E2E dry-run drill，確認 alert content、redaction、dedupe、Discord template rendering、`force_dry_run=True`、以及 `SENT` fail gate。
- **重要性**：若 30-day forward clock 啟動後 alerting 從未經過真實端對端驗證，stop gate 命中時 Rick 可能完全不知情（silent failure in alerting layer）。
- **Codex implementation output（2026-05-18）**：
  - Added `scripts/drill_forward_alerts.py`.
  - Added `tests/forward_record/test_alert_e2e_drill.py`.
  - Generated `outputs/forward_record/drill/20260517_drill_report.json`.
  - Generated `docs/research/review_packets/REVIEW-009d_PACKET.md`.
  - Generated `docs/research/review_packets/REVIEW-009d_NUMBERS.json`.
  - Drill result: `PASS`; `dry_run=true`; `live_alerts_used=false`; `external_post_attempted=false`; Discord probe status `DRY_RUN`; no `SENT` observed.
  - Scenario result: S-A1/S-A2/S-A3/S-A4/S-A5/S-A5b/S-A6/S-A7 triggered as expected; S-A1b/S-A3b/S-A4b/S-A6b suppressed/skipped as expected.
  - Redaction validation PASS; dedupe validation PASS; Discord template validation PASS; safety scan PASS.
  - Paper execution remains FORBIDDEN; live trading remains FORBIDDEN; 30-day forward clock remains NOT_STARTED.
- **禁止**：不可連接 Bybit；不可要求或讀取 credential material；不可送真實 Discord alert；不可使用 `--live-alerts`；不可啟動 30-day forward clock；不可批准 paper/live execution；不可修改策略訊號。

---

## TASK-006 — Paper Trading Planning Modules（legacy section continues）
...
## TASK-008 — Alpha-Space Concentration Cap（REVIEW-007 結構性發現 follow-up，2026-05-16 新增）

- **狀態**：**DONE**（Opus REVIEW-008 CONDITIONAL_PASS，2026-05-17）
  - W-1 caveat（observation）：top5_conc 87.95% > 75% 原目標；Opus 裁定降為觀察指標，可接受上限 ≤ 90% + single_conc < 25%（已達標 23.43%）
  - 推薦 variant：`A_roll12_share20_exclude`（Pareto-dominant）；列入 TASK-006 tertiary / shadow-track
  - TASK-008b 設為 backlog（低優先度；Rick 可視 75% 是否仍為硬目標再決定）
- **工單**：`docs/research/codex_workorders/TASK-008_alpha_space_concentration_cap.md`（v1.0，2026-05-17）
...
- **Owner**：Claude（寫工單）→ Codex（後續實作）→ Opus（REVIEW-008）
- **預估**：M（3–5 天，包含 backtest 重跑）
- **三個必研究變體**：
  - **Variant A**：Rolling alpha-contribution cap（滾動 alpha 貢獻上限；exclude / penalize）
  - **Variant B**：Alpha-share-based position sizing（反向 alpha share 調整 weight）
  - **Variant C**：Top contributor cooldown / blacklist（連續 K 期後冷卻 M 期）
- **比較指標**：Sharpe / IR vs eqw / max DD / net alpha / top5_conc / single_conc / long_net / short_net / alpha_retention / cost_impact / turnover_change
- **目標**：top5_conc < 75%（attribution 口徑）且 Sharpe ≥ 0.70（active 口徑）
- **介入點**：`src/signals/prev3y_momentum.py` → `build_prev3y_targets()` post-selection（via `src/variants/task008.py`）；不修改主流程
- **禁止**：weight-space overlay；修改 prev3y_momentum.py 主流程；接 exchange；paper/live trading
- **與 TASK-006 paper trading 的關係**：TASK-008 完成後產出的新 baseline 是**正式 paper 上線版本**；TASK-006 現有 combined_paper_safe_variant 是「過渡版」。
- **不擋短期 forward record**：30-day forward record 用 combined_paper_safe_variant 並行進行；TASK-008 不是 forward record 的前置條件。
- **禁止**：在沒有 Opus REVIEW-008 PASS 下宣稱 baseline 已更新。

### Codex implementation output（2026-05-17）
- Added `src/variants/task008.py` and `scripts/task008_alpha_conc_cap.py`.
- Added focused tests under `tests/variants/test_task008.py`.
- Generated REVIEW-008 outputs:
...
- Runner status: `REVIEW_READY`; fail gates `0`; warning gates `19`; baseline mismatch `5.55e-17`.
- Best alpha-space candidate by Sharpe among passing implementations: `A_roll12_share20_exclude` / `A_roll12_share20_penalize50` / `A_roll24_share20_exclude`, Sharpe `0.9636`, IR vs equal-weight `0.7289`, net alpha `31.00%`, alpha retention `108.66%`, top5 concentration `87.95%`.
- Concentration target `<75%` is not met by the best Sharpe candidates; this remains a REVIEW-008 decision item, not a Codex approval.
- Validation: `python -m unittest tests.monitor.test_heartbeat tests.monitor.test_alerts tests.monitor.test_channels tests.variants.test_task008 -v` PASS (`24` tests); `python -m py_compile src\variants\task008.py scripts\task008_alpha_conc_cap.py` PASS.
- No main strategy changes; `src/signals/prev3y_momentum.py` was not modified; TASK-007b weight-space redistribution was not reused.
- Paper execution and live trading remain FORBIDDEN.

---

## TASK-009 — Forward Record Runner（2026-05-17 新增）

- **狀態**：**DONE**（Opus REVIEW-009 PASS，2026-05-17）
  - W-1 caveat：secret scan false positive（`api_key_request` substring）→ TASK-009a 補白名單
  - W-2 caveat：dry-run primary = shadow weights（alpha cap 未觸發日）→ TASK-009a 加 `alpha_cap_triggered_today`
  - W-3 caveat：working tree uncommitted diffs → Rick forward clock 前需 clean working tree
  - VPS_DEPLOYMENT_CHECKLIST.md Phase 6：解鎖
- **工單**：`docs/research/codex_workorders/TASK-009_forward_record_runner.md`（v1.0，2026-05-17）
- **Owner**：Codex（實作）→ Sonnet（review packet）→ Opus（若需 review）
- **預估**：S（2–3 天）
- **依賴**：TASK-006 ✅、TASK-007b ✅、TASK-008 ✅（shadow-track）、TASK-005 ✅（monitor 整合）
- **目標**：建立 `scripts/run_forward_record.py` + `apps/forward_record/` 模組；解鎖 `VPS_DEPLOYMENT_CHECKLIST.md` Phase 6
- **Primary spec**：`combined_paper_safe_variant`（3 overlay rules）
- **Shadow-track**：`A_roll12_share20_exclude`（`--shadow-track` 旗標啟用）
- **輸出目錄**：
...
- **Codex 產出（2026-05-17）**：
  ...
  - Dry-run outputs under primary/shadow output directories
- **Codex validation（2026-05-17）**：`python -m unittest tests.forward_record -v` PASS（11 tests）；CLI dry-run PASS；safety scan PASS；paper/live remain FORBIDDEN；30-day clock NOT_STARTED
- **9 種單元測試**（含 mock market data；不打真實 API）
- **禁止**：不下單；不連寫入 API；不把 API key 寫入任何輸出；不修改策略主流程；不重跑官方輸出

---

## TASK-009a — Forward Record Runner 小修補（2026-05-17 新增）

- **狀態**：**TODO**
- **Owner**：Codex（實作）→ Sonnet（快速 review，不需 Opus）
- **預估**：XS（< 1 天）
- **觸發原因**：Opus REVIEW-009 W-1 / W-2 caveat 建議
- **禁止**：不得修改策略主流程；不得重跑官方輸出；不得連接 Bybit；不得啟動 forward clock
- **兩項修補**：

### 修補 A：scan_no_secrets_in_outputs() 白名單 / 範圍限縮

```python
# apps/forward_record/safety.py
# 現狀：掃描整個共享 log 目錄，會命中 TASK-005/005a 的既有 log
# 修補選項（任選一）：
# Option 1 — Allowlist
ALLOWLISTED_SUBSTRINGS = ["api_key_request"]   # 狀態訊息，非 key 值
# Option 2 — 限縮掃描至 runner 寫入的特定檔案 prefix
files = [p for p in root.rglob("*.json") if p.stem.startswith(date)]
# Option 3 — 在 run_safety_scan() 中只掃 forward_record/ 目錄，不含共享 log 目錄
```

驗收：對 `outputs/logs/prev3y_crypto/` 執行 `scan_no_secrets_in_outputs()` → `status: PASS`（不誤判）。

### 修補 B：positions.parquet 加 alpha_cap_triggered_today 欄位

```python
# apps/forward_record/shadow.py（或 primary.py 共用 _decorate_positions）
# 新增欄位：
frame["alpha_cap_triggered_today"] = frame["variant_weight"] != frame["original_weight"]
# 若為 primary track（combined_paper_safe_variant），此欄位恆為 False（無 alpha-space cap）
```

驗收：shadow positions.parquet 中 `alpha_cap_triggered_today` 欄位存在；dry-run 日（20260430）= False；live run 中 cap 觸發日 = True。

### Completion 驗收

```bash
python -m unittest discover -s tests/forward_record -v   # 仍 ≥ 11 PASS
python scripts/run_forward_record.py --dry-run --shadow-track --date 20260517
# 驗證：
# 1. scan_no_secrets_in_outputs 對 outputs/logs/prev3y_crypto/ 不再 FAIL
# 2. shadow positions 含 alpha_cap_triggered_today 欄位
```

---

## TASK-009b — Forward Record Monitor Alerting（2026-05-17 新增）

- **狀態**：**REVIEW**（Codex implementation completed 2026-05-18；awaiting REVIEW-009b）
- **Owner**：Codex（實作）→ Sonnet（review draft）→ Opus（若有重要判斷）
- **預估**：S（2–3 天）
- **觸發原因**：Opus REVIEW-009 建議；30-day clock 啟動後若無 alerting，Stop gate 無法及時通知
- **依賴**：TASK-009 ✅ DONE；TASK-005 ✅ DONE（Discord monitor channel）
- **禁止**：不得連接 Bybit 寫入 endpoint；不得下單；不得批准 paper/live；不得啟動 forward clock；不得修改策略主流程
- **Codex 產出（2026-05-18）**：
...
- **Codex validation（2026-05-18）**：`python -m unittest tests.forward_record -v` PASS（26 tests）；`python -m unittest tests.monitor.test_channels -v` PASS（13 tests）；runner dry-run PASS；alert log dry_run=true；Discord POST NOT_ATTEMPTED；paper/live remain FORBIDDEN；30-day clock NOT_STARTED

### 目標

在每日 `run_forward_record.py` 執行後，自動：
1. 評估 W-1~W-6 / S-1~S-6 gate 結果
2. 若有 Warning gate → 傳送 Discord WARNING 訊息（透過 TASK-005 `apps/monitor/channels/discord.py`）
3. 若有 Stop gate → 傳送 Discord CRITICAL 訊息；更新 `forward_summary.json` `clock_paused=true`
4. 若 `review_006b_trigger_ready = true` → 傳送 Discord REVIEW_READY 通知（提醒 Rick）

### 實作位置

```
apps/forward_record/alerting.py   （新建）
  └── send_gate_alerts(gate_result, config, dry_run=True)
      └── 呼叫 apps/monitor/channels/discord.DiscordChannel
```

### Alert 訊息格式

```
[W-1] Forward Record Day {N}: sharpe_rolling_30d={X:.4f} < 0.5
Strategy: combined_paper_safe_variant | Date: {YYYYMMDD}
Action: Extended recording to Day 45. Rick notified.
paper_execution_status: FORBIDDEN | live_trading_status: FORBIDDEN

[S-2] CRITICAL: Forward Record Day {N}: max_dd={X:.2%} ≤ -40%
Clock PAUSED. Awaiting Rick instruction.
paper_execution_status: FORBIDDEN | live_trading_status: FORBIDDEN

[REVIEW_READY] Forward Record Day 30 complete.
Sharpe(30d)={X:.4f} | MaxDD={Y:.2%} | No stop gates triggered.
REVIEW-006b packet ready for Opus review. Paper execution remains FORBIDDEN.
```

### 測試要求

- mock Discord channel（不打真實 webhook）
- W/S gate 觸發路徑各有測試
- `review_006b_trigger_ready` 通知路徑測試
- `dry_run=True` 時不發送真實 Discord 訊息
