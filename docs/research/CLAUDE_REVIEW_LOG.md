...
## REVIEW-009b — TASK-009b Forward Monitor Alerting
...
|---|---|
| **最終 Verdict** | **PASS** |
| 審查人（Draft） | Claude Sonnet，2026-05-18 |
| 最終裁定 | Opus，2026-05-18 |
| Fail gates（10 項） | 0/10 fail（全部通過） |
| Warning 數量 | 3 件（全部 CAVEAT，non-blocking） |
| TASK-009b 狀態 | **DONE** |
...
### 2. Fail Gates（10/10 PASS）

| # | Gate | 判定 | 依據 |
|---|---|---|---|
| FG-1 | 無 order endpoint import | PASS | scan_no_order_endpoints → violations=[] |
| FG-2 | FORBIDDEN_live_trading = NOT_ATTEMPTED | PASS | _safety_fields() hardcoded；alert_log 確認 |
| FG-3 | FORBIDDEN_order_endpoint = NOT_ATTEMPTED | PASS | 同上 |
| FG-4 | FORBIDDEN_bybit_write = NOT_ATTEMPTED | PASS | 無任何 bybit import |
| FG-5 | dry_run 預設 True | PASS | force_dry_run: bool = True；live_alerts: bool = False |
| FG-6 | 雙重 dry_run gate 邏輯正確 | PASS | `True if force_dry_run or not live_alerts else discord_channel.dry_run` |
| FG-7 | 全部 7 項 alert conditions 實作（A-1~A-7） | PASS | REVIEW-009b_NUMBERS.json 全部 IMPLEMENTED |
| FG-8 | 15/15 tests PASS | PASS | pytest 直接執行 |
| FG-9 | clock_started 不變動 | PASS | 無任何 clock_started mutation |
| FG-10 | 驗證期間無實際 Discord POST | PASS | external_post_attempted=false |

### 3. Warning Rulings（全部 CAVEAT，non-blocking）
...
- AlertConditionResult dataclass：每個 condition 獨立可測
- A-6 duplicate suppression：讀前日 alert_log，同日只通知一次
- A-7 last-resort gate：_safety_fields() hardcoded NOT_ATTEMPTED
- Triple dry_run gate：CLI → `force_dry_run or not live_alerts` → yaml dry_run:true

### 5. Audit Statement

- alerting.py（197 行）、alert_conditions.py（294 行）、test_alerting.py（267 行）全讀
- pytest 直接執行：15/15 PASS；scan_no_order_endpoints → violations=[]
- 未執行：Bybit 連接、Discord 實際 POST、30-day clock 啟動、paper/live execution 批准

---

## REVIEW-009d — TASK-009d Alert Delivery E2E Drill
...
| Warning 數量 | 3 件（全部 CAVEAT，non-blocking） |
| TASK-009d 狀態 | **DONE** |
| Alert E2E drill readiness | **READY** |
| 30-day clock | NOT_STARTED |
| Paper execution | FORBIDDEN |
| Live trading | FORBIDDEN |
...
|---|---|---|---|
| FG-1 | 無 order endpoint import | PASS | scan_no_order_endpoints → violations=[] |
| FG-2 | FORBIDDEN_live_trading = NOT_ATTEMPTED | PASS | drill_report.json 確認；hardcoded |
| FG-3 | FORBIDDEN_order_endpoint = NOT_ATTEMPTED | PASS | 同上 |
| FG-4 | FORBIDDEN_bybit_write = NOT_ATTEMPTED | PASS | 無任何 bybit import |
| FG-5 | FORBIDDEN_real_discord_post = NOT_ATTEMPTED | PASS | external_post_attempted=false；statuses=["DRY_RUN"] |
| FG-6 | dry_run=True 強制 | PASS | force_dry_run=True；live_alerts=False |
| FG-7 | ChannelResult.status ≠ SENT | PASS | sent_fail_gate=PASS；sent_seen=false |
| FG-8 | 18/18 tests PASS | PASS | python -m unittest 直接執行；0.083s |
| FG-9 | 12/12 drill scenarios PASS | PASS | 正 trigger 8 + 負 trigger 4 全部符合預期 |
| FG-10 | clock_started 不變動 | PASS | drill_report.json clock_started=false |

### 3. Drill Scenarios（12/12 PASS）

正 trigger：S-A1（runner missing）、S-A2（stop gate CRITICAL）、S-A3（warning streak）、S-A4（alpha gap）、S-A5（stats missing）、S-A5b（data_source=FAILED）、S-A6（review_006b 首日）、S-A7（FORBIDDEN violation CRITICAL）

負 trigger：S-A1b（正常日不誤報）、S-A3b（gap 中斷不誤報）、S-A4b（shadow 不存在跳過）、S-A6b（次日 dedupe 抑制）

### 4. Warning Rulings（全部 CAVEAT，non-blocking）

| Warning | 說明 | 處置 |
|---|---|---|
| W-1 | `_message_preview()` 注入 context 使 has_date 等 content checks inflated | → TASK-009c（合併 REVIEW-009b/009d warnings） |
| W-2 | `_sanitize_text("None"→"n/a")` 弱化 no_placeholder check | → TASK-009c |
| W-3 | A-5 false positive negative scenario S-A5c 未補 | → TASK-009c 完成 A-5 marker 收斂後補 S-A5c |

### 5. Validation Summary

- Redaction PASS：9 pattern × 12 scenarios = 0 violations
- Dedupe PASS：A-6 首日 triggered=True / 次日 suppressed=True；A-2 non-dedupe 確認
- Discord template PASS：6 項全 True（non-empty / condition_id / date / action / no_placeholder / severity）

### 6. Audit Statement

- drill_forward_alerts.py（613 行）、test_alert_e2e_drill.py（126 行）全讀
- python -m unittest 直接執行：18 passed in 0.083s
- drill_report.json、REVIEW-009d_NUMBERS.json 直接讀取確認
- 未執行：Discord 實際 POST、--live-alerts、Bybit 連接、30-day clock 啟動、paper/live execution 批准

---

## REVIEW-009c — TASK-009c Forward Record Tech Debt Fixes C-1~C-6
...
| 審查人（Draft） | Claude Sonnet，2026-05-18 |
| 最終裁定 | Opus，2026-05-18 |
| 修補項目（6 件） | C-1~C-6 全部 PASS |
| TASK-009c 狀態 | **DONE** |
| Tech debt readiness | **READY** |
| 30-day clock | NOT_STARTED |
| Paper execution | FORBIDDEN |
| Live trading | FORBIDDEN |
...
| 修補 | 說明 | 判定 | 根據 |
|---|---|---|---|
| C-1 | A-5 `CacheMarketDataProvider` marker 移除；保留 `("data_source=FAILED","RuntimeError")` | **PASS** | `alert_conditions.py` line 220 確認；S-A5c drill scenario triggered=false |
| C-2 | `_extract_yyyymmdd()` 改用 pathlib stem + word-boundary regex `(?<!\d)(\d{8})(?!\d)` | **PASS** | lines 303–308；邊界測試含多 date 路徑 stem 優先 |
| C-3 | `configs/forward_record.yaml` 新增（18 行 `{date}` template）；`alerting.py` 新增 `resolve_forward_output_paths_from_config()`；`_resolve_runtime_paths()` 優先讀 config | **PASS** | yaml 存在；`{date}` 佔位符支援落地；legacy REVIEW_NUMBERS_PATH 保留為 fallback（non-blocking） |
| C-4 | `_scenario()` 新增 `raw_content` sub-dict（`has_date_in_raw`, `has_action_in_raw`, `has_condition_id_in_raw`）；has_condition_id_in_raw=false 正確反映 raw message 不含 condition_id | **PASS** | drill_report.json 全 13 scenarios 有 raw_content 欄位 |
| C-5 | `AlertConditionResult.__post_init__` lines 23–27：`message is None` / `action_required is None` 直接 raise ValueError；`no_placeholder_raw` pre-sanitize check 加入 drill | **PASS** | drill_report.json `no_placeholder_raw=true` for all 13 scenarios |
| C-6 | S-A5c negative scenario 補入 drill（log 含 CacheMarketDataProvider init text，無 FAILED/RuntimeError → triggered=false）；scenarios 12 → 13 | **PASS** | drill_report.json S-A5c triggered=false, result=PASS；REVIEW-009d_NUMBERS.json `"S-A5c": true` |

### 3. Test Results

| Suite | Count | 結果 | 環境 |
|---|---|---|---|
| tests.forward_record（含 test_alerting + test_alert_e2e_drill） | 54 + 22 + 21 = 97 | PASS | Windows（Codex） |
| tests.monitor.test_channels | 13 | PASS | Windows（Codex）；Linux sandbox 有 stale .pyc artifact（見 §4） |
| Drill scenarios | 13/13 | PASS | S-A1~A7 + S-A1b/A3b/A4b/A6b + S-A5c 全部符合預期 |

### 4. Sandbox Artifact 判定（非 code regression）

Linux sandbox 執行 `tests.monitor.test_channels` 時出現 4 項失敗，根因為 NTFS→Linux mount sync lag 導致 `apps/monitor/config.py` 被截斷（7,395 bytes）並觸發 stale `.pyc` 讀取。Opus 裁定：**infrastructure noise，非 code regression，不阻擋 TASK-009c DONE**。Windows 環境 13/13 PASS 已記錄於 COMMAND_LOG.md（2026-05-18 08:25 entry）。

### 5. Open Notes（non-blocking）

- **NOTE-1**：`REVIEW_NUMBERS_PATH` legacy constant 保留於 `alerting.py` 作為平滑遷移 fallback；`configs/forward_record.yaml` 現已存在，fallback 不會被觸發。可於未來 cleanup sprint 移除。
- **NOTE-2**：`drill_report.json` 頂層 `"task": "TASK-009d"` tag 為 TASK-009d 原始 drill script 所設，TASK-009c re-run 未更新。對內容驗證無影響，可於 cleanup sprint 修正。

### 6. Safety Gate 彙總

| Gate | 狀態 |
|---|---|
| Bybit 連接 | NOT_ATTEMPTED |
| Discord 真實 POST | NOT_ATTEMPTED |
| --live-alerts | NOT_ATTEMPTED |
| 30-day forward clock | NOT_STARTED |
| Paper execution | FORBIDDEN |
| Live trading | FORBIDDEN |
| 策略訊號修改 | 未修改 |
| `apps/paper_trading/` 修改 | 未修改 |
| immutable run outputs 修改 | 未修改 |

### 7. Audit Statement

- `apps/forward_record/alert_conditions.py`、`alerting.py`、`configs/forward_record.yaml`、`scripts/drill_forward_alerts.py`、`tests/forward_record/test_alerting.py`、`tests/forward_record/test_alert_e2e_drill.py` 全讀（COMMAND_LOG + Sonnet draft 確認）
- REVIEW-009c_DRAFT_BY_SONNET.md（6 section）、REVIEW-009c_NUMBERS.json、REVIEW-009d_NUMBERS.json 讀取確認
- 未執行：Discord 實際 POST、--live-alerts、Bybit 連接、30-day clock 啟動、paper/live execution 批准、策略訊號修改
...
| FG-4 | FORBIDDEN_bybit_write = NOT_ATTEMPTED | PASS | 無任何 bybit import |
| FG-5 | dry_run 預設 True | PASS | force_dry_run: bool = True；live_alerts: bool = False |
| FG-6 | 雙重 dry_run gate 邏輯正確 | PASS | `True if force_dry_run or not live_alerts else discord_channel.dry_run` — 三條件同時成立才允許 POST |
| FG-7 | 全部 7 項 alert conditions 實作（A-1~A-7） | PASS | REVIEW-009b_NUMBERS.json 全部 IMPLEMENTED |
| FG-8 | 15/15 tests PASS | PASS | pytest 直接執行；15 collected，15 passed |
| FG-9 | clock_started 不變動 | PASS | alerting.py / alert_conditions.py 無任何 clock_started mutation |
| FG-10 | 驗證期間無實際 Discord POST | PASS | external_post_attempted=false；live_alerts_used_in_validation=false |

### 3. Warning Rulings（3 件，全部 CAVEAT）

| Warning | 說明 | Opus 裁定 | 後續處置 |
|---|---|---|---|
| W-1 | A-5 `CacheMarketDataProvider` log marker 潛在 false positive | CAVEAT | 歸 TASK-009c：marker 改為精準字串或加白名單 |
| W-2 | `_extract_yyyymmdd()` path parsing 取第一個 8 位數字序列，路徑格式變更時脆弱 | CAVEAT | 歸 TASK-009c：邊界測試 + 改用 pathlib / 佔位符 |
| W-3 | `REVIEW_NUMBERS_PATH` 依賴 review artifact（REVIEW-009_NUMBERS.json）而非 runtime config | CAVEAT | 歸 TASK-009c：output path 搬至 runtime config（yaml） |

### 4. Architecture Highlights

- **AlertConditionResult dataclass**：每個 condition 獨立可測，to_dict() 直接序列化至 alert_log.json
- **A-6 duplicate suppression**：讀取前日 alert_log 的 alerts_sent，同日只通知一次 review_006b_trigger_ready
- **A-7 last-resort gate**：_safety_fields() hardcoded NOT_ATTEMPTED，A-7 只在程式碼 bug 引入時觸發
- **_discord_channel() fallback**：無 discord channel 時回傳 enabled=False dry_run=True，不 raise exception
- **Triple dry_run gate**：CLI --dry-run → force_dry_run → yaml dry_run:true，三條件均需解除才能實際 POST

### 5. TASK-009c / TASK-009d 建議

- **TASK-009c**（XS）：W-1/W-2/W-3 三項 caveat 的技術修補；marker 精準化 + yyyymmdd 邊界測試 + path 搬 config
- **TASK-009d**（S；30-day clock 前置必辦）：alert delivery 端對端演練；VPS 上用真實 --live-alerts 驗證 Discord 實際送達，驗證後改回 dry_run

### 6. Audit Statement

- 程式碼全讀：alerting.py（197 行）、alert_conditions.py（294 行）、test_alerting.py（267 行）、run_forward_record.py（末端整合）
- pytest 直接執行確認：15/15 PASS
- safety scan 直接執行：scan_no_order_endpoints → violations=[]
- Codex 完成報告讀取：REVIEW-009b_PACKET.md、REVIEW-009b_NUMBERS.json
- 未執行：Bybit 連接、Discord 實際 POST、30-day clock 啟動、paper/live execution 批准、策略程式修改
...
**Date**：2026-05-17  
**對應任務**：TASK-005 → **DONE**  
**審查依據**：Sonnet REVIEW-005 draft + B-1 hotfix check（v2.0）→ Opus final decision
...
| Verdict | **PASS** |
| TASK-005 狀態 | **DONE** |
| `single_channel_only` 定性 | **Caveat（非 Blocking）** |
| TASK-005a 建立 | **TODO**（paper execution gate 條件之一）|
| Paper execution | **FORBIDDEN** |
| Live trading | **FORBIDDEN** |

---

### 2. Fail Gates（全 9 項 = false）
...
| Gate | 結果 | 說明 |
|---|---|---|
| `single_channel_only` | ⚠️ TRIGGERED | 只有 `local_jsonl (dry_run=true)`；無真實推播 |
...
### 4. Observer-Only Scope 確認

- 無下單程式碼（`forbidden_token_violations = []`）✅
- 無 exchange 連線（`exchange_connection_made = false`）✅
- 無 API key 請求（`api_key_requested = false`）✅
- 無 paper/live 啟動（`paper_execution_started = false`、`live_trading_started = false`）✅
- 無 bot auto-restart（`monitor_auto_restart_present = false`）✅
- `apps/monitor/` 獨立，未侵入 `src/` 或 `apps/paper_trading/` ✅
- `.gitignore` 已保護 4 個 secret pattern ✅
...
| 項目 | 值 |
|---|---|
| `heartbeat.parquet` rows | 1（sample）|
| `alerts JSONL` rows | 1（sample）|
| `interval_seconds` | 60（符合工單 ≤60）|
| `failure_threshold` | 3（符合工單）|
| `dedup_window_minutes` | 30（符合工單）|
| `equity_floor_usd` | 8000（對齊 TASK-006 NAV floor）|
| `reproducibility_hash` | `25cbf9c172b7bf377974e0fd1d568d57a888c8b090c25049f460b3c2ca42a606` |

---

### 6. B-1 Hotfix 說明

Sonnet REVIEW-005 draft 初次發現 `.gitignore` 截斷（`configs/monitor_secre`），safety scan 實際回傳 FAIL。後確認為 Linux sandbox mount 快取問題——Codex 已在 Windows 檔案系統修正，Read 工具（直接讀 Windows 路徑）確認 4 個 secret pattern 完整。B-1 已 CLOSED。

---

### 7. Opus Final Decision

#### `single_channel_only`（Q2）裁定：Caveat，非 Blocking

`single_channel_only` 定性為 **Warning + caveat**（非 Blocking）：
- TASK-005 研究基建已完整交付（架構、schema、safety scan、tests 全 PASS）。
- 推播 channel 的缺失不影響監控資料完整性，只影響即時通知能力。
- Paper trading 尚未開始；VPS 正式上線時同步接通推播即可。
- **TASK-005a（Real Alert Channel）**設為 TODO，作為 paper execution 前置條件之一（除非 Rick 明示豁免）。

#### Paper Execution Gate 現況（更新後）

| 前置條件 | 狀態 |
|---|---|
| TASK-007b DONE | ✅ 2026-05-17 |
| TASK-005 DONE | ✅ 2026-05-17 |
| TASK-005a Real Alert Channel | ❌ TODO |
...
- **TASK-005**：→ DONE
- **TASK-005a**：→ TODO（Real Alert Channel；paper execution gate 條件之一）
- **Paper execution**：仍 FORBIDDEN
- **Live trading**：仍 FORBIDDEN
- **REVIEW-006b 啟動條件**：3/3 前置滿足（TASK-007b ✅、addenda ✅）；最大阻擋 = 30 天 forward paper record

---

### 9. 審查基礎聲明

- 本審查依 Token Budget Rule，只讀 PACKET.md + NUMBERS.json + .gitignore + heartbeat.parquet（schema）+ alerts.jsonl（sample）+ setup.log + monitor.yaml + safety.py。
- 未修改任何策略程式、官方輸出、raw data。
- 未重跑任何任務。
- 未批准 paper execution；未批准 live trading。
- 未標記 TASK-005a DONE（TASK-005a 為 TODO）
...
| TASK-009 狀態 | **DONE** |
| Fail gates | **0 / 10 觸發** |
| W-1 secret scan false positive | **CAVEAT（非 BLOCKING）** — `api_key_request=NOT_ATTEMPTED` substring 誤判；建議 TASK-009a 補白名單 |
| W-2 dry-run primary = shadow weights | **CAVEAT（非 BLOCKING）** — 該日 TASK-008 alpha cap 未觸發；建議 TASK-009a 加 `alpha_cap_triggered_today` 欄位 |
| W-3 working tree uncommitted diffs | **CAVEAT（非 BLOCKING）** — task007 CSV + trading.db 既有差異；Rick 啟動 forward clock 前需 clean working tree |
| Runner readiness | **READY**（dry-run REVIEW_READY，11/11 unit tests PASS）|
| 30-day clock | **NOT_STARTED** |
| Paper execution | **FORBIDDEN** |
| Live trading | **FORBIDDEN** |
...
#### W-1：secret scan false positive（CAVEAT）

`scan_no_secrets_in_outputs()` 對共享 log 目錄回傳 FAIL，原因：
- `20260517_forward_record.log` 含 `api_key_request=NOT_ATTEMPTED`（status 訊息，無真實 key 值）
- 既有 TASK-005/005a log 含 `api_key`、`token`、`webhook`（不在 TASK-009 掃描範圍）

Runner 自身的 `run_safety_scan()` 範圍正確，`REVIEW-009_NUMBERS.json safety_scan.status = PASS`。

**Opus 裁定**：CAVEAT（非 BLOCKING）。建議 TASK-009a 補白名單（`api_key_request` → allowlist）或限縮掃描範圍。

#### W-2：dry-run primary = shadow weights（CAVEAT）

Dry-run 日期（20260430）的 primary 與 shadow positions 權重完全相同（0 weight differences），原因是該日 TASK-008 alpha-space cap 未觸發任何 symbol（variant_weight = original_weight for all symbols）。

**Opus 裁定**：CAVEAT（非 BLOCKING）。預期行為。建議 TASK-009a 在 positions.parquet 加入 `alpha_cap_triggered_today` bool 欄位，以便 live run 時明確記錄 cap 是否觸發。

#### W-3：working tree 既有未 commit 差異（CAVEAT）

`outputs/variants/prev3y_crypto/20260515_task007_variant_concentration.csv` 與 `data/trading.db` 有未 commit 的 git 差異，與 TASK-009 runner 無關（runner 不寫入這些路徑）。

**Opus 裁定**：CAVEAT（非 BLOCKING）。Rick 在啟動 30-day forward clock 之前，需確保 working tree clean（建議 `git stash` 或 `git commit` 現有差異），以確保 forward record 起始狀態明確可追溯。

---

### 4. 架構審查亮點

- `apps/forward_record/primary.py` 正確 import `apps/paper_trading/overlay.py`（不重複實作 overlay logic）
- `ForwardRecordConfig` 繼承 `PaperTradingConfig`（via `paper_config` field），參數來源單一
- Shadow track 使用 `task008_variant_detail.csv` 的 `A_roll12_share20_exclude` weights，輸出到獨立目錄
- `BybitReadOnlyMarketDataProvider.allow_network=False` → 拋出 `RuntimeError`（完全禁止非 cache 的 API 呼叫）
- `gate_checker.evaluate_gates()` 正確實作 W-1~W-6 / S-1~S-6，邊界值均有測試覆蓋
- Reproducibility hash 存在，primary ≠ shadow hash（正確）

---

### 5. TASK-009a / TASK-009b 建議

**TASK-009a（小修補，non-blocking）**：
- `scan_no_secrets_in_outputs()` 補 `api_key_request` 白名單，或限縮掃描範圍至 runner 直接寫入的檔案
- `positions.parquet` 加 `alpha_cap_triggered_today: bool` 欄位（shadow track 可觀測性）

**TASK-009b（forward monitor alerting，forward clock 啟動前置）**：
- 實作每日 gate check → monitor alert 的整合（W-1~W-6 / S-1~S-6 → TASK-005 Discord channel）
- `forward_summary.json` review_006b_trigger_ready = true 時自動通知 Rick
- 此任務為 30-day forward clock 啟動前的必要前置（clock 啟動後若無 alerting，Stop gate 無法及時觸發通知）

---

### 6. 審查基礎聲明

- 未修改 `src/signals/prev3y_momentum.py` 或任何主策略程式。
- 未重跑 baseline / cost stress / attribution / TASK-008 variants。
- 未連接 Bybit；未要求或使用任何 API key。
- 未啟動 30-day forward clock。
- 未批准 paper execution；未批准 live trading。
...
| Verdict | **CONDITIONAL_PASS** |
| TASK-008 狀態 | **DONE** |
| W-1 top5_conc 87.95% > 75% | **CAVEAT（非 BLOCKING）** — top5 < 75% 改為 observation metric |
| 實務可接受區間 | top5_conc ≤ 90% 且 single_conc < 25%（需 Rick 最終簽核）|
| 推薦 variant | **A_roll12_share20_exclude**（Pareto-dominant）|
| Variant B | REJECTED（集中度幾乎無改善）|
| Variant C | REJECTED + 永久封存（no_DOT paradox 第二次正式重現）|
| Paper execution | **FORBIDDEN** |
| Live trading | **FORBIDDEN** |
...
| Gate | 結果 |
|---|---|
| F-1 Baseline reconciliation ≤ 1e-10 | PASS — 5.55e-17 ✅ |
| F-2 paper/live FORBIDDEN | PASS ✅ |
| F-3 No weight-space overlay | PASS ✅ |
| F-4 prev3y_momentum.py 主流程未修改 | PASS ✅ |
| F-5 Attribution 公式一致 | PASS ✅ |
| F-6 top5_conc ≤ 100%（valid variants）| PASS ✅ |
| F-7 All outputs present | PASS ✅ |
| F-8 Reproducibility | PASS — hash `4074c89b…` ✅ |

---

### 3. 推薦 Variant：A_roll12_share20_exclude

| 指標 | Baseline | A_roll12_share20_exclude | Δ |
|---|---|---|---|
| Sharpe (active) | 0.8918 | **0.9636** | +8.0% |
| top5_conc | 95.56% | **87.95%** | −7.61pp（W-1 caveat）|
...
| cost_impact | — | **−9.73 bps** | 成本降低 |
| turnover_change | 1.00× | 0.964× | 略降 |

**參數退化紀錄**：A_roll12_share20_exclude = A_roll24_share20_exclude = A_roll12_share20_penalize50（三者完全等效）。推薦使用 A_roll12_share20_exclude 作為 canonical specification。

---

### 4. Variant B 結論

Variant B（alpha-share-based position sizing）因等權基準使 weight 調整空間極小，top5_conc 僅從 95.56% 降至 95.22%（0.34pp），集中度改善可忽略。**REJECTED — 攻擊的是錯誤的 lever（weight magnitude），而非根本問題（symbol selection）。**

---

### 5. Variant C 結論 + 永久封存

Variant C（cooldown blacklist，k≥6）在移除高 alpha contributor 後，alpha 分母崩潰，top5_conc 爆炸至 642%，Sharpe 降至 0.155，alpha_retention 僅 18.75%。

**no_DOT paradox 第二次正式重現（規模版本）：**
- TASK-007 首現：移除 DOT → top5 95.56% → 116.13%（+20.57pp）
- TASK-008 Variant C：移除連續高貢獻者 → top5 高達 642%

**結構性解釋**：動量策略的 alpha 本質上集中於高 momentum 符號。強制移除這些符號使剩餘 alpha pool 極小，attribution 分母縮水幅度遠超分子，集中度比例反向爆炸。這不是實作缺陷，而是動量策略的數學結構性質。

**Variant C 永久封存**：不得在此策略中重做任何形式的 cooldown/blacklist 機制，除非整個策略框架重設計。

---

### 6. W-1 Caveat 裁定（Opus）

top5_conc < 75% 從 TASK-008 hard target 改為 observation metric：
- Prev3Y momentum 策略的 top5_conc 結構性下限約 87–90%（Variant C 悖論證明）
- 實務可接受區間：**top5_conc ≤ 90% 且 single_conc < 25%**（需 Rick 最終簽核）
- A_roll12_share20_exclude 已達 87.95%，屬可接受區間內

---

### 7. TASK-006 Paper Runbook 更新

A_roll12_share20_exclude 加入 TASK-006 paper runbook 的 **secondary / shadow-track candidate**：
- Primary spec 不更動：`combined_paper_safe_variant`（REVIEW-007 確認）
- Secondary spec（shadow stats）：`A_roll12_share20_exclude`
- 30-day forward record 期間，同時記錄 A_roll12_share20_exclude 的 shadow statistics（不影響主軌操作）
- REVIEW-006b 時一併呈現兩軌比較數字

---

### 8. TASK-008b 狀態

TASK-008b 暫不開工，列入 backlog。若未來 Rick 決定追求 top5_conc < 75%，再開啟。
...
- 未修改 src/signals/prev3y_momentum.py 或任何主策略程式。
- 未重跑 baseline / cost stress / attribution。
- 未修改官方輸出（outputs/backtests/ 等）。
- 未啟用 weight-space cap / redistribution。
- 未批准 paper execution；未批准 live trading。
- 未標記 TASK-008 DONE（由本記錄授權後執行）。

---

## REVIEW-005a — Real Alert Channel（2026-05-17）
...
| Verdict | **PASS** |
| TASK-005a 狀態 | **DONE** |
| `external_channels_dry_run_only` 定性 | **Caveat for TASK-005a DONE；但 Blocker for paper execution unlock** |
| local_jsonl 保留 | ✅ 保留（WRITTEN，delivered_count=1）|
...
| configs/monitor_secrets.local.yaml | ✅ 不存在（正確）|
| Exchange API | ❌ 無 |
| Order submission | ❌ 無 |
| Auto-restart | ❌ 無 |
| Paper execution | **FORBIDDEN** |
...
| Gate | 結果 | 定性 |
|---|---|---|
| `external_channels_dry_run_only` | ⚠️ TRIGGERED | **Caveat（TASK-005a DONE 不受阻）；Blocker for paper execution unlock** |
...
- **local_jsonl**：WRITTEN，delivered_count=1，external_post_attempted=false，local_jsonl_retained=true ✅
- **Telegram**：DRY_RUN，external_post_attempted=false，endpoint=`<redacted>`，token redacted ✅
- **Discord**：DRY_RUN，external_post_attempted=false，endpoint=`<redacted>`，webhook redacted ✅
- **configs/monitor_secrets.local.yaml**：未建立（正確）✅
- **configs/monitor_secrets.example.yaml**：存在，含空值佔位符 ✅

---

### 5. Safety Scan — PASS

`safety_scan.status = PASS`；`secret_ignore.status = PASS`（4 patterns 由 Read tool 確認）。
...
- Packet-reported：`fail_gates.test_failure = false`（13 tests，Windows 環境 PASS）
- Bash sandbox 觀察（informational）：2 FAIL + 3 ERROR，均判定為 mount cache / stale .pyc 環境 artifacts，非真實程式碼缺陷。
- Reproducibility hash：`06a28f791dbfeb931a35dadf1eb856f92c791d0bf8648b09ba004da5b8d58817`（三源一致）

---

### 7. Rick 手動驗證 Gate（paper execution 前置條件）

在 paper execution 啟動前，Rick 必須完成以下手動驗證：

- **Rick 必須對至少一個外部 channel 執行真實 `--test-send`**（Telegram 或 Discord）。
- **Evidence 必須以已遮蔽敏感資訊的形式儲存至**：
  `outputs/monitor/test_send/<YYYYMMDD>_<channel>_proof.txt`
- 這是 Rick ops 驗證，**不是** Codex 任務。
- **不得請求 Rick 在 chat 貼上 token / webhook URL。**

---

### 8. Paper Execution Gate 現況（更新後）

| 前置條件 | 狀態 |
|---|---|
| TASK-007b DONE | ✅ 2026-05-17 |
| TASK-005 DONE | ✅ 2026-05-17 |
| TASK-005a Real Alert Channel DONE | ✅ 2026-05-17（REVIEW-005a PASS）|
| Rick 真實 --test-send 證據（≥1 channel）| ❌ NOT_DONE |
| TASK-006 三個補件落地 | ✅ 2026-05-17 |
| 30 天 forward paper record（Sharpe > 0.5）| ❌ NOT_STARTED |
| Opus REVIEW-006b PASS | ❌ 未開啟 |
| Rick 明示批准 | ❌ 未批准 |
...
- **TASK-005a**：→ DONE
- **external_channels_dry_run_only**：Caveat（不擋 TASK-005a DONE），但 Blocker for paper execution unlock
- **Paper execution**：仍 FORBIDDEN（Rick test-send + 30 天 forward record + REVIEW-006b + Rick 批准等條件未達成）
- **Live trading**：仍 FORBIDDEN（不變）
...
- 本 final decision 依 CLAUDE_COMMANDS.md#record-review-005a-final 執行。
- 所有決定依 Opus final decision summary 照實記錄。
- 未修改策略程式、官方輸出或 raw data。
- 未重跑任何任務。
- 未批准 paper execution；未批准 live trading。
- 未執行真實 --test-send；未要求 Rick 貼 token / webhook。。
