...
SUPPORTED_WS_SCHEMA_VERSIONS = frozenset({ws.SCHEMA_VERSION})  # {1}
...
def validate_ws_artifact_compatibility(
    ...
    *,
    ...
    # 1. Schema / version / lineage
    if str(ws_artifact.get("schema", "")) != SUPPORTED_WS_SCHEMA:
        failures.append("ws_schema_unsupported")
    if ws_artifact.get("schema_version") not in SUPPORTED_WS_SCHEMA_VERSIONS:
        failures.append("ws_schema_version_unsupported")
    ...
    # 7. Plan-only run compatibility (date / policy / strategy / symbols)
    if str(ws_artifact.get("date", requested_strategy_date)) not in (
            "", str(requested_strategy_date)):
        # The WS artifact does not carry the strategy date itself; this guards a
        # future schema that adds it. Absence is not a failure.
        failures.append("ws_artifact_date_incompatible")
    ...
    if strategy_symbol_source_fingerprint is not None:
        ws_fp = universe.get("symbol_universe_fingerprint")
        # Only compared when an explicit expectation is supplied by the caller.
        if str(strategy_symbol_source_fingerprint) != str(ws_fp):
            failures.append("strategy_symbol_source_fingerprint_mismatch")
...
__all__ = [
    ...
    "SUPPORTED_WS_SCHEMA", "SUPPORTED_WS_SCHEMA_VERSIONS", "PLANNER_PRICE_FIELD",
