"""
src/demo_tiny_guarded_cleanup_dry_run_adapter.py
TASK-014AG: Guarded Cleanup-only Dry-run Adapter.

Pure-computation / dry-run adapter for the future guarded
`guarded_cleanup_only` command.  Consolidates 14 upstream artifacts
...
tiny_guarded_entry_dry_run_adapter /
tiny_guarded_stop_attach_dry_run_adapter) and emits a dry-run adapter
artifact answering: if some future task ever wants to graduate to a
guarded real *cleanup* (cleanup only --- no entry, no stop attach, no
full lifecycle), what inputs, preconditions, manual confirmations,
preview cleanup-request envelope, readonly verification plan, failure
policy, and audit artifacts are required?

This module DOES NOT implement, design, or invoke a real cleanup
sender.  No /v5/order/create call, no /v5/position/trading-stop call,
no order send, no position modification, no real cleanup, no close-only
sender reuse, no emergency close, no leverage / transfer / withdraw, no
token validation, no socket open, no .env read, no dotenv, no signing.
It does not modify main.py, src/risk.py, or BybitExecutor.

Stages:

  stage_0_artifact_preflight
      Validate 14 upstream artifacts + runtime proof envelope + the
      TASK-014AD guarded_runner_design_review readiness_conclusion is
      DESIGN_REVIEW_READY_NOT_EXECUTABLE + the TASK-014AE
      guarded_entry_dry_run_adapter status is acceptable + the
      TASK-014AF guarded_stop_attach_dry_run_adapter status is
      acceptable.

  stage_1_cleanup_adapter_scope
      Assert cleanup-only dry-run adapter scope.  Not full lifecycle;
      no entry included; no stop attach; no real cleanup implemented;
      no endpoint invoked; no position modified; no secrets loaded.

  stage_2_cleanup_precondition_contract
      Document the preconditions any future guarded_cleanup_only
      command must satisfy: existing tiny entry symbol=SOLUSDT,
      side=long, qty=0.1, cleanup side=Sell, reduceOnly=True,
      closeOnTrigger=False, positionIdx=0, orderType=Market,
      category=linear, max_notional_usdt=10, 5 protected positions
  ...
  stage_3_manual_confirmation_dry_run_contract
      Document the cleanup token pattern
      (CONFIRM_DEMO_TINY_CLEANUP_*), and the
      ...
      --expected-cleanup-symbol SOLUSDT, --expected-cleanup-qty 0.1,
      --expected-cleanup-side Sell, --expected-reduce-only true
      flags.  Tokens / flags are NEVER validated by this task.

  stage_4_cleanup_request_envelope_dry_run
      Build a single preview-only cleanup-request envelope.
      ...
      qty=0.1, reduceOnly=True, closeOnTrigger=False, positionIdx=0,
      orderLinkId carries DRYRUN prefix.  No sender adapter.

  stage_5_cleanup_readonly_verification_plan
      Pre-cleanup readonly required.  Post-cleanup readonly required.
      Pre-cleanup expects selected_symbol PRESENT with qty=0.1 /
      side=long and 5 protected positions intact.  Post-cleanup
      expects selected_symbol ABSENT or qty=0 and 5 protected
      positions unchanged.  Real readonly after execution is NOT
      performed in this task --- verification_plan_only=True.

  stage_6_cleanup_failure_policy
      Future guarded_cleanup_only failure policy: request rejected /
      selected symbol absent / selected symbol side mismatch /
      selected symbol qty mismatch / cleanup side invalid / reduceOnly
      missing or false / partial cleanup / readonly unavailable /
      live endpoint detected / secret emission detected => FAIL_CLOSED.
      Existing protected position mismatch => MANUAL_REVIEW_REQUIRED.
      No auto retry / second cleanup / emergency close / entry /
      stop attach / next step.

  stage_7_audit_artifact_generation
      Audit artifacts emitted: precondition_contract /
      manual_confirmation_contract / cleanup_request_envelope /
      readonly_verification_plan / failure_policy /
      final_cleanup_adapter_verdict.  response status fixed to
      DRY_RUN_NOT_SENT, response_from_exchange=False, sanitized=True,
      no secrets.

  stage_8_final_cleanup_adapter_verdict
      Resolve final status:
          TINY_GUARDED_CLEANUP_DRY_RUN_ADAPTER_READY        (default)
      ...
      real_cleanup_implemented=False, send_allowed=False,
      order_endpoint_called=False, stop_endpoint_called=False,
      no_position_modified=True, no_secrets_loaded=True,
      g20_lifted=False.

Modes:
  cleanup_adapter_checklist           --- default
  cleanup_dry_run_approval            --- --allow-cleanup-dry-run-approval
  real_cleanup_execution_guard        --- --allow-real-cleanup-execution
...
DRYRUN_ORDER_LINK_ID_PREFIX = "DRYRUN_TINY_CLEANUP_"
...
STAGE_1_CLEANUP_ADAPTER_SCOPE                = "stage_1_cleanup_adapter_scope"
STAGE_2_CLEANUP_PRECONDITION_CONTRACT        = "stage_2_cleanup_precondition_contract"
STAGE_3_MANUAL_CONFIRMATION_DRY_RUN_CONTRACT = "stage_3_manual_confirmation_dry_run_contract"
STAGE_4_CLEANUP_REQUEST_ENVELOPE_DRY_RUN     = "stage_4_cleanup_request_envelope_dry_run"
STAGE_5_CLEANUP_READONLY_VERIFICATION_PLAN   = "stage_5_cleanup_readonly_verification_plan"
STAGE_6_CLEANUP_FAILURE_POLICY               = "stage_6_cleanup_failure_policy"
STAGE_7_AUDIT_ARTIFACT_GENERATION            = "stage_7_audit_artifact_generation"
STAGE_8_FINAL_CLEANUP_ADAPTER_VERDICT        = "stage_8_final_cleanup_adapter_verdict"
...
STATUS_ADAPTER_READY               = "TINY_GUARDED_CLEANUP_DRY_RUN_ADAPTER_READY"
STATUS_ADAPTER_READY_EXEC_DISABLED = (
    "TINY_GUARDED_CLEANUP_DRY_RUN_ADAPTER_READY_BUT_EXECUTION_DISABLED"
)
STATUS_REAL_CLEANUP_NOT_IMPL       = "REAL_CLEANUP_EXECUTION_NOT_IMPLEMENTED"
STATUS_FAIL_CLOSED                 = "FAIL_CLOSED"

MODE_CLEANUP_ADAPTER_CHECKLIST     = "cleanup_adapter_checklist"
MODE_CLEANUP_DRY_RUN_APPROVAL      = "cleanup_dry_run_approval"
MODE_REAL_CLEANUP_EXECUTION_GUARD  = "real_cleanup_execution_guard"
...
# ---------------------------------------------------------------------------
# Future guarded cleanup payload contract (documentation only)
# ---------------------------------------------------------------------------

EXPECTED_PRE_CLEANUP_SYMBOL      = "SOLUSDT"
EXPECTED_PRE_CLEANUP_SIDE        = "long"
EXPECTED_PRE_CLEANUP_QTY         = 0.1
EXPECTED_CLEANUP_SIDE            = "Sell"
EXPECTED_CLEANUP_ORDER_TYPE      = "Market"
...
# ---------------------------------------------------------------------------
# Gate constants
# General (26) + Scope (11) + Precondition (16) + Manual confirmation (12)
# + Envelope (15) + Readonly plan (10) + Failure policy (17) + Audit (5)
# + Execution guard (5) = 117
# ---------------------------------------------------------------------------
...
# Scope gates (11)
GATE_GUARDED_CLEANUP_DRY_RUN_ADAPTER_ONLY       = "guarded_cleanup_dry_run_adapter_only"
GATE_CLEANUP_ONLY                               = "cleanup_only"
...
GATE_REAL_CLEANUP_NOT_IMPLEMENTED               = "real_cleanup_not_implemented_in_this_task"
...
# Precondition gates (16)
GATE_PRECONDITION_SYMBOL_MATCHES                = "precondition_symbol_matches"
GATE_PRECONDITION_EXPECTED_CLEANUP_SYMBOL       = "precondition_expected_cleanup_symbol_solusdt"
GATE_PRECONDITION_SIDE_LONG                     = "precondition_pre_cleanup_side_long"
GATE_PRECONDITION_QTY_TINY                      = "precondition_pre_cleanup_qty_tiny"
GATE_PRECONDITION_CLEANUP_SIDE_SELL             = "precondition_cleanup_side_sell"
GATE_PRECONDITION_REDUCE_ONLY_TRUE              = "precondition_reduce_only_true"
GATE_PRECONDITION_CLOSE_ON_TRIGGER_FALSE        = "precondition_close_on_trigger_false"
...
GATE_PRECONDITION_MAX_NOTIONAL_CAP              = "precondition_max_notional_cap_10"
GATE_PRECONDITION_SELECTED_SYMBOL_PRESENT       = "precondition_selected_symbol_present_before_cleanup"
...
GATE_EXPECTED_CLEANUP_SYMBOL_FLAG_DOCUMENTED    = "expected_cleanup_symbol_flag_documented"
GATE_EXPECTED_CLEANUP_QTY_FLAG_DOCUMENTED       = "expected_cleanup_qty_flag_documented"
GATE_EXPECTED_CLEANUP_SIDE_FLAG_DOCUMENTED      = "expected_cleanup_side_flag_documented"
GATE_EXPECTED_REDUCE_ONLY_FLAG_DOCUMENTED       = "expected_reduce_only_flag_documented"
GATE_CONFIRMATION_FLAGS_NOT_VALIDATED           = "confirmation_flags_not_validated"

# Envelope gates (15)
GATE_CLEANUP_ENVELOPE_PRESENT                   = "cleanup_envelope_present"
...
GATE_ENVELOPE_SIDE_SELL                         = "envelope_side_sell"
GATE_ENVELOPE_QTY_TINY                          = "envelope_qty_tiny"
GATE_ENVELOPE_REDUCE_ONLY_TRUE                  = "envelope_reduce_only_true"
...
# Readonly plan gates (10)
GATE_PRE_CLEANUP_READONLY_REQUIRED              = "pre_cleanup_readonly_required"
GATE_POST_CLEANUP_READONLY_REQUIRED             = "post_cleanup_readonly_required"
GATE_PRE_CLEANUP_SELECTED_SYMBOL_PRESENT        = "pre_cleanup_selected_symbol_present"
GATE_PRE_CLEANUP_EXPECTED_SYMBOL                = "pre_cleanup_expected_symbol_solusdt"
GATE_PRE_CLEANUP_EXPECTED_QTY                   = "pre_cleanup_expected_qty_tiny"
GATE_PRE_CLEANUP_EXPECTED_SIDE_LONG             = "pre_cleanup_expected_side_long"
GATE_POST_CLEANUP_SELECTED_SYMBOL_ABSENT        = "post_cleanup_selected_symbol_absent_or_zero"
...
GATE_CLEANUP_SIDE_INVALID_FAIL_CLOSED           = "cleanup_side_invalid_fail_closed"
GATE_REDUCE_ONLY_INVALID_FAIL_CLOSED            = "reduce_only_invalid_fail_closed"
GATE_PARTIAL_CLEANUP_FAIL_CLOSED                = "partial_cleanup_fail_closed"
...
GATE_REAL_CLEANUP_EXECUTION_NOT_IMPL            = "real_cleanup_execution_not_implemented"
...
@dataclass
class TinyGuardedCleanupDryRunAdapterResult:
    """Read-only outcome of one guarded cleanup dry-run adapter pass."""
    timestamp_utc:                str
    mode:                         str
    selected_symbol:              str
    ...
    cleanup_adapter_scope:                  dict[str, Any] = field(default_factory=dict)
    cleanup_precondition_contract:          dict[str, Any] = field(default_factory=dict)
    manual_confirmation_dry_run_contract:   dict[str, Any] = field(default_factory=dict)
    cleanup_request_envelope:               dict[str, Any] = field(default_factory=dict)
    cleanup_readonly_verification_plan:     dict[str, Any] = field(default_factory=dict)
    cleanup_failure_policy:                 dict[str, Any] = field(default_factory=dict)
    audit_artifacts:                        dict[str, Any] = field(default_factory=dict)
    final_cleanup_adapter_verdict:          dict[str, Any] = field(default_factory=dict)
    ...
    cleanup_dry_run_approval_allowed: bool = False
    real_cleanup_execution_requested: bool = False
    real_execution_allowed:           bool = False
    real_cleanup_implemented:         bool = False
    guarded_cleanup_dry_run_adapter:  bool = True
    ...
    next_required_task:           str  = "TASK-014AH_guarded_tiny_lifecycle_dry_run_summary"
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "cleanup_adapter_scope":           dict(self.cleanup_adapter_scope),
            "cleanup_precondition_contract":   dict(self.cleanup_precondition_contract),
            "manual_confirmation_dry_run_contract": dict(self.manual_confirmation_dry_run_contract),
            "cleanup_request_envelope":        dict(self.cleanup_request_envelope),
            "cleanup_readonly_verification_plan": dict(self.cleanup_readonly_verification_plan),
            "cleanup_failure_policy":          dict(self.cleanup_failure_policy),
            "audit_artifacts":                 dict(self.audit_artifacts),
            "final_cleanup_adapter_verdict":   dict(self.final_cleanup_adapter_verdict),
            "cleanup_token_pattern":           self.cleanup_token_pattern,
            "required_confirmation_flags":     list(self.required_confirmation_flags),
            "cleanup_dry_run_approval_allowed": self.cleanup_dry_run_approval_allowed,
            "real_cleanup_execution_requested": self.real_cleanup_execution_requested,
            "real_execution_allowed":          self.real_execution_allowed,
            "real_cleanup_implemented":        self.real_cleanup_implemented,
            "guarded_cleanup_dry_run_adapter": self.guarded_cleanup_dry_run_adapter,
...
# ---------------------------------------------------------------------------
# Guarded cleanup dry-run adapter
# ---------------------------------------------------------------------------

class DemoTinyGuardedCleanupDryRunAdapter:
    """
    Pure-computation guarded cleanup dry-run adapter.  Consolidates the
    14 upstream artifacts (10 baseline + AA lifecycle summary + AB
    runner design + AC runner dry-run + AD guarded design review + AE
    guarded entry dry-run adapter + AF guarded stop-attach dry-run
    adapter --- minus the entry / stop permission gates, since this
    adapter is cleanup-only) and emits a dry-run adapter artifact
    answering: what inputs, preconditions, manual confirmations,
    request envelope, readonly verification plan, and audit artifacts
    are required for any future guarded_cleanup_only command?
    ...
    design, or describe a real cleanup sender that could be executed.

    --allow-cleanup-dry-run-approval --> status promoted to
        TINY_GUARDED_CLEANUP_DRY_RUN_ADAPTER_READY_BUT_EXECUTION_DISABLED.

    --allow-real-cleanup-execution --> status fixed to
    ...
    def run_checklist(
        ...
        if allow_real_cleanup_execution:
            mode = MODE_REAL_CLEANUP_EXECUTION_GUARD
        elif allow_cleanup_dry_run_approval:
            mode = MODE_CLEANUP_DRY_RUN_APPROVAL
        else:
            mode = MODE_CLEANUP_ADAPTER_CHECKLIST
        ...
        elif sym != EXPECTED_PRE_CLEANUP_SYMBOL:
            blocked.append(GATE_SELECTED_SYMBOL_NOT_SOLUSDT)

        stages[STAGE_0_ARTIFACT_PREFLIGHT] = {
            "stage":   STAGE_0_ARTIFACT_PREFLIGHT,
            "summary": "Validate 14 upstream artifacts + runtime proof envelope + AD readiness + AE entry adapter status + AF stop adapter status.",
            ...
            "selected_symbol":                          sym,
            "selected_symbol_expected":                 EXPECTED_PRE_CLEANUP_SYMBOL,
        ...
        # stage_1_cleanup_adapter_scope
        # ===============================================================
        cleanup_adapter_scope: dict[str, Any] = {
            ...
            "scope_summary": (
                "TASK-014AG only adapts the future guarded_cleanup_only "
                "command into a dry-run artifact.  It does not implement "
                "the real cleanup sender, does not include entry or "
                "stop attach, does not run the full lifecycle, does not "
                "send any order, does not call any endpoint, does not "
                "load any secret, and does not touch any existing "
                "position."
            ),
        }
        stages[STAGE_1_CLEANUP_ADAPTER_SCOPE] = {
            "stage":   STAGE_1_CLEANUP_ADAPTER_SCOPE,
            "summary": "Assert guarded cleanup dry-run adapter scope (cleanup-only).",
            "cleanup_adapter_scope":                cleanup_adapter_scope,
        }
        blocked.append(GATE_GUARDED_CLEANUP_DRY_RUN_ADAPTER_ONLY)
        blocked.append(GATE_CLEANUP_ONLY)
        ...
        blocked.append(GATE_REAL_CLEANUP_NOT_IMPLEMENTED)
        ...
        # stage_2_cleanup_precondition_contract
        # ===============================================================
        sym_eff = sym or EXPECTED_PRE_CLEANUP_SYMBOL
        cleanup_precondition_contract: dict[str, Any] = {
            "symbol":                               EXPECTED_INSTRUMENT_CATEGORY + ":" + sym_eff,
            "selected_symbol":                      sym_eff,
            "expected_cleanup_symbol":              EXPECTED_PRE_CLEANUP_SYMBOL,
            "expected_pre_cleanup_side":            EXPECTED_PRE_CLEANUP_SIDE,
            "expected_pre_cleanup_qty":             EXPECTED_PRE_CLEANUP_QTY,
            "cleanup_side":                         EXPECTED_CLEANUP_SIDE,
            "orderType":                            EXPECTED_CLEANUP_ORDER_TYPE,
            "reduceOnly":                           EXPECTED_REDUCE_ONLY,
            "closeOnTrigger":                       EXPECTED_CLOSE_ON_TRIGGER,
        ...
        stages[STAGE_2_CLEANUP_PRECONDITION_CONTRACT] = {
            "stage":   STAGE_2_CLEANUP_PRECONDITION_CONTRACT,
            "summary": "Document the preconditions any future guarded_cleanup_only command must satisfy.",
            "cleanup_precondition_contract":        cleanup_precondition_contract,
        }
        blocked.append(GATE_PRECONDITION_SYMBOL_MATCHES)
        blocked.append(GATE_PRECONDITION_EXPECTED_CLEANUP_SYMBOL)
        blocked.append(GATE_PRECONDITION_SIDE_LONG)
        blocked.append(GATE_PRECONDITION_QTY_TINY)
        blocked.append(GATE_PRECONDITION_CLEANUP_SIDE_SELL)
        blocked.append(GATE_PRECONDITION_REDUCE_ONLY_TRUE)
        blocked.append(GATE_PRECONDITION_CLOSE_ON_TRIGGER_FALSE)
        ...
        manual_confirmation_dry_run_contract: dict[str, Any] = {
            ...
            "expected_cleanup_symbol_required":     True,
            "expected_cleanup_qty_required":        True,
            "expected_cleanup_side_required":       True,
            "expected_reduce_only_required":        True,
        }
        ...
        blocked.append(GATE_EXPECTED_CLEANUP_SYMBOL_FLAG_DOCUMENTED)
        blocked.append(GATE_EXPECTED_CLEANUP_QTY_FLAG_DOCUMENTED)
        blocked.append(GATE_EXPECTED_CLEANUP_SIDE_FLAG_DOCUMENTED)
        blocked.append(GATE_EXPECTED_REDUCE_ONLY_FLAG_DOCUMENTED)
        blocked.append(GATE_CONFIRMATION_FLAGS_NOT_VALIDATED)

        # ===============================================================
        # stage_4_cleanup_request_envelope_dry_run
        # ===============================================================
        cleanup_request_envelope: dict[str, Any] = {
            "preview_only":                         True,
            "send_allowed":                         False,
            ...
            "orderType":                            EXPECTED_CLEANUP_ORDER_TYPE,
            "qty":                                  EXPECTED_PRE_CLEANUP_QTY,
            "reduceOnly":                           EXPECTED_REDUCE_ONLY,
            "closeOnTrigger":                       EXPECTED_CLOSE_ON_TRIGGER,
            "positionIdx":                          EXPECTED_POSITION_IDX,
            "orderLinkId_prefix":                   DRYRUN_ORDER_LINK_ID_PREFIX,
            "orderLinkId_example":                  DRYRUN_ORDER_LINK_ID_PREFIX + sym_eff,
            "no_sender_adapter":                    True,
        }
        stages[STAGE_4_CLEANUP_REQUEST_ENVELOPE_DRY_RUN] = {
            "stage":   STAGE_4_CLEANUP_REQUEST_ENVELOPE_DRY_RUN,
            "summary": "Build preview-only cleanup-request envelope (no sender adapter, no signature).",
            "cleanup_request_envelope":             cleanup_request_envelope,
        }
        blocked.append(GATE_CLEANUP_ENVELOPE_PRESENT)
        ...
        blocked.append(GATE_ENVELOPE_SIDE_SELL)
        blocked.append(GATE_ENVELOPE_QTY_TINY)
        blocked.append(GATE_ENVELOPE_REDUCE_ONLY_TRUE)
        ...
        # stage_5_cleanup_readonly_verification_plan
        # ===============================================================
        cleanup_readonly_verification_plan: dict[str, Any] = {
            "pre_cleanup_readonly_required":            True,
            "post_cleanup_readonly_required":           True,
            "pre_cleanup_selected_symbol_present":      True,
            "pre_cleanup_expected_symbol":              sym_eff,
            "pre_cleanup_expected_qty":                 EXPECTED_PRE_CLEANUP_QTY,
            "pre_cleanup_expected_side":                EXPECTED_PRE_CLEANUP_SIDE,
            "post_cleanup_selected_symbol_absent_or_zero": True,
            "existing_positions_unchanged_required":    True,
            "verification_plan_only":                   True,
            "real_readonly_after_execution_not_performed": True,
        }
        stages[STAGE_5_CLEANUP_READONLY_VERIFICATION_PLAN] = {
            "stage":   STAGE_5_CLEANUP_READONLY_VERIFICATION_PLAN,
            "summary": "Pre/post readonly verification plan (plan only; never executed).",
            "cleanup_readonly_verification_plan":   cleanup_readonly_verification_plan,
        }
        blocked.append(GATE_PRE_CLEANUP_READONLY_REQUIRED)
        blocked.append(GATE_POST_CLEANUP_READONLY_REQUIRED)
        blocked.append(GATE_PRE_CLEANUP_SELECTED_SYMBOL_PRESENT)
        blocked.append(GATE_PRE_CLEANUP_EXPECTED_SYMBOL)
        blocked.append(GATE_PRE_CLEANUP_EXPECTED_QTY)
        blocked.append(GATE_PRE_CLEANUP_EXPECTED_SIDE_LONG)
        blocked.append(GATE_POST_CLEANUP_SELECTED_SYMBOL_ABSENT)
        ...
        # stage_6_cleanup_failure_policy
        # ===============================================================
        cleanup_failure_policy: dict[str, Any] = {
            ...
            "cleanup_side_invalid":              "FAIL_CLOSED",
            "reduce_only_invalid":               "FAIL_CLOSED",
            "partial_cleanup":                   "FAIL_CLOSED",
        ...
        stages[STAGE_6_CLEANUP_FAILURE_POLICY] = {
            "stage":   STAGE_6_CLEANUP_FAILURE_POLICY,
            "summary": "Future guarded_cleanup_only failure / abort policy.",
            "cleanup_failure_policy":               cleanup_failure_policy,
        }
        ...
        blocked.append(GATE_CLEANUP_SIDE_INVALID_FAIL_CLOSED)
        blocked.append(GATE_REDUCE_ONLY_INVALID_FAIL_CLOSED)
        blocked.append(GATE_PARTIAL_CLEANUP_FAIL_CLOSED)
        ...
        audit_artifacts: dict[str, Any] = {
            "precondition_contract":                dict(cleanup_precondition_contract),
            "manual_confirmation_contract":         dict(manual_confirmation_dry_run_contract),
            "cleanup_request_envelope":             dict(cleanup_request_envelope),
            "readonly_verification_plan":           dict(cleanup_readonly_verification_plan),
            "failure_policy":                       dict(cleanup_failure_policy),
            "final_cleanup_adapter_verdict":        {},  # filled below
        ...
        stages[STAGE_7_AUDIT_ARTIFACT_GENERATION] = {
            ...
            "summary": "Emit cleanup-only dry-run audit artifacts (DRY_RUN_NOT_SENT).",
        ...
        # stage_8_final_cleanup_adapter_verdict
        # ===============================================================
        blocked.append(GATE_REAL_CLEANUP_EXECUTION_NOT_IMPL)
        ...
        elif allow_real_cleanup_execution:
            failed_stage = ""
            status_out = STATUS_REAL_CLEANUP_NOT_IMPL
            mode_out   = MODE_REAL_CLEANUP_EXECUTION_GUARD
        elif allow_cleanup_dry_run_approval:
            failed_stage = ""
            status_out = STATUS_ADAPTER_READY_EXEC_DISABLED
            mode_out   = MODE_CLEANUP_DRY_RUN_APPROVAL
        else:
            failed_stage = ""
            status_out = STATUS_ADAPTER_READY
            mode_out   = MODE_CLEANUP_ADAPTER_CHECKLIST

        final_cleanup_adapter_verdict: dict[str, Any] = {
            "cleanup_dry_run_approval_allowed":     allow_cleanup_dry_run_approval,
            "real_cleanup_execution_requested":     bool(allow_real_cleanup_execution),
            "real_execution_allowed":               False,
            "real_cleanup_implemented":             False,
            "guarded_cleanup_dry_run_adapter":      True,
            "cleanup_only":                         True,
        ...
        audit_artifacts["final_cleanup_adapter_verdict"] = dict(final_cleanup_adapter_verdict)

        stages[STAGE_8_FINAL_CLEANUP_ADAPTER_VERDICT] = {
            "stage":   STAGE_8_FINAL_CLEANUP_ADAPTER_VERDICT,
            "summary": "Final cleanup adapter verdict + permanent execution guard.",
            "final_cleanup_adapter_verdict":        final_cleanup_adapter_verdict,
        }

        return TinyGuardedCleanupDryRunAdapterResult(
