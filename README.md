...
| latest validation | `py_compile` src + preview + test → PASS；`pytest tests/demo_trading/test_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_design.py -q` → **299 PASS**（含 7 FIX1 tests）；AW regression → 292 PASS；AV regression → 259 PASS；AU regression → 235 PASS；AT regression → 199 PASS；AS regression → 180 PASS；AR regression → 175 PASS；AQ regression → 138 PASS；合計 **1777/1777 PASS** |
