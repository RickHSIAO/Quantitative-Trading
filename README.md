...
## Demo Trading Guarded Lifecycle Status（updated by TASK-014BI, 2026-06-18）

共同狀態板，供 Rick / ChatGPT / Claude / Codex / Opus 三方協作對齊。本區塊由
TASK-014BI 同步更新；不改任何 execution logic、不解除 G20、不開啟 real trading。
...
| 欄位 | 值 |
|---|---|
| latest_completed_task | TASK-014BI |
| latest_commit | pending local — `TASK-014BI: add demo-only tiny execution adapter payload dry-run (offline; consumes BH directly; emits JSON+MD report)`（local only；尚未推遠端） |
| current_phase | demo-only tiny execution adapter **payload dry-run (offline) completed** — **TASK-014BI**；新增 BI 三件套（src 含 `DryRunCase` / `DryRunOutcome` / `DryRunReport` 三個 frozen dataclass + 18-case canonical 表 + 4 live-endpoint denial 檢查 + `run_dry_run` + `write_report` JSON+Markdown 報告寫入；scripts 提供 CLI 含 `--write-report`，exit 0 iff 全部 outcomes 與預期一致；tests Stage 1 focused-core **44** tests，含 identity / case 表覆蓋（BTCUSDT、ETHUSDT、5 個 protected symbols 各一 parametrize）/ live-endpoint denial / 4 個報告檔生成 + JSON round-trip + Markdown 內容 / 6 個 tokenize+ast static-source safety invariants 證明 BI 不 import 任何 network library、不 import BybitExecutor、不讀 env、無 send / place_order / post_order / submit_order、不 import main 或 src.risk、且 BI 直接 import BH）。BI 直接消費 BH module 的 pure 函式 — 無 sender、無 endpoint call、無 socket、無 secret 讀取 |
| next_required_task | `TASK-014BJ_demo_only_tiny_execution_adapter_endpoint_guard_integration`（**implementation path 下一步；不是 review-chain 後綴**；offline endpoint-guard 整合，亦可改為 final demo-only pre-execution checklist 變體；由 Rick 在 NEXT_ACTION.md 顯式授權後才會啟動） |
...
| latest validation | `py_compile` BI src + preview + tests → PASS；BI Stage 1 focused-core suite **44 PASS**；BH Stage 1 regression **45 PASS**；`pytest tests/demo_trading/ --ignore=test_demo_emergency_close_sender.py` → **7732 PASS**（= BH baseline 7688 + BI stage1 44；emergency_close_sender 既有失敗排除）；BI preview smoke（`--write-report`）exit 0；summary `total=22 built=4 rejected=18 unexpected=0 all_match=True`；4 個報告檔寫入 `outputs/demo_trading/demo_only_tiny_execution_adapter_payload_dry_run/`（latest JSON+MD + 時間戳 JSON+MD）；每個 built payload 帶 `_demo_only_audit_response_status=DEMO_ONLY_TINY_BH_NOT_SENT`；每個 live-endpoint case 都被 `bh.assert_endpoint_is_demo_only` 以 `live denylist` 訊息拒絕 |
| protected positions（never touched） | ENAUSDT / TIAUSDT / AIXBTUSDT / POLYXUSDT / EDUUSDT |
| adapter identity (BH upstream) | `IDENTITY=DEMO-ONLY-TINY-EXECUTION-ADAPTER-IMPLEMENTATION-PATH-SCAFFOLD`、`ADAPTER_CONTRACT_VERSION=demo_only_tiny_execution_adapter_implementation_path_scaffold_v1`（BI 直接 import BH module 並呼叫其 pure 函式，沒有平行實作；BH module 仍不定義 `send` / `place_order` / `post_order` / `submit_order`，仍不 import `BybitExecutor` 或任何 network library） |
| BI identity | `IDENTITY=DEMO-ONLY-TINY-EXECUTION-ADAPTER-PAYLOAD-DRY-RUN`、`IMPLEMENTATION_PATH_PHASE=offline_payload_dry_run`、`IS_REVIEW_CHAIN_SUFFIX=False`、`UPSTREAM_TASK=TASK-014BH` |
| order link id prefix | `DEMO_ONLY_TINY_BH_`（offline payload label；無 sender；不會作為 real order client id 送出） |
| audit response_status | `DEMO_ONLY_TINY_BH_NOT_SENT`（無任何 outbound request — module 設計上無 sender 可用；BI 報告每個 built payload 均含此 marker） |
| BI report output dir | `outputs/demo_trading/demo_only_tiny_execution_adapter_payload_dry_run/`（含 `latest_*.json` / `latest_*.md` / 時間戳 `*_<UTC_TS>.{json,md}`；已加入 .gitignore，不入 repo） |
| implementation_path_phase | `offline_payload_dry_run`（BI；上游 BH 為 `scaffold`；下一步 `TASK-014BJ_demo_only_tiny_execution_adapter_endpoint_guard_integration`，**不是** review-chain 後綴；real execution 仍 disabled，需 Rick 在 NEXT_ACTION.md 另行授權 explicit demo-only tiny order execution 任務才能寫任何 sender code） |
