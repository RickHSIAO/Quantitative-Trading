...
## Demo Trading Guarded Lifecycle Status（updated by TASK-014BA-FIX1, 2026-06-16）

共同狀態板，供 Rick / ChatGPT / Claude / Codex / Opus 三方協作對齊。本區塊由
TASK-014BA-FIX1 同步更新；不改任何 execution logic、不解除 G20、不開啟 real trading。

> TASK-014BA-FIX1 修正 TASK-014BA bulk-rename 殘留：BA preview 在 VPS 因 CLI 缺少
> `--from-latest-entry-disabled-implementation-scaffold-manual-authorization-gate-readiness-review`
> 旗標而 FAIL_CLOSED。FIX1 把 AZ readiness_review 重新接成 BA 的 direct upstream
> （AY/AX/AW/AV/AU/AT/AS/AR/AQ 改走 chained-through-AZ proof），新增 1 個 contract
> version 常數、15 個 AZ-direct hard-fail gates、14 個 AZ→AY 巢狀 simulated_approval
> hard-fail gates、30 個對應 dataclass 欄位 + `to_dict()` 輸出、`run_readiness_review()`
> 新增 `entry_disabled_implementation_scaffold_manual_authorization_gate_readiness_review`
> kwarg、scripts 新增 default dir/loader/CLI 旗標/missing-artifact fail-closed/AZ 報告 row、
> 重新命名 `--allow-...-readiness-review` → `--allow-...-final-pre-execution-review`、
> intro/banner wording 改為「BA consumes AZ readiness review」。新增 25 個 BA-FIX1
> regression tests（CLI help / subprocess happy-path / missing-AZ artifact / 6 個代表性
> hard-fail gate / field exposure / report contents / fixture happy-path）。仍無 sender、
> 無 real execution adapter、無 endpoint call、無 secret 讀取、無 G20 lift、無 position 修改。

| 欄位 | 值 |
|---|---|
| latest_completed_task | TASK-014BA-FIX1 |
| latest_commit | local — `TASK-014BA-FIX1: wire AZ readiness review direct upstream preview`（local；尚未推遠端；上游 commit `4d18930 TASK-014BA` 已在 origin/main） |
...
| latest validation | `py_compile` BA src + preview + test → PASS；`pytest tests/demo_trading/test_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py -q` → **508 PASS**（baseline 483 + FIX1 新增 25）；AZ 481 regression PASS；BA + AZ + 上游 demo chain（AY readiness/dry-run 系列）合計 **2867 PASS**；CLI 新旗標 `--from-latest-entry-disabled-implementation-scaffold-manual-authorization-gate-readiness-review` 已暴露、`--allow-...-readiness-review` 已重新命名為 `--allow-...-final-pre-execution-review`（`--help` 確認） |
