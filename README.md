...
- **EMA50 斜率方向確認**：過濾下跌段急彈翻紅、上漲段假摔翻黑的假訊號，大幅改善 2022 震盪盤（+13,716 USD）的績效。
- **倉位風險上限放寬**：`MAX_RISK_PCT` 由 0.02 放寬至 0.05，`MAX_POSITION_PCT` 由 0.10 放寬至 0.20。新增 `DEFAULT_RISK_PCT=0.04` 取代原本硬編碼的 0.02 預設值，解除對大多數標的的綁束。
- **類別槓桿（Leverage by Class）**：新增 `LEVERAGE_BY_CLASS = {'Crypto': 2.0}`。加密貨幣永續合約為保證金交易，現金回測過度保守；可用現金、開倉檢查皆改用保證金口徑，放大 Crypto 的 Alpha（$611/trade vs US $57/trade）。Pnl 與回報率計算基於實際數量 × 價差，維持原語義。

**v1.6 回測績效**（120 檔資產，2020-03 至 2026-05，初始資金 $100k）：

| 指標 | v1.5 baseline | **v1.6 最終** |
|---|---:|---:|
| 年化報酬 | 9.01% | **19.24%** |
| 總報酬（6 年） | 56.14% | **148.49%** |
| Sharpe Ratio | 0.443 | **0.683** |
| 盈虧比 | 1.308 | **1.445** |
| 最大回撤 | -9.73% | **-15.74%** |
| 勝率 | 45.4% | **48.1%** |
