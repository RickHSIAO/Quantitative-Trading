...
# 2. 執行回測，輸出 Excel 報告
python main.py backtest --with-vp --moat-tf-only
...
| 參數 | 說明 |
|------|------|
| `--with-vp` | 啟用 Volume Profile 策略（預設關閉，速度較慢；目前最佳回測組合會啟用） |
...
- **預設關閉**：此策略計算耗時，需明確加上 `--with-vp` 才啟用；最新最佳回測組合會啟用 VP
...
- **美股 MACD 雙確認改為可選**：`ENABLE_US_MACD_FILTER` 控制是否啟用，目前最佳回測設定為 `False`
- **台股特化 hook**：預留處置股（`is_disposition`）與主力籌碼（`chip_buy_days`）欄位接口
- **Volume Profile 預設關閉**：因計算較慢，改為 `--with-vp` 手動啟用；目前最佳回測組合會啟用 VP
- **倉位 sizing 修正**：回測開倉以 `available_cash` 作為 `position_size()` 基準，避免用總資金重複放大風險
- **新 CLI 參數**：`--with-vp`、`--moat-tf-only`、`--rs-pct`
- **最佳已驗證組合**：`python main.py backtest --with-vp --moat-tf-only`，run_id=34，總報酬 59.35%、PF 1.397、最大回撤 -12.41%
