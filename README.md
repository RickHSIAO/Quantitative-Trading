...
## Demo Trading Guarded Lifecycle Status（updated by TASK-014BJ, 2026-06-18）

共同狀態板，供 Rick / ChatGPT / Claude / Codex / Opus 三方協作對齊。本區塊由
TASK-014BJ 同步更新；不改任何 execution logic、不解除 G20、不開啟 real trading。
...
| 欄位 | 值 |
|---|---|
| latest_completed_task | TASK-014BJ |
| latest_commit | pending local — `TASK-014BJ: add demo-only tiny execution adapter endpoint guard integration (single future-safe entry point; consumes BH directly; emits JSON+MD report)`（local only；尚未推遠端） |
| current_phase | demo-only tiny execution adapter **endpoint guard integration (offline) completed** — **TASK-014BJ**；新增 BJ 三件套（src 含單一 future-safe `integrate_demo_only_tiny_request` entry point 串接 BH 8 個 guard step + `IntegrationRequest` / `GuardDecision` / `IntegrationResult` / `IntegrationCase` / `IntegrationOutcome` / `IntegrationReport` 六個 frozen dataclass + 20-case canonical 表 + `run_integration_dry_run` + `write_report` JSON+Markdown 報告寫入；scripts CLI 含 `--write-report` / `--print-decisions`，exit 0 iff 全部 outcomes 與預期一致；tests Stage 1 focused-core **61** tests，含 identity / chain-break markers / `GUARD_STEPS` 完整 / 20-case 覆蓋（BTCUSDT、ETHUSDT、5 個 protected symbols 各一 parametrize）/ 14 個 reject-step 直呼驗證 / 4 個 live URL（root + order endpoint + mirror order endpoint + websocket）各自被擋在 `endpoint_target` step / 報告 4 個檔案 + JSON round-trip + Markdown 內容 / 8 個 tokenize+ast static-source safety invariants（BJ 不 import 任何 network library、不 import BybitExecutor、不讀 env、無 send / place_order / post_order / submit_order、不 import main 或 src.risk、BJ 直接 import BH、phase 與 `IS_REVIEW_CHAIN_SUFFIX = False` literal 存在、`final_pre_execution_checklist` literal 存在）+ runtime：BybitExecutor / main / src.risk 模組未被載入、BH/BI markers 仍 intact）。BJ 直接消費 BH module 的 pure guard 函式 — 無 sender、無 endpoint call、無 socket、無 secret 讀取 |
| next_required_task | `TASK-014BK_demo_only_tiny_execution_adapter_final_pre_execution_checklist`（**implementation path 下一步；不是 review-chain 後綴**；offline final demo-only pre-execution checklist，亦可改為 explicit demo-only tiny order preparation 變體；由 Rick 在 NEXT_ACTION.md 顯式授權後才會啟動） |
...
| latest validation | `py_compile` BJ src + preview + tests → PASS；BJ Stage 1 focused-core suite **61 PASS**；BH+BI regression **89 PASS**（45 + 44）；`pytest tests/demo_trading/ --ignore=test_demo_emergency_close_sender.py --basetemp=.pytest_basetemp` → **7793 PASS**（= BI baseline 7732 + BJ stage1 61；emergency_close_sender 既有失敗排除）；BJ preview smoke（`--write-report`）exit 0；summary `total=20 ok=4 rejected=16 unexpected=0 all_match=True`；4 個報告檔寫入 `outputs/demo_trading/demo_only_tiny_execution_adapter_endpoint_guard_integration/`（latest JSON+MD + 時間戳 JSON+MD）；每個 built payload 帶 `_demo_only_audit_response_status=DEMO_ONLY_TINY_BH_NOT_SENT` + `_demo_only_bj_audit_response_status=DEMO_ONLY_TINY_BJ_NOT_SENT`；每個 live URL（root / order endpoint / mirror order endpoint / websocket）都被 `endpoint_target` step 以 `live denylist` 訊息拒絕 |
| protected positions（never touched） | ENAUSDT / TIAUSDT / AIXBTUSDT / POLYXUSDT / EDUUSDT |
| adapter identity (BH upstream) | `IDENTITY=DEMO-ONLY-TINY-EXECUTION-ADAPTER-IMPLEMENTATION-PATH-SCAFFOLD`、`ADAPTER_CONTRACT_VERSION=demo_only_tiny_execution_adapter_implementation_path_scaffold_v1`（BJ 直接 import BH module 並呼叫其 pure guard 函式，沒有平行實作；BH module 仍不定義 `send` / `place_order` / `post_order` / `submit_order`，仍不 import `BybitExecutor` 或任何 network library） |
| BI identity (BJ upstream) | `IDENTITY=DEMO-ONLY-TINY-EXECUTION-ADAPTER-PAYLOAD-DRY-RUN`、`IMPLEMENTATION_PATH_PHASE=offline_payload_dry_run`、`IS_REVIEW_CHAIN_SUFFIX=False`、`UPSTREAM_TASK=TASK-014BH` |
| BJ identity | `IDENTITY=DEMO-ONLY-TINY-EXECUTION-ADAPTER-ENDPOINT-GUARD-INTEGRATION`、`IMPLEMENTATION_PATH_PHASE=endpoint_guard_integration`、`IS_REVIEW_CHAIN_SUFFIX=False`、`UPSTREAM_TASK=TASK-014BI`、`INTEGRATION_CONTRACT_VERSION=demo_only_tiny_execution_adapter_endpoint_guard_integration_v1`、`GUARD_STEPS=(environment, symbol, existing_positions, side, qty_cap, notional_cap, order_link_id_prefix, endpoint_target)` |
| order link id prefix | `DEMO_ONLY_TINY_BH_`（offline payload label；無 sender；不會作為 real order client id 送出） |
| audit response_status | `DEMO_ONLY_TINY_BH_NOT_SENT` + BJ 額外貼 `DEMO_ONLY_TINY_BJ_NOT_SENT`（無任何 outbound request — 兩層 module 均無 sender；BJ 報告每個 built payload 兩 marker 並存） |
| BJ report output dir | `outputs/demo_trading/demo_only_tiny_execution_adapter_endpoint_guard_integration/`（含 `latest_*.json` / `latest_*.md` / 時間戳 `*_<UTC_TS>.{json,md}`；已加入 .gitignore，不入 repo） |
| implementation_path_phase | `endpoint_guard_integration`（BJ；上游 BI 為 `offline_payload_dry_run`、BH 為 `scaffold`；下一步 `TASK-014BK_demo_only_tiny_execution_adapter_final_pre_execution_checklist`，**不是** review-chain 後綴；real execution 仍 disabled，需 Rick 在 NEXT_ACTION.md 另行授權 explicit demo-only tiny order preparation 任務才能寫任何 sender code） |
