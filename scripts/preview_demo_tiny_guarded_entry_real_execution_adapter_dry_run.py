"""
scripts/preview_demo_tiny_guarded_entry_real_execution_adapter_dry_run.py
TASK-014AO: Guarded Tiny Entry Real Execution Adapter Dry-run CLI.

Usage (ADAPTER DRY-RUN CHECKLIST --- default, no network,
       no implementation):
  python scripts/preview_demo_tiny_guarded_entry_real_execution_adapter_dry_run.py \\
...
Usage (ADAPTER DRY-RUN APPROVAL --- dry-run-only, no execution):
  ... --allow-adapter-dry-run
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_real_execution_adapter_dry_run/
      {ts}_tiny_guarded_entry_real_execution_adapter_dry_run.json
      {ts}_tiny_guarded_entry_real_execution_adapter_dry_run.md
      latest_tiny_guarded_entry_real_execution_adapter_dry_run.json
      latest_tiny_guarded_entry_real_execution_adapter_dry_run.md

IMPORTANT:
  - TASK-014AO produces a STRICT ADAPTER-DRY-RUN-ONLY artifact.
    ...
    approval-input validation, NO adapter implementation, NO adapter
    `send` method.  Adapter contract / inputs / outputs / request
    envelope / response simulation / boundaries / forbidden surfaces /
    fail-closed policy / audit schema are documented only --- this
    task does NOT implement the adapter.  No automatic git commit,
    no automatic git push.
  - Even with --allow-adapter-dry-run the dry-run review only emits a
    sanitized dry-run artifact.  Readiness conclusion is fixed at
    DESIGN_REVIEW_READY_NOT_EXECUTABLE.
  - Even with --allow-real-entry-execution the dry-run review returns
...
Exit codes:
  0  tiny_guarded_entry_real_execution_adapter_dry_run_ready /
...
_DEFAULT_OUTPUT_DIR          = (
    ROOT / "outputs" / "demo_trading" / "tiny_guarded_entry_real_execution_adapter_dry_run"
)
...
def _print_result(r: TinyGuardedEntryRealExecutionAdapterDryRunResult) -> None:
    ...
    print(f"  adapter_dry_run_allowed                    : {r.adapter_dry_run_allowed}")
    ...
    print(f"  guarded_entry_real_execution_adapter_dry_run: {r.guarded_entry_real_execution_adapter_dry_run}")
    print(f"  adapter_dry_run_only                       : {r.adapter_dry_run_only}")
    ...
    print(f"  upstream_entry_adapter_design_readiness            : {r.upstream_entry_adapter_design_readiness_conclusion}")
    print(f"  upstream_entry_adapter_design_grants_exec          : {r.upstream_entry_adapter_design_grants_execution}")
    print(f"  upstream_entry_adapter_design_impl_included        : {r.upstream_entry_adapter_design_implementation_included}")
    print(f"  upstream_entry_adapter_design_exec_included        : {r.upstream_entry_adapter_design_execution_included}")
...
def _write_report(
    r: TinyGuardedEntryRealExecutionAdapterDryRunResult,
    output_dir: Path,
) -> None:
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Real Execution Adapter Dry-run (TASK-014AO)",
        "",
        ...
        f"| adapter_dry_run_allowed | {r.adapter_dry_run_allowed} |",
        ...
        f"| guarded_entry_real_execution_adapter_dry_run | {r.guarded_entry_real_execution_adapter_dry_run} |",
        f"| adapter_dry_run_only | {r.adapter_dry_run_only} |",
        ...
        f"| upstream_entry_adapter_design_readiness_conclusion | {r.upstream_entry_adapter_design_readiness_conclusion} |",
        f"| upstream_entry_adapter_design_grants_execution | {r.upstream_entry_adapter_design_grants_execution} |",
        f"| upstream_entry_adapter_design_implementation_included | {r.upstream_entry_adapter_design_implementation_included} |",
        f"| upstream_entry_adapter_design_execution_included | {r.upstream_entry_adapter_design_execution_included} |",
        f"| next_required_task | {r.next_required_task} |",
        "",
        "## Adapter Dry-run Scope",
        "",
        "```json",
        json.dumps(r.adapter_dry_run_scope, indent=2),
        "```",
        "",
        "## Adapter Dry-run Contract",
        "",
        "```json",
        json.dumps(r.adapter_dry_run_contract, indent=2),
        "```",
        "",
        "## Dry-run Input Validation Simulation",
        "",
        "```json",
        json.dumps(r.dry_run_input_validation_simulation, indent=2),
        "```",
        "",
        "## Dry-run Request Envelope",
        "",
        "```json",
        json.dumps(r.dry_run_request_envelope, indent=2),
        "```",
        "",
        "## Entry Payload Dry-run Preview",
        "",
        "```json",
        json.dumps(r.entry_payload_dry_run_preview, indent=2),
        "```",
        "",
        "## Dry-run Response Simulation",
        "",
        "```json",
        json.dumps(r.dry_run_response_simulation, indent=2),
        "```",
        "",
        "## Secret and Signature Dry-run Boundary",
        "",
        "```json",
        json.dumps(r.secret_and_signature_dry_run_boundary, indent=2),
        "```",
        "",
        "## Stop / Cleanup Dry-run Boundary",
        "",
        "```json",
        json.dumps(r.stop_cleanup_dry_run_boundary, indent=2),
        "```",
        "",
        "## Forbidden Execution Surface Dry-run",
        "",
        "```json",
        json.dumps(r.forbidden_execution_surface_dry_run, indent=2),
        "```",
        "",
        "## Failure and Abort Adapter Dry-run",
        "",
        "```json",
        json.dumps(r.failure_and_abort_adapter_dry_run, indent=2),
        "```",
        "",
        ...
        "```",
        "",
        "## Final Adapter Dry-run Verdict",
        "",
        "```json",
        json.dumps(r.final_adapter_dry_run_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AO is a STRICT ADAPTER-DRY-RUN-ONLY module.",
        ...
        "> Even with --allow-adapter-dry-run the dry-run review",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("ADAPTER DRY-RUN APPROVAL --- NO NETWORK --- dry-run-only")
    else:
        print("ADAPTER DRY-RUN CHECKLIST --- NO NETWORK --- dry-run-only")
    print("TASK-014AO: Guarded Tiny Entry Real Execution Adapter Dry-run")
    ...
    gate = DemoTinyGuardedEntryRealExecutionAdapterDryRun()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry REAL EXECUTION ADAPTER DRY-RUN. "
            ...
            "/ request envelope / response simulation / boundaries / "
            "forbidden surfaces / fail-closed policy / audit schema are "
            "documented only.  Even with --allow-adapter-dry-run this "
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to adapter-dry-run against.  "
    ...
    parser.add_argument("--expected-commit-hash", default="",
                        ...
                        help=("Optional expected commit hash to record in the "
                              ...
                              "dry-run review NEVER performs an automatic git "
                              "commit or push)."))
    parser.add_argument("--allow-adapter-dry-run", action="store_true",
                        help=("Promote envelope to adapter_dry_run_approval. "
                              "Even with this flag, TASK-014AO only emits an "
                              "adapter-dry-run artifact "
    ...
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AO "
    ...
    sys.exit(run_execute(
        ...
        allow_adapter_dry_run=args.allow_adapter_dry_run,
