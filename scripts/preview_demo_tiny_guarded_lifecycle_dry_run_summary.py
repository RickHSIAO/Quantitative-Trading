...
TASK-014AH: Guarded Tiny Lifecycle Dry-run Summary CLI.

Usage (SUMMARY CHECKLIST --- default, no network, no implementation):
  python scripts/preview_demo_tiny_guarded_lifecycle_dry_run_summary.py \\
...
Usage (SUMMARY DRY-RUN APPROVAL --- dry-run only, no execution):
...
Usage (REAL LIFECYCLE EXECUTION GUARD --- always returns
       ...
       no real lifecycle runner implemented):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_lifecycle_dry_run_summary/
      {ts}_tiny_guarded_lifecycle_dry_run_summary.json
      {ts}_tiny_guarded_lifecycle_dry_run_summary.md
      latest_tiny_guarded_lifecycle_dry_run_summary.json
      latest_tiny_guarded_lifecycle_dry_run_summary.md

IMPORTANT:
  - TASK-014AH produces a STRICT SUMMARY-ONLY REVIEW artifact.  No
    guarded runner, no entry execution, no stop-attach execution, no
    cleanup execution, no endpoint invoked.  No network, no socket,
    no environment-variable reads, no signing, no order send, no stop
    send.
  - Even with --allow-summary-approval the summary only emits a
    sanitized dry-run report.  Readiness conclusion is fixed at
    DESIGN_REVIEW_READY_NOT_EXECUTABLE.
  - Even with --allow-real-lifecycle-execution the summary returns
    REAL_LIFECYCLE_EXECUTION_NOT_IMPLEMENTED (no socket opened).
  ...
  - No close-only sender / no emergency-close sender / no new-entry
    ...
    real lifecycle runner real execution path is invoked.

Exit codes:
  0  tiny_guarded_lifecycle_dry_run_summary_ready /
...
def _print_result(r: TinyGuardedLifecycleDryRunSummaryResult) -> None:
    ...
    print(f"  summary_dry_run_approval_allowed           : {r.summary_dry_run_approval_allowed}")
    ...
    print(f"  guarded_lifecycle_dry_run_summary          : {r.guarded_lifecycle_dry_run_summary}")
    print(f"  summary_only                               : {r.summary_only}")
...
def _write_report(
    r: TinyGuardedLifecycleDryRunSummaryResult,
    output_dir: Path,
) -> None:
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Lifecycle Dry-run Summary (TASK-014AH)",
        "",
        ...
        f"| guarded_lifecycle_dry_run_summary | {r.guarded_lifecycle_dry_run_summary} |",
        f"| summary_only | {r.summary_only} |",
        ...
        "",
        "## Token Patterns (documented, never validated by this task)",
        "",
        f"- entry_token_pattern: `{r.entry_token_pattern}`",
        f"- stop_token_pattern: `{r.stop_token_pattern}`",
        f"- cleanup_token_pattern: `{r.cleanup_token_pattern}`",
        "",
        "## Summary Scope",
        "",
        "```json",
        json.dumps(r.summary_scope, indent=2),
        "```",
        "",
        "## Cross-adapter Consistency Matrix",
        "",
        "```json",
        json.dumps(r.cross_adapter_consistency_matrix, indent=2),
        "```",
        "",
        "## Manual Confirmation Matrix",
        "",
        "```json",
        json.dumps(r.manual_confirmation_matrix, indent=2),
        "```",
        "",
        "## Execution Forbidden Matrix",
        "",
        "```json",
        json.dumps(r.execution_forbidden_matrix, indent=2),
        "```",
        "",
        "## Sequential Dry-run Lifecycle Plan",
        "",
        "```json",
        json.dumps(r.sequential_dry_run_lifecycle_plan, indent=2),
        "```",
        "",
        "## Failure and Abort Summary",
        "",
        "```json",
        json.dumps(r.failure_and_abort_summary, indent=2),
        "```",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_lifecycle_summary_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AH is a STRICT SUMMARY-ONLY REVIEW module.  It NEVER",
        "> opens a socket, NEVER invokes /v5/order/create, NEVER invokes",
        ...
        "> NEVER invokes the real stop-attach sender's real execution path,",
        "> NEVER invokes a real cleanup sender's real execution path, NEVER",
        "> implements a real guarded lifecycle runner, NEVER reads",
        "> environment variables / secrets, NEVER signs any request, NEVER",
        "> includes entry / stop-attach / cleanup execution, NEVER lifts",
        "> TASK-014L sender G20 (protected_entry_policy_missing).",
        "> The 5 existing demo positions are NEVER touched.",
        "> Even with --allow-summary-approval the summary only emits a",
        "> dry-run artifact (readiness_conclusion remains",
        "> DESIGN_REVIEW_READY_NOT_EXECUTABLE).",
        "> Even with --allow-real-lifecycle-execution this task returns",
        "> REAL_LIFECYCLE_EXECUTION_NOT_IMPLEMENTED (no socket opened).",
        "",
    ]
    md_text = "\n".join(md_lines)
...
def run_execute(
    ...
    if allow_real_lifecycle_execution:
        ...
        print("SUMMARY DRY-RUN APPROVAL --- NO NETWORK --- dry-run only")
    else:
        print("SUMMARY CHECKLIST --- NO NETWORK --- dry-run only")
    print("TASK-014AH: Guarded Tiny Lifecycle Dry-run Summary")
    print(_SEP)
    ...
    summary = DemoTinyGuardedLifecycleDryRunSummary()
    result  = summary.run_checklist(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded lifecycle DRY-RUN SUMMARY. "
            "No network, no live endpoint, no orders / positions modified, "
            "no guarded runner implemented, no real lifecycle execution, "
            ...
            "--allow-summary-approval this only emits a dry-run summary "
            "artifact; even with --allow-real-lifecycle-execution it "
            "returns REAL_LIFECYCLE_EXECUTION_NOT_IMPLEMENTED."
        ),
    )
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to dry-run-summarize against.  MUST equal "
                              "SOLUSDT (post-entry symbol).  MUST NOT be in "
                              "the 5 existing demo position symbols "
                              "(ENAUSDT / TIAUSDT / AIXBTUSDT / POLYXUSDT / "
                              "EDUUSDT)."))
    parser.add_argument("--allow-summary-approval", action="store_true",
                        help=("Promote envelope to summary_dry_run_approval. "
                              "Even with this flag, TASK-014AH only emits a "
                              "dry-run summary report (readiness_conclusion "
    ...
    parser.add_argument("--allow-real-lifecycle-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny lifecycle runner execution.  TASK-014AH "
                              "returns REAL_LIFECYCLE_EXECUTION_NOT_IMPLEMENTED "
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_summary_approval=args.allow_summary_approval,
