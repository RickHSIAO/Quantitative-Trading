"""
scripts/preview_demo_tiny_guarded_entry_final_pre_execution_review.py
TASK-014AL: Guarded Tiny Entry Final Pre-execution Review CLI.

Usage (FINAL PRE-EXECUTION REVIEW CHECKLIST --- default, no network,
       no implementation):
  python scripts/preview_demo_tiny_guarded_entry_final_pre_execution_review.py \\
...
Usage (FINAL PRE-EXECUTION REVIEW APPROVAL --- review-only, no execution):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_final_pre_execution_review/
      {ts}_tiny_guarded_entry_final_pre_execution_review.json
      {ts}_tiny_guarded_entry_final_pre_execution_review.md
      latest_tiny_guarded_entry_final_pre_execution_review.json
      latest_tiny_guarded_entry_final_pre_execution_review.md

IMPORTANT:
  - TASK-014AL produces a STRICT FINAL-PRE-EXECUTION-REVIEW-ONLY
    artifact.  No guarded entry sender, no order send, no stop-attach
    send, no cleanup send, no endpoint invoked.  No network, no
    socket, no environment-variable reads, no signing, no order send,
    NO real token validation, NO token-to-authorization mapping.
    Token-pattern matching is simulated only.  No automatic git
    commit, no automatic git push.
  - Even with --allow-review-approval the review only emits a
    sanitized review artifact.  Readiness conclusion is fixed at
...
Exit codes:
  0  tiny_guarded_entry_final_pre_execution_review_ready /
...
def _print_result(r: TinyGuardedEntryFinalPreExecutionReviewResult) -> None:
    ...
    print(f"  review_approval_allowed                    : {r.review_approval_allowed}")
    ...
    print(f"  guarded_entry_final_pre_execution_review   : {r.guarded_entry_final_pre_execution_review}")
    print(f"  final_pre_execution_review_only            : {r.final_pre_execution_review_only}")
    ...
    print(f"  upstream_entry_manual_auth_dry_run_readiness: {r.upstream_entry_manual_auth_dry_run_readiness_conclusion}")
    print(f"  upstream_entry_manual_auth_dry_run_authorization: {r.upstream_entry_manual_auth_dry_run_authorization_result}")
...
def _write_report(
    r: TinyGuardedEntryFinalPreExecutionReviewResult,
    output_dir: Path,
) -> None:
    ...
    base = "tiny_guarded_entry_final_pre_execution_review"
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Final Pre-execution Review (TASK-014AL)",
        "",
        ...
        f"| guarded_entry_final_pre_execution_review | {r.guarded_entry_final_pre_execution_review} |",
        f"| final_pre_execution_review_only | {r.final_pre_execution_review_only} |",
        ...
        f"| upstream_entry_manual_auth_dry_run_readiness_conclusion | {r.upstream_entry_manual_auth_dry_run_readiness_conclusion} |",
        f"| upstream_entry_manual_auth_dry_run_authorization_result | {r.upstream_entry_manual_auth_dry_run_authorization_result} |",
        f"| next_required_task | {r.next_required_task} |",
        "",
        "## Token Pattern (documented + simulated, never validated as real authorization)",
        "",
        f"- entry_token_pattern: `{r.entry_token_pattern}`",
        f"- sample_token:        `{r.sample_token}`",
        "",
        "## Final Pre-execution Review Scope",
        "",
        "```json",
        json.dumps(r.final_pre_execution_review_scope, indent=2),
        "```",
        "",
        "## Repo and Docs Final Review",
        "",
        "```json",
        json.dumps(r.repo_and_docs_final_review, indent=2),
        "```",
        "",
        "## Runtime Readonly Final Review",
        "",
        "```json",
        json.dumps(r.runtime_readonly_final_review, indent=2),
        "```",
        "",
        "## Entry Order Parameters Final Review",
        "",
        "```json",
        json.dumps(r.entry_order_parameters_final_review, indent=2),
        "```",
        "",
        "## Manual Authorization Final Review",
        "",
        "```json",
        json.dumps(r.manual_authorization_final_review, indent=2),
        "```",
        "",
        "## Stop / Cleanup Boundary Final Review",
        "",
        "```json",
        json.dumps(r.stop_cleanup_boundary_final_review, indent=2),
        "```",
        "",
        "## Forbidden Automation Final Review",
        "",
        "```json",
        json.dumps(r.forbidden_automation_final_review, indent=2),
        "```",
        "",
        "## Failure and Abort Final Review",
        "",
        "```json",
        json.dumps(r.failure_and_abort_final_review, indent=2),
        "```",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_pre_execution_review_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AL is a STRICT FINAL-PRE-EXECUTION-REVIEW-ONLY",
        "> module.  It NEVER opens a socket, NEVER invokes",
        "> /v5/order/create, NEVER invokes /v5/position/trading-stop,",
        "> NEVER modifies any position, NEVER invokes the close-only",
        "> sender, NEVER invokes the emergency-close sender, NEVER",
        "> invokes the new-entry sender's real execution path, NEVER",
        "> invokes the real stop-attach sender's real execution path,",
        "> NEVER invokes a real cleanup sender's real execution path,",
        "> NEVER invokes a real guarded entry sender's real execution",
        "> path, NEVER implements a real guarded entry runner, NEVER",
        "> reads environment variables / secrets, NEVER signs any",
        "> request, NEVER validates the token as real authorization",
        "> (only simulates pattern-match), NEVER treats any token as",
        ...
        "> Even with --allow-review-approval the review only emits a",
        "> sanitized review artifact (readiness_conclusion remains",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("FINAL PRE-EXECUTION REVIEW APPROVAL --- NO NETWORK --- review-only")
    else:
        print("FINAL PRE-EXECUTION REVIEW CHECKLIST --- NO NETWORK --- review-only")
    print("TASK-014AL: Guarded Tiny Entry Final Pre-execution Review")
    print(_SEP)
    ...
    review = DemoTinyGuardedEntryFinalPreExecutionReview()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry FINAL PRE-EXECUTION REVIEW. "
            ...
            "no token-to-authorization mapping, no automatic git commit, "
            "no automatic git push.  Token-pattern matching is simulated "
            "only.  Even with --allow-review-approval this only emits a "
            "review artifact; even with --allow-real-entry-execution it "
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to final-pre-execution-review against.  "
    ...
    parser.add_argument("--expected-commit-hash", default="",
                        ...
                        help=("Optional expected commit hash to record in the "
                              ...
                              "review NEVER performs an automatic git commit "
                              "or push)."))
    parser.add_argument("--allow-review-approval", action="store_true",
                        help=("Promote envelope to final_pre_execution_review_approval. "
                              "Even with this flag, TASK-014AL only emits a "
                              "final-pre-execution-review artifact "
                              "(readiness_conclusion stays "
                              "DESIGN_REVIEW_READY_NOT_EXECUTABLE).  "
                              "No execution, no implementation, no real "
                              "token validation, no token-to-authorization "
                              "mapping, no automatic git commit, no "
                              "automatic git push."))
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AL "
                              "returns REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED "
    ...
    sys.exit(run_execute(
        ...
        allow_review_approval=args.allow_review_approval,
