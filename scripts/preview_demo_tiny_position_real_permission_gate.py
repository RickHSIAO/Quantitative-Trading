...
TASK-014W: Demo Tiny Isolated Position Real Execution Permission Gate /
           Manual Approval Checklist CLI.

Usage (CHECKLIST --- default, no network, envelope-only):
  python scripts/preview_demo_tiny_position_real_permission_gate.py \\
...
Usage (PERMISSION GATE DRY RUN --- envelope-only, status promotes to
       REAL_PERMISSION_GATE_READY_BUT_EXECUTION_DISABLED if hard-fail
       gates absent):
  python scripts/preview_demo_tiny_position_real_permission_gate.py \\
    ...
    --symbol SOLUSDT --allow-real-permission-gate \\
    [--write-report]

Usage (REAL TINY POSITION GUARD --- always returns
       REAL_TINY_POSITION_NOT_IMPLEMENTED, no socket opened):
  python scripts/preview_demo_tiny_position_real_permission_gate.py \\
    --from-latest-readonly --from-latest-reconciliation \\
    --from-latest-protection --from-latest-contract \\
    --from-latest-noop-plan --from-latest-lifecycle \\
    --symbol SOLUSDT --allow-real-tiny-position \\
    [--write-report]
...
IMPORTANT:
  - This is a PERMISSION GATE module.  No network at all.  Even with
    --allow-real-permission-gate, the gate only emits a checklist
    report and never executes anything.  Even with
    --allow-real-tiny-position, the gate returns
    REAL_TINY_POSITION_NOT_IMPLEMENTED.  Real execution is reserved
...
Exit codes:
  0  checklist_ready / gate_ready_exec_disabled / real_tiny_not_implemented
...
_DEFAULT_OUTPUT_DIR      = ROOT / "outputs" / "demo_trading" / "tiny_position_real_permission_gate"
...
def _print_result(r: TinyPositionRealPermissionGateResult) -> None:
    print(f"  mode                              : {r.mode}")
    ...
    print(f"  approval_token_patterns           : {r.approval_token_patterns}")
    print(f"  three_step_approval_required      : {r.three_step_approval_required}")
    print(f"  real_permission_gate_dry_run_allowed: {r.real_permission_gate_dry_run_allowed}")
...
def _write_report(
    r: TinyPositionRealPermissionGateResult,
    output_dir: Path,
) -> None:
    ...
    json_path   = output_dir / f"{ts_safe}_tiny_position_real_permission_gate.json"
    json_latest = output_dir / "latest_tiny_position_real_permission_gate.json"
    md_path     = output_dir / f"{ts_safe}_tiny_position_real_permission_gate.md"
    md_latest   = output_dir / "latest_tiny_position_real_permission_gate.md"
    ...
    md_lines: list[str] = [
        "# Demo Tiny Isolated Position Real Execution Permission Gate (TASK-014W)",
        "",
        ...
        f"| approval_token_patterns | {', '.join(r.approval_token_patterns)} |",
        f"| three_step_approval_required | {r.three_step_approval_required} |",
        f"| real_permission_gate_dry_run_allowed | {r.real_permission_gate_dry_run_allowed} |",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014W is a PERMISSION GATE.  It NEVER opens a socket,",
        ...
        "> Real tiny-position execution is reserved for TASK-014X /",
        "> TASK-014Y / TASK-014Z; even with --allow-real-tiny-position",
        "> this task returns REAL_TINY_POSITION_NOT_IMPLEMENTED.",
        "",
    ]
    md_text = "\n".join(md_lines)
...
def run_execute(
    ...
    if allow_real_tiny_position:
        ...
        print("REAL PERMISSION GATE DRY RUN --- NO NETWORK --- envelope-only")
    else:
        print("CHECKLIST --- NO NETWORK --- envelope-only")
    print("TASK-014W: Demo Tiny Isolated Position Real Execution Permission Gate")
    print(_SEP)
    ...
    gate    = DemoTinyPositionRealPermissionGate()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Demo tiny isolated position REAL EXECUTION PERMISSION GATE "
            "/ manual approval checklist.  No network, no live "
            "endpoint, no orders / positions modified, no real "
            "trading-stop call.  Even with --allow-real-permission-gate "
            "this only emits a checklist; even with "
            "--allow-real-tiny-position it returns "
    ...
    parser.add_argument("--allow-real-permission-gate", action="store_true",
                        help=("Promote envelope to permission_gate_dry_run.  "
                              "Even with this flag, TASK-014W only emits a "
    ...
    parser.add_argument("--allow-real-tiny-position", action="store_true",
                        help=("Guarded flag for the future real tiny "
                              "position lifecycle.  TASK-014W returns "
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_real_permission_gate=args.allow_real_permission_gate,
