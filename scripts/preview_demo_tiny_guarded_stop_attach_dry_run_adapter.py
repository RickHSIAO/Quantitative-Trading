"""
scripts/preview_demo_tiny_guarded_stop_attach_dry_run_adapter.py
TASK-014AF: Guarded Stop-attach-only Dry-run Adapter CLI.

Usage (STOP ADAPTER CHECKLIST --- default, no network, no implementation):
  python scripts/preview_demo_tiny_guarded_stop_attach_dry_run_adapter.py \\
...
Usage (STOP DRY-RUN APPROVAL --- dry-run only, no execution):
  ... --allow-stop-dry-run-approval

Usage (REAL STOP EXECUTION GUARD --- always returns
       REAL_STOP_ATTACH_EXECUTION_NOT_IMPLEMENTED, no socket opened,
       no real stop attach implemented):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_stop_attach_dry_run_adapter/
      {ts}_tiny_guarded_stop_attach_dry_run_adapter.json
      {ts}_tiny_guarded_stop_attach_dry_run_adapter.md
      latest_tiny_guarded_stop_attach_dry_run_adapter.json
      latest_tiny_guarded_stop_attach_dry_run_adapter.md

IMPORTANT:
  - TASK-014AF produces a STOP-ATTACH-ONLY DRY-RUN ADAPTER artifact only.
    No guarded runner, no real stop attach, no entry, no cleanup is
  ...
  - Even with --allow-stop-dry-run-approval the adapter only emits a
    sanitized dry-run report.  Readiness conclusion is fixed at
    DESIGN_REVIEW_READY_NOT_EXECUTABLE.
  - Even with --allow-real-stop-execution the adapter returns
    REAL_STOP_ATTACH_EXECUTION_NOT_IMPLEMENTED (no socket opened).
  ...
  - No close-only sender / no emergency-close sender / no new-entry
    ...
    execution path is invoked.

Exit codes:
  0  tiny_guarded_stop_attach_dry_run_adapter_ready /
     tiny_guarded_stop_attach_dry_run_adapter_ready_exec_disabled /
...
def _print_result(r: TinyGuardedStopAttachDryRunAdapterResult) -> None:
    ...
    print(f"  stop_token_pattern                         : {r.stop_token_pattern}")
    print(f"  required_confirmation_flags ({len(r.required_confirmation_flags)}):")
    for f in r.required_confirmation_flags:
        print(f"    - {f}")
    print(f"  stop_dry_run_approval_allowed              : {r.stop_dry_run_approval_allowed}")
    print(f"  real_stop_execution_requested              : {r.real_stop_execution_requested}")
    print(f"  real_execution_allowed                     : {r.real_execution_allowed}")
    print(f"  real_stop_attach_implemented               : {r.real_stop_attach_implemented}")
    print(f"  guarded_stop_attach_dry_run_adapter        : {r.guarded_stop_attach_dry_run_adapter}")
    print(f"  stop_attach_only                           : {r.stop_attach_only}")
    print(f"  entry_included                             : {r.entry_included}")
    print(f"  cleanup_included                           : {r.cleanup_included}")
...
def _write_report(
    r: TinyGuardedStopAttachDryRunAdapterResult,
    output_dir: Path,
) -> None:
    ...
    base = "tiny_guarded_stop_attach_dry_run_adapter"
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Stop-attach-only Dry-run Adapter (TASK-014AF)",
        "",
        ...
        "",
        "## Final Stop Adapter Verdict",
        "",
        "| field | value |",
        "|---|---|",
        f"| selected_symbol | {r.selected_symbol or '(none)'} |",
        f"| existing_position_symbols | {', '.join(r.existing_position_symbols) or '(none)'} |",
        f"| stop_dry_run_approval_allowed | {r.stop_dry_run_approval_allowed} |",
        f"| real_stop_execution_requested | {r.real_stop_execution_requested} |",
        f"| real_execution_allowed | {r.real_execution_allowed} |",
        f"| real_stop_attach_implemented | {r.real_stop_attach_implemented} |",
        f"| guarded_stop_attach_dry_run_adapter | {r.guarded_stop_attach_dry_run_adapter} |",
        f"| stop_attach_only | {r.stop_attach_only} |",
        f"| entry_included | {r.entry_included} |",
        f"| cleanup_included | {r.cleanup_included} |",
    ...
    md_lines += [
        ...
        "",
        "```json",
        json.dumps(r.stop_adapter_scope, indent=2),
        "```",
        "",
        "## Stop Precondition Contract",
        "",
        "```json",
        json.dumps(r.stop_precondition_contract, indent=2),
        "```",
        "",
        ...
        "```",
        "",
        "## Stop Request Envelope (preview only, no sender adapter)",
        "",
        "```json",
        json.dumps(r.stop_request_envelope, indent=2),
        "```",
        "",
        "## Stop Readonly Verification Plan",
        "",
        "```json",
        json.dumps(r.stop_readonly_verification_plan, indent=2),
        "```",
        "",
        "## Stop Failure Policy",
        "",
        "```json",
        json.dumps(r.stop_failure_policy, indent=2),
        "```",
        "",
        "## Audit Artifacts",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_stop_adapter_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AF is a STOP-ATTACH-ONLY DRY-RUN ADAPTER module.  It",
        "> NEVER opens a socket, NEVER invokes /v5/position/trading-stop,",
        "> NEVER invokes /v5/order/create, NEVER modifies any position,",
        "> NEVER invokes the close-only sender, NEVER invokes the",
        "> emergency-close sender, NEVER invokes the new-entry sender's",
        "> real execution path, NEVER implements a real stop-attach sender,",
        "> NEVER reads environment variables / secrets, NEVER signs any",
        "> request, NEVER implements a guarded runner, NEVER includes entry",
        "> or cleanup, NEVER lifts TASK-014L sender G20",
        "> (protected_entry_policy_missing).",
        "> The 5 existing demo positions are NEVER touched.",
        "> Even with --allow-stop-dry-run-approval the adapter only emits",
        "> a dry-run artifact (readiness_conclusion remains",
        "> DESIGN_REVIEW_READY_NOT_EXECUTABLE).",
        "> Even with --allow-real-stop-execution this task returns",
        "> REAL_STOP_ATTACH_EXECUTION_NOT_IMPLEMENTED (no socket opened).",
        "",
    ]
    md_text = "\n".join(md_lines)
...
def run_execute(
    ...
    if allow_real_stop_execution:
        print("REAL STOP EXECUTION GUARD --- NO NETWORK --- REAL_STOP_ATTACH_EXECUTION_NOT_IMPLEMENTED")
    elif allow_stop_dry_run_approval:
        print("STOP DRY-RUN APPROVAL --- NO NETWORK --- dry-run only")
    else:
        print("STOP ADAPTER CHECKLIST --- NO NETWORK --- dry-run only")
    print("TASK-014AF: Guarded Stop-attach-only Dry-run Adapter")
    print(_SEP)
    ...
    adapter = DemoTinyGuardedStopAttachDryRunAdapter()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded stop-attach-only DRY-RUN ADAPTER. "
            "No network, no live endpoint, no orders / positions modified, "
            "no guarded runner implemented, no real stop attach implemented, "
            "no entry included, no cleanup included, no full lifecycle "
            ...
            "execution.  Even with --allow-stop-dry-run-approval this only "
            "emits a dry-run adapter artifact; even with "
            "--allow-real-stop-execution it returns "
            "REAL_STOP_ATTACH_EXECUTION_NOT_IMPLEMENTED."
        ),
    )
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        ...
                        help=("Symbol to dry-run-adapt against.  MUST equal "
                              ...
                              "EDUUSDT)."))
    parser.add_argument("--allow-stop-dry-run-approval", action="store_true",
                        help=("Promote envelope to stop_dry_run_approval. "
                              "Even with this flag, TASK-014AF only emits a "
                              "dry-run adapter report (readiness_conclusion "
                              "stays DESIGN_REVIEW_READY_NOT_EXECUTABLE).  "
                              "No execution, no implementation."))
    parser.add_argument("--allow-real-stop-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny stop-attach execution.  TASK-014AF "
                              "returns REAL_STOP_ATTACH_EXECUTION_NOT_IMPLEMENTED "
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_stop_dry_run_approval=args.allow_stop_dry_run_approval,
        allow_real_stop_execution=args.allow_real_stop_execution,
