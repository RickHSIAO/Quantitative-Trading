...
TASK-014Z: Tiny Isolated Demo Cleanup Permission Gate / Dry-run Only CLI.

Usage (CHECKLIST --- default, no network, envelope-only):
  python scripts/preview_demo_tiny_cleanup_permission_gate.py \\
...
Usage (REAL CLEANUP PERMISSION DRY RUN --- envelope-only):
  ... --allow-real-cleanup-permission

Usage (REAL TINY CLEANUP GUARD --- always returns
...
Writes (when --write-report):
  outputs/demo_trading/tiny_cleanup_permission_gate/
      {ts}_tiny_cleanup_permission_gate.json
      {ts}_tiny_cleanup_permission_gate.md
      latest_tiny_cleanup_permission_gate.json
      latest_tiny_cleanup_permission_gate.md

IMPORTANT:
  - This is a PERMISSION GATE module.  No network at all.  Even with
    --allow-real-cleanup-permission, the gate only emits a checklist
    report and never executes anything.  Even with
    --allow-real-cleanup, the gate returns
    REAL_CLEANUP_NOT_IMPLEMENTED.  Real cleanup (close-only) execution
    is reserved for a future task.
...
_DEFAULT_OUTPUT_DIR          = ROOT / "outputs" / "demo_trading" / "tiny_cleanup_permission_gate"
...
def _print_result(r: TinyCleanupPermissionGateResult) -> None:
    print(f"  mode                                       : {r.mode}")
    ...
    print(f"  entry_rounded_tiny_qty                     : {r.entry_rounded_tiny_qty}")
    print(f"  lifecycle_tiny_qty                         : {r.lifecycle_tiny_qty}")
    ...
    print(f"  post_cleanup_verification_plan             : {r.post_cleanup_verification_plan}")
    print(f"  failure_response_plan                      : {r.failure_response_plan}")
    print(f"  real_cleanup_permission_dry_run_allowed    : {r.real_cleanup_permission_dry_run_allowed}")
    ...
    print(f"  real_cleanup_requested                     : {r.real_cleanup_requested}")
...
def _write_report(
    r: TinyCleanupPermissionGateResult,
    output_dir: Path,
) -> None:
    ...
    json_path   = output_dir / f"{ts_safe}_tiny_cleanup_permission_gate.json"
    json_latest = output_dir / "latest_tiny_cleanup_permission_gate.json"
    md_path     = output_dir / f"{ts_safe}_tiny_cleanup_permission_gate.md"
    md_latest   = output_dir / "latest_tiny_cleanup_permission_gate.md"
    ...
    md_lines: list[str] = [
        "# Tiny Isolated Demo Cleanup Permission Gate (TASK-014Z)",
        "",
        ...
        f"| entry_rounded_tiny_qty | {r.entry_rounded_tiny_qty} |",
        f"| lifecycle_tiny_qty | {r.lifecycle_tiny_qty} |",
        f"| expected_tiny_qty | {r.expected_tiny_qty} |",
        f"| cleanup_token_pattern | {r.cleanup_token_pattern} |",
        f"| real_cleanup_permission_dry_run_allowed | {r.real_cleanup_permission_dry_run_allowed} |",
        ...
        f"| real_cleanup_requested | {r.real_cleanup_requested} |",
        ...
        "```",
        "",
        "## Post-cleanup Required Verification Plan",
        "",
        "```json",
        json.dumps(r.post_cleanup_verification_plan, indent=2),
        "```",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014Z is a PERMISSION GATE.  It NEVER opens a socket,",
        ...
        "> Real tiny cleanup execution is reserved for a future task;",
        "> even with --allow-real-cleanup this task returns",
        "> REAL_CLEANUP_NOT_IMPLEMENTED.",
        "",
    ]
    md_text = "\n".join(md_lines)
...
def run_execute(
    ...
    if allow_real_cleanup:
        print("REAL TINY CLEANUP GUARD --- NO NETWORK --- REAL_CLEANUP_NOT_IMPLEMENTED")
    elif allow_real_cleanup_permission:
        print("REAL CLEANUP PERMISSION DRY RUN --- NO NETWORK --- envelope-only")
    else:
        print("CHECKLIST --- NO NETWORK --- envelope-only")
    print("TASK-014Z: Tiny Isolated Demo Cleanup Permission Gate / Dry-run Only")
    print(_SEP)
    ...
    gate    = DemoTinyCleanupPermissionGate()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny isolated demo cleanup PERMISSION GATE / DRY-RUN ONLY. "
            "No network, no live endpoint, no orders / positions modified, "
            "no real cleanup call, no close-only sender, no emergency-close "
            "sender.  Even with --allow-real-cleanup-permission this only "
            "emits a checklist; even with --allow-real-cleanup it returns "
            "REAL_CLEANUP_NOT_IMPLEMENTED."
        ),
    )
    ...
    parser.add_argument("--allow-real-cleanup-permission", action="store_true",
                        help=("Promote envelope to cleanup_permission_dry_run.  "
                              "Even with this flag, TASK-014Z only emits a "
                              "checklist report.  No execution."))
    parser.add_argument("--allow-real-cleanup", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny cleanup (close-only).  TASK-014Z returns "
                              "REAL_CLEANUP_NOT_IMPLEMENTED even when this "
                              "flag is set (no socket opened)."))
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_real_cleanup_permission=args.allow_real_cleanup_permission,
        allow_real_cleanup=args.allow_real_cleanup,
