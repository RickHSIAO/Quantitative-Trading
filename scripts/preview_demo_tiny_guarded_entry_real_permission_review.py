"""
scripts/preview_demo_tiny_guarded_entry_real_permission_review.py
TASK-014AI: Guarded Tiny Entry Real Permission Review CLI.

Usage (PERMISSION REVIEW CHECKLIST --- default, no network, no implementation):
  python scripts/preview_demo_tiny_guarded_entry_real_permission_review.py \\
...
Usage (PERMISSION REVIEW APPROVAL --- review-only, no execution):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_real_permission_review/
      {ts}_tiny_guarded_entry_real_permission_review.json
      {ts}_tiny_guarded_entry_real_permission_review.md
      latest_tiny_guarded_entry_real_permission_review.json
      latest_tiny_guarded_entry_real_permission_review.md

IMPORTANT:
  - TASK-014AI produces a STRICT PERMISSION-REVIEW-ONLY artifact.  No
    guarded entry sender, no order send, no stop-attach send, no
    cleanup send, no endpoint invoked.  No network, no socket, no
    environment-variable reads, no signing, no order send.
...
Exit codes:
  0  tiny_guarded_entry_real_permission_review_ready /
...
def _print_result(r: TinyGuardedEntryRealPermissionReviewResult) -> None:
    ...
    print(f"  guarded_entry_real_permission_review       : {r.guarded_entry_real_permission_review}")
    ...
    print(f"  upstream_guarded_lifecycle_summary_readiness: {r.upstream_guarded_lifecycle_summary_readiness_conclusion}")
...
def _write_report(
    r: TinyGuardedEntryRealPermissionReviewResult,
    output_dir: Path,
) -> None:
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Real Permission Review (TASK-014AI)",
        "",
        ...
        "",
        "## Final Entry Real Permission Review Verdict",
        "",
        "| field | value |",
        "|---|---|",
        ...
        f"| guarded_entry_real_permission_review | {r.guarded_entry_real_permission_review} |",
        ...
        f"| upstream_guarded_lifecycle_summary_readiness_conclusion | {r.upstream_guarded_lifecycle_summary_readiness_conclusion} |",
        ...
        "",
        "## Permission Review Scope",
        "",
        "```json",
        json.dumps(r.permission_review_scope, indent=2),
        "```",
        "",
        "## Entry Real Permission Conditions",
        "",
        "```json",
        json.dumps(r.entry_real_permission_conditions, indent=2),
        "```",
        "",
        "## Manual Authorization Review",
        "",
        "```json",
        json.dumps(r.manual_authorization_review, indent=2),
        "```",
        "",
        "## Entry Request Review Envelope",
        "",
        "```json",
        json.dumps(r.entry_request_review_envelope, indent=2),
        "```",
        "",
        "## Required Post-Entry Protection Review",
        "",
        "```json",
        json.dumps(r.required_post_entry_protection_review, indent=2),
        "```",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_entry_real_permission_review_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AI is a STRICT PERMISSION-REVIEW-ONLY module.  It",
        "> NEVER opens a socket, NEVER invokes /v5/order/create, NEVER",
        "> invokes /v5/position/trading-stop, NEVER modifies any",
        "> position, NEVER invokes the close-only sender, NEVER invokes",
        "> the emergency-close sender, NEVER invokes the new-entry",
        "> sender's real execution path, NEVER invokes the real",
        "> stop-attach sender's real execution path, NEVER invokes a",
        "> real cleanup sender's real execution path, NEVER invokes a",
        "> real guarded entry sender's real execution path, NEVER",
        "> implements a real guarded entry runner, NEVER reads",
        "> environment variables / secrets, NEVER signs any request,",
        "> NEVER includes entry / stop-attach / cleanup execution,",
        "> NEVER lifts TASK-014L sender G20 (protected_entry_policy_missing).",
        "> The 5 existing demo positions are NEVER touched.",
        "> Even with --allow-review-approval the review only emits a",
        "> review artifact (readiness_conclusion remains",
        "> DESIGN_REVIEW_READY_NOT_EXECUTABLE).",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("PERMISSION REVIEW APPROVAL --- NO NETWORK --- review only")
    else:
        print("PERMISSION REVIEW CHECKLIST --- NO NETWORK --- review only")
    print("TASK-014AI: Guarded Tiny Entry Real Permission Review")
    print(_SEP)
    ...
    review = DemoTinyGuardedEntryRealPermissionReview()
    result = review.run_checklist(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry REAL PERMISSION REVIEW. "
            ...
            "guarded entry sender real execution.  Even with "
            "--allow-review-approval this only emits a review artifact; "
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to permission-review against.  MUST equal "
    ...
    parser.add_argument("--allow-review-approval", action="store_true",
                        help=("Promote envelope to permission_review_approval. "
                              "Even with this flag, TASK-014AI only emits a "
                              "permission-review artifact (readiness_conclusion "
    ...
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AI "
