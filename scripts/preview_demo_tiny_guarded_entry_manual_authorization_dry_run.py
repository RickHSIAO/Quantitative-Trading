"""
scripts/preview_demo_tiny_guarded_entry_manual_authorization_dry_run.py
TASK-014AK: Guarded Tiny Entry Manual Authorization Dry-run CLI.

Usage (AUTHORIZATION DRY-RUN CHECKLIST --- default, no network, no
       implementation):
  python scripts/preview_demo_tiny_guarded_entry_manual_authorization_dry_run.py \\
...
Usage (AUTHORIZATION DRY-RUN APPROVAL --- dry-run only, no execution):
  ... --allow-dry-run-approval
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_manual_authorization_dry_run/
      {ts}_tiny_guarded_entry_manual_authorization_dry_run.json
      {ts}_tiny_guarded_entry_manual_authorization_dry_run.md
      latest_tiny_guarded_entry_manual_authorization_dry_run.json
      latest_tiny_guarded_entry_manual_authorization_dry_run.md

IMPORTANT:
  - TASK-014AK produces a STRICT MANUAL-AUTHORIZATION-DRY-RUN-ONLY
    ...
    Token-pattern matching is simulated only.
  - Even with --allow-dry-run-approval the dry-run only emits a
    sanitized dry-run artifact.  Readiness conclusion is fixed at
    DESIGN_REVIEW_READY_NOT_EXECUTABLE.
  - Even with --allow-real-entry-execution the dry-run returns
    REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED (no socket opened).
...
Exit codes:
  0  tiny_guarded_entry_manual_authorization_dry_run_ready /
...
def _print_result(r: TinyGuardedEntryManualAuthorizationDryRunResult) -> None:
    ...
    print(f"  dry_run_approval_allowed                   : {r.dry_run_approval_allowed}")
    ...
    print(f"  guarded_entry_manual_authorization_dry_run : {r.guarded_entry_manual_authorization_dry_run}")
    print(f"  authorization_dry_run_only                 : {r.authorization_dry_run_only}")
    ...
    print(f"  upstream_entry_manual_auth_design_readiness: {r.upstream_entry_manual_auth_design_readiness_conclusion}")
...
def _write_report(
    r: TinyGuardedEntryManualAuthorizationDryRunResult,
    output_dir: Path,
) -> None:
    ...
    base = "tiny_guarded_entry_manual_authorization_dry_run"
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Manual Authorization Dry-run (TASK-014AK)",
        "",
        ...
        f"| guarded_entry_manual_authorization_dry_run | {r.guarded_entry_manual_authorization_dry_run} |",
        f"| authorization_dry_run_only | {r.authorization_dry_run_only} |",
        ...
        f"| upstream_entry_manual_auth_design_readiness_conclusion | {r.upstream_entry_manual_auth_design_readiness_conclusion} |",
        ...
        "",
        "## Authorization Dry-run Scope",
        "",
        "```json",
        json.dumps(r.authorization_dry_run_scope, indent=2),
        "```",
        "",
        "## Authorization Token Dry-run",
        "",
        "```json",
        json.dumps(r.authorization_token_dry_run, indent=2),
        "```",
        "",
        "## Required Flags Dry-run",
        "",
        "```json",
        json.dumps(r.required_flags_dry_run, indent=2),
        "```",
        "",
        "## Pre-Execution Readiness Dry-run",
        "",
        "```json",
        json.dumps(r.pre_execution_readiness_dry_run, indent=2),
        "```",
        "",
        "## Entry Request Template Dry-run",
        "",
        "```json",
        json.dumps(r.entry_request_template_dry_run, indent=2),
        "```",
        "",
        "## Post-Entry Boundary Dry-run",
        "",
        "```json",
        json.dumps(r.post_entry_boundary_dry_run, indent=2),
        "```",
        "",
        "## Failure and Abort Dry-run",
        "",
        "```json",
        json.dumps(r.failure_and_abort_dry_run, indent=2),
        "```",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_manual_authorization_dry_run_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AK is a STRICT MANUAL-AUTHORIZATION-DRY-RUN-ONLY",
        ...
        "> (protected_entry_policy_missing).  The 5 existing demo",
        "> positions are NEVER touched.  Even with",
        "> --allow-dry-run-approval the dry-run only emits a dry-run",
        "> artifact (readiness_conclusion remains",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("AUTHORIZATION DRY-RUN APPROVAL --- NO NETWORK --- dry-run only")
    else:
        print("AUTHORIZATION DRY-RUN CHECKLIST --- NO NETWORK --- dry-run only")
    print("TASK-014AK: Guarded Tiny Entry Manual Authorization Dry-run")
    print(_SEP)
    ...
    dry_run = DemoTinyGuardedEntryManualAuthorizationDryRun()
    result = dry_run.run_checklist(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry MANUAL AUTHORIZATION DRY-RUN. "
            ...
            "no token-to-authorization mapping.  Token-pattern matching "
            "is simulated only.  Even with --allow-dry-run-approval this "
            "only emits a dry-run artifact; even with "
            "--allow-real-entry-execution it returns "
            "REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED."
        ),
    )
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to dry-run authorization against.  MUST "
                              "equal SOLUSDT (post-entry symbol).  MUST NOT "
                              "be in the 5 existing demo position symbols "
                              "(ENAUSDT / TIAUSDT / AIXBTUSDT / POLYXUSDT / "
                              "EDUUSDT)."))
    parser.add_argument("--allow-dry-run-approval", action="store_true",
                        help=("Promote envelope to manual_authorization_dry_run_approval. "
                              "Even with this flag, TASK-014AK only emits a "
                              "manual-authorization-dry-run artifact "
    ...
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AK "
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_dry_run_approval=args.allow_dry_run_approval,
