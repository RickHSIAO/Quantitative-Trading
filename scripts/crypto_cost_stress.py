"""Crypto strategy cost stress test.

Runs the current Crypto strategy with identical signals and position rules while
changing only the backtest cost assumptions.
"""
...
def _scenarios(funding_daily: float) -> list[Scenario]:
    no_funding = {"Crypto": 0.0}
    funding = {"Crypto": funding_daily}
    no_extra_slip = {"Crypto": 0.0}
    extra_slip_0_1 = {"Crypto": 0.001}
    extra_slip_0_2 = {"Crypto": 0.002}
    return [
        Scenario("A", "current cost model", {
            "BACKTEST_TP_AS_TAKER": False,
            "BACKTEST_SLIPPAGE_ON_TP": False,
            "SLIPPAGE_PCT": 0.001,
            "BACKTEST_FUNDING_DAILY_PCT_BY_CLASS": no_funding,
            "BACKTEST_EXTRA_SLIPPAGE_PCT_BY_CLASS": no_extra_slip,
        }),
        Scenario("B", "TP taker fee + TP market slippage", {
            "BACKTEST_TP_AS_TAKER": True,
            "BACKTEST_SLIPPAGE_ON_TP": True,
            "SLIPPAGE_PCT": 0.001,
            "BACKTEST_FUNDING_DAILY_PCT_BY_CLASS": no_funding,
            "BACKTEST_EXTRA_SLIPPAGE_PCT_BY_CLASS": no_extra_slip,
        }),
        Scenario("C", "0.2% total market slippage", {
            "BACKTEST_TP_AS_TAKER": False,
            "BACKTEST_SLIPPAGE_ON_TP": False,
            "SLIPPAGE_PCT": 0.001,
            "BACKTEST_FUNDING_DAILY_PCT_BY_CLASS": no_funding,
            "BACKTEST_EXTRA_SLIPPAGE_PCT_BY_CLASS": extra_slip_0_1,
        }),
        Scenario("D", "0.3% total market slippage", {
            "BACKTEST_TP_AS_TAKER": False,
            "BACKTEST_SLIPPAGE_ON_TP": False,
            "SLIPPAGE_PCT": 0.001,
            "BACKTEST_FUNDING_DAILY_PCT_BY_CLASS": no_funding,
            "BACKTEST_EXTRA_SLIPPAGE_PCT_BY_CLASS": extra_slip_0_2,
        }),
        Scenario("E", f"funding cost {funding_daily:.4%} per day", {
            "BACKTEST_TP_AS_TAKER": False,
            "BACKTEST_SLIPPAGE_ON_TP": False,
            "SLIPPAGE_PCT": 0.001,
            "BACKTEST_FUNDING_DAILY_PCT_BY_CLASS": funding,
            "BACKTEST_EXTRA_SLIPPAGE_PCT_BY_CLASS": no_extra_slip,
        }),
        Scenario("F", f"TP taker + 0.3% slippage + funding {funding_daily:.4%} per day", {
            "BACKTEST_TP_AS_TAKER": True,
            "BACKTEST_SLIPPAGE_ON_TP": True,
            "SLIPPAGE_PCT": 0.001,
            "BACKTEST_FUNDING_DAILY_PCT_BY_CLASS": funding,
...
def _row(scenario: Scenario, metrics: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _print_rows(rows: list[dict[str, Any]]) -> None:
    print("\nCrypto cost stress test")
    print("scenario  return  CAGR    MDD      PF     Sharpe  Calmar  WR      avgR    trades  description")
    for row in rows:
        print(
            f"{row['scenario']:<8} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Crypto cost stress scenarios.")
    ...
    parser.add_argument("--funding-daily", type=float, default=0.0003,
                        help="Daily funding cost on notional. Default 0.0003 = 0.03%%/day.")
    parser.add_argument("--output", default="output/crypto_cost_stress.csv")
    ...
    for scenario in _scenarios(args.funding_daily):
        overrides = copy.deepcopy(candidate_overrides)
        overrides.update(copy.deepcopy(scenario.overrides))
        saved = _apply_overrides(overrides)
        try:
            if args.candidate:
                metrics = _run_candidate(base, args.start_date, args.end_date)
            else:
                metrics = _run_crypto(base, args.start_date, args.end_date)
            rows.append(_row(scenario, metrics))
        finally:
