...
Preview CLI for TASK-014BD -- Guarded Entry Real Execution Adapter
...
consumes the TASK-014BC manual authorization review dry-run JSON
artifact at runtime, evaluates BD's 36 hard-fail gates, and emits a
readiness-review artifact for the future TASK-014BE final pre-execution
review.

Usage (default -- consumes BC latest artifact):
  python scripts/preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_readiness_review.py \\
...
IMPORTANT:
  - TASK-014BD is a STRICT DISABLED-IMPLEMENTATION-SCAFFOLD-MANUAL-
    AUTHORIZATION-GATE-FINAL-PRE-EXECUTION-REVIEW-MANUAL-AUTHORIZATION-
    REVIEW-READINESS-REVIEW-ONLY review. No guarded entry sender, no
    adapter implementation, no order send, no stop-attach send, no
    cleanup send, no endpoint invoked. No network, no socket, no
    environment-variable reads, no signing. NO real token validation,
    NO token-to-authorization mapping, NO real phrase validation, NO
    phrase-to-authorization mapping, NO real approval-input validation.
    No automatic git commit, no automatic git push.
  - Even with --allow-disabled-...-readiness-review the module only
    emits a documented-only-never-authorized review artifact.
...
from src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_readiness_review import (
    ...
    get_default_bc_artifact_path,
    get_default_bd_output_dir,
    run_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_readiness_review as run_bd,
    write_report,
)

_SEP = "-" * 72

_DEFAULT_BC_ARTIFACT_DIR = ROOT / BC_DEFAULT_ARTIFACT_DIR
_DEFAULT_BC_ARTIFACT_FILE = _DEFAULT_BC_ARTIFACT_DIR / BC_DEFAULT_ARTIFACT_FILE
_DEFAULT_OUTPUT_DIR = ROOT / BD_DEFAULT_OUTPUT_DIR
...
def _print_banner() -> None:
    ...
    print(
        "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE FINAL "
        "PRE-EXECUTION REVIEW MANUAL AUTHORIZATION REVIEW READINESS REVIEW "
        "--- NO NETWORK --- documented-only-never-authorized"
    )
    print(_SEP)
    print(
...
def _print_result(r: BDResult) -> None:
    ...
    print(
        "  conclusion                                 : "
        f"{r.disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_readiness_review_conclusion}"
    )
    print(
        "  authorization_result                       : "
        f"{r.disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_readiness_review_authorization_result}"
    )
    ...
    print(f"  manual_authorization_review_readiness_review_only : {r.manual_authorization_review_readiness_review_only}")
...
def run_execute(
    *,
    ...
    allow_manual_authorization_review_readiness_review: bool = False,
    allow_real_entry_execution: bool = False,
    write_report_flag: bool = False,
    bc_artifact_path: Path | None = None,
    output_dir: Path | None = None,
) -> int:
    """Execute one BD readiness review and print a stdout summary.

    Returns an exit code suitable for sys.exit().
    """
    _bc_path = bc_artifact_path or _DEFAULT_BC_ARTIFACT_FILE
    _out_dir = output_dir or _DEFAULT_OUTPUT_DIR

    _print_banner()
    print(f"  bc_artifact_path                           : {_bc_path}")
    ...
    print(
        "  allow_disabled_..._readiness_review        : "
        f"{allow_manual_authorization_review_readiness_review}"
    )
    ...
    result = run_bd(
        symbol=symbol,
        bc_artifact_path=_bc_path,
        allow_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_readiness_review=allow_manual_authorization_review_readiness_review,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "PRE-EXECUTION REVIEW MANUAL AUTHORIZATION REVIEW READINESS "
            "REVIEW (TASK-014BD).  Consumes TASK-014BC Disabled "
            "Implementation Scaffold Manual Authorization Gate Final "
            "Pre-Execution Review Manual Authorization Review Dry-Run "
            "output at runtime (BC is the dry-run; BD is the readiness "
            "review) and produces a readiness-review artifact for the "
            "future TASK-014BE final pre-execution review.  "
            "Documented-only-never-authorized: readiness review only -- "
            "no real execution, no sender, no endpoint call, no secret "
            "read, no G20 lift, no position modification, main.py / "
            "src/risk.py / BybitExecutor untouched.  No network, no "
            "live endpoint, no orders / positions modified, no guarded "
            "entry sender implemented, no executable adapter, no "
            "adapter `send` method, no real entry execution, no real "
            "token validation, no token-to-authorization mapping, no "
            "real phrase validation, no phrase-to-authorization "
            "mapping, no real approval-input validation, no automatic "
            "git commit, no automatic git push.  Even with "
            ...
            "REVIEW MANUAL AUTHORIZATION REVIEW READINESS REVIEW READY "
            "NOT EXECUTABLE; even with --allow-real-entry-execution it "
            "returns REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED."
        ),
        epilog=(
            "TASK-014BD is a documented-only-never-authorized readiness "
            "review module.  It never authorizes real execution, never "
            "opens a socket, never invokes /v5/order/create, never "
            "invokes /v5/position/trading-stop, never modifies any of "
            "the 5 existing demo positions (ENAUSDT / TIAUSDT / "
            "AIXBTUSDT / POLYXUSDT / EDUUSDT), never lifts TASK-014L "
            "sender G20."
        ),
    )
    ...
    parser.add_argument(
        ...
        dest="from_latest_bc",
        help=(
            "Read the TASK-014BC disabled implementation scaffold "
            "manual authorization gate final pre-execution review "
            "manual authorization review dry-run JSON from the default "
            "outputs/demo_trading/.../manual_authorization_review_"
            "dry_run/latest_*.json location."
        ),
    )
    parser.add_argument(
        "--bc-artifact-path",
        ...
        metavar="PATH",
        help=(
            "Explicit BC artifact path override.  When set, this path "
            "is read instead of the default --from-latest-* location.  "
            "The file must be a valid TASK-014BC manual authorization "
            "review dry-run JSON artifact."
        ),
    )
    parser.add_argument(
        "--symbol",
    ...
    parser.add_argument(
        ...
        metavar="HASH",
        help=(
            "BD commit hash for the report (recorded only; this "
            "preview does not validate it as authorization)."
        ),
    )
    parser.add_argument(
        "--allow-disabled-implementation-scaffold-manual-authorization-gate-final-pre-execution-review-manual-authorization-review-readiness-review",
        dest="allow_manual_authorization_review_readiness_review",
        action="store_true",
        help=(
            "Promote envelope to manual_authorization_review_readiness_"
            "review checklist confirmation.  Even with this flag, "
            "TASK-014BD only emits a documented-only-never-authorized "
            "readiness-review artifact (conclusion remains READINESS_"
            "REVIEW_READY_NOT_EXECUTABLE).  No implementation, no "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "place_order / execute.  TASK-014BD returns "
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Override the default BD output directory.  Only "
    ...
    # Resolve BC artifact path: explicit override > --from-latest > default.
    if args.bc_artifact_path:
        bc_path: Path | None = Path(args.bc_artifact_path).resolve()
    elif args.from_latest_bc:
        bc_path = _DEFAULT_BC_ARTIFACT_FILE
    else:
        bc_path = _DEFAULT_BC_ARTIFACT_FILE
    ...
    sys.exit(
        run_execute(
            ...
            allow_manual_authorization_review_readiness_review=args.allow_manual_authorization_review_readiness_review,
            allow_real_entry_execution=args.allow_real_entry_execution,
            write_report_flag=args.write_report,
            bc_artifact_path=bc_path,
            output_dir=out_dir,
        )
    )
