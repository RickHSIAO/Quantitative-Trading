"""
scripts/preview_demo_tiny_guarded_entry_dry_run_adapter.py
TASK-014AE: Guarded Entry-only Dry-run Adapter CLI.

Usage (ENTRY ADAPTER CHECKLIST --- default, no network, no implementation):
  python scripts/preview_demo_tiny_guarded_entry_dry_run_adapter.py \\
...
Usage (ENTRY DRY-RUN APPROVAL --- dry-run only, no execution):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_dry_run_adapter/
      {ts}_tiny_guarded_entry_dry_run_adapter.json
      {ts}_tiny_guarded_entry_dry_run_adapter.md
      latest_tiny_guarded_entry_dry_run_adapter.json
      latest_tiny_guarded_entry_dry_run_adapter.md

IMPORTANT:
  - TASK-014AE produces an ENTRY-ONLY DRY-RUN ADAPTER artifact only.
    No guarded runner, no real entry, no stop attach, no cleanup is
    implemented.  No endpoint is invoked.  No network, no socket, no
    environment-variable reads, no signing, no order send.
  - Even with --allow-entry-dry-run-approval the adapter only emits a
    sanitized dry-run report.  Readiness conclusion is fixed at
    DESIGN_REVIEW_READY_NOT_EXECUTABLE.
  - Even with --allow-real-entry-execution the adapter returns
    REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED (no socket opened).
...
Exit codes:
  0  tiny_guarded_entry_dry_run_adapter_ready /
     tiny_guarded_entry_dry_run_adapter_ready_exec_disabled /
...
def _print_result(r: TinyGuardedEntryDryRunAdapterResult) -> None:
    ...
    for f in r.required_confirmation_flags:
        print(f"    - {f}")
    print(f"  entry_dry_run_approval_allowed             : {r.entry_dry_run_approval_allowed}")
    ...
    print(f"  guarded_entry_dry_run_adapter              : {r.guarded_entry_dry_run_adapter}")
    print(f"  entry_only                                 : {r.entry_only}")
...
def _write_report(
    r: TinyGuardedEntryDryRunAdapterResult,
    output_dir: Path,
) -> None:
    ...
    base = "tiny_guarded_entry_dry_run_adapter"
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry-only Dry-run Adapter (TASK-014AE)",
        "",
        ...
        f"| guarded_entry_dry_run_adapter | {r.guarded_entry_dry_run_adapter} |",
        f"| entry_only | {r.entry_only} |",
    ...
    md_lines += [
        ...
        "",
        "```json",
        json.dumps(r.entry_adapter_scope, indent=2),
        "```",
        "",
        "## Entry Precondition Contract",
        "",
        "```json",
        json.dumps(r.entry_precondition_contract, indent=2),
        "```",
        "",
        ...
        "```",
        "",
        "## Entry Request Envelope (preview only, no sender adapter)",
        "",
        "```json",
        json.dumps(r.entry_request_envelope, indent=2),
        "```",
        "",
        "## Entry Readonly Verification Plan",
        "",
        "```json",
        json.dumps(r.entry_readonly_verification_plan, indent=2),
        "```",
        "",
        "## Entry Failure Policy",
        "",
        "```json",
        json.dumps(r.entry_failure_policy, indent=2),
        "```",
        "",
        "## Audit Artifacts",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_entry_adapter_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AE is an ENTRY-ONLY DRY-RUN ADAPTER module.  It NEVER",
        ...
        "> real entry, NEVER includes stop attach or cleanup, NEVER lifts",
        "> TASK-014L sender G20 (protected_entry_policy_missing).",
        "> The 5 existing demo positions are NEVER touched.",
        "> Even with --allow-entry-dry-run-approval the adapter only emits",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("ENTRY DRY-RUN APPROVAL --- NO NETWORK --- dry-run only")
    else:
        print("ENTRY ADAPTER CHECKLIST --- NO NETWORK --- dry-run only")
    print("TASK-014AE: Guarded Entry-only Dry-run Adapter")
    print(_SEP)
    ...
    adapter = DemoTinyGuardedEntryDryRunAdapter()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry-only DRY-RUN ADAPTER. "
            "No network, no live endpoint, no orders / positions modified, "
            "no guarded runner implemented, no real entry implemented, no "
            "stop attach included, no cleanup included, no full lifecycle "
            "included, no close-only sender, no emergency-close sender, no "
            "new-entry sender real execution.  Even with "
            "--allow-entry-dry-run-approval this only emits a dry-run "
            "adapter artifact; even with --allow-real-entry-execution it "
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to dry-run-adapt against.  MUST NOT be "
                              "in the 5 existing demo position symbols "
                              "(ENAUSDT / TIAUSDT / AIXBTUSDT / POLYXUSDT / "
                              "EDUUSDT)."))
    parser.add_argument("--allow-entry-dry-run-approval", action="store_true",
                        help=("Promote envelope to entry_dry_run_approval. "
                              "Even with this flag, TASK-014AE only emits a "
    ...
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry execution.  TASK-014AE returns "
                              "REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED even when "
                              "this flag is set (no socket opened, no real "
                              "entry implemented)."))
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_entry_dry_run_approval=args.allow_entry_dry_run_approval,
