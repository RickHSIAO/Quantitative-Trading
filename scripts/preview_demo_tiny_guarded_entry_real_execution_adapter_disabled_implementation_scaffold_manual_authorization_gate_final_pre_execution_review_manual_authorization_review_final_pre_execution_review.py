...
Preview CLI for TASK-014BE -- Guarded Entry Real Execution Adapter
...
consumes the TASK-014BD manual authorization review READINESS REVIEW
JSON artifact at runtime, evaluates BE's 37 hard-fail gates, and emits
a final-pre-execution-review artifact for the future TASK-014BF manual
authorization review.

TASK phase mapping:
  * TASK-014BE -- FINAL PRE-EXECUTION REVIEW (this module).
  * TASK-014BD -- READINESS REVIEW (direct upstream consumed at runtime).
  * TASK-014BC -- DRY-RUN (chained proof through BD only).
  * TASK-014BB / BA / AZ / AY / AX / AW / AV / AU / AT / AS / AR / AQ --
    chained proof through BD; NOT consumed directly by BE.

Usage (default -- consumes BD latest artifact):
  python scripts/preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_final_pre_execution_review.py \\
...
IMPORTANT:
  - TASK-014BE is a STRICT DISABLED-IMPLEMENTATION-SCAFFOLD-MANUAL-
    ...
    REVIEW-FINAL-PRE-EXECUTION-REVIEW-ONLY review.  No guarded entry
    sender, no adapter implementation, no order send, no stop-attach
    send, no cleanup send, no endpoint invoked.  No network, no socket,
    no environment-variable reads, no signing.  NO real token
    validation, NO token-to-authorization mapping, NO real phrase
    validation, NO phrase-to-authorization mapping, NO real
    approval-input validation.  No automatic git commit, no automatic
    git push.
  - Even with --allow-disabled-..-final-pre-execution-review the module
...
from src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_final_pre_execution_review import (
    ...
    get_default_bd_artifact_path,
    get_default_be_output_dir,
    run_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_final_pre_execution_review as run_be,
    write_report,
)

_SEP = "-" * 72

_DEFAULT_BD_ARTIFACT_DIR = ROOT / BD_DEFAULT_ARTIFACT_DIR
_DEFAULT_BD_ARTIFACT_FILE = _DEFAULT_BD_ARTIFACT_DIR / BD_DEFAULT_ARTIFACT_FILE
_DEFAULT_OUTPUT_DIR = ROOT / BE_DEFAULT_OUTPUT_DIR
...
def _print_banner() -> None:
    ...
    print(
        ...
        "PRE-EXECUTION REVIEW --- NO NETWORK --- "
...
def _print_result(r: BEResult) -> None:
    ...
    print(
        "  conclusion                                 : "
        f"{r.disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_final_pre_execution_review_conclusion}"
    )
    print(
        "  authorization_result                       : "
        f"{r.disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_final_pre_execution_review_authorization_result}"
    )
    ...
    print(f"  manual_authorization_review_final_pre_execution_review_only : {r.manual_authorization_review_final_pre_execution_review_only}")
...
def run_execute(
    *,
    ...
    allow_manual_authorization_review_final_pre_execution_review: bool = False,
    allow_real_entry_execution: bool = False,
    write_report_flag: bool = False,
    bd_artifact_path: Path | None = None,
    output_dir: Path | None = None,
) -> int:
    """Execute one BE final pre-execution review and print a stdout summary.

    Returns an exit code suitable for sys.exit().
    """
    _bd_path = bd_artifact_path or _DEFAULT_BD_ARTIFACT_FILE
    _out_dir = output_dir or _DEFAULT_OUTPUT_DIR

    _print_banner()
    print(f"  bd_artifact_path                           : {_bd_path}")
    ...
    print(
        "  allow_disabled_..._final_pre_execution_review : "
        f"{allow_manual_authorization_review_final_pre_execution_review}"
    )
    ...
    result = run_be(
        symbol=symbol,
        bd_artifact_path=_bd_path,
        allow_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_final_pre_execution_review=allow_manual_authorization_review_final_pre_execution_review,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "PRE-EXECUTION REVIEW MANUAL AUTHORIZATION REVIEW FINAL "
            "PRE-EXECUTION REVIEW (TASK-014BE).  Consumes TASK-014BD "
            "Disabled Implementation Scaffold Manual Authorization Gate "
            "Final Pre-Execution Review Manual Authorization Review "
            "Readiness Review output at runtime (BD is the readiness "
            "review; BE is the final pre-execution review; BC is the "
            "dry-run) and produces a final-pre-execution-review artifact "
            "for the future TASK-014BF manual authorization review.  "
            "BE directly consumes BD only -- BC, BB, BA, AZ, AY, AX, AW, "
            "AV, AU, AT, AS, AR, AQ are chained proof through BD, not "
            "consumed directly by BE.  Documented-only-never-authorized: "
            "final pre-execution review only -- no real execution, no "
            ...
            "REVIEW READY NOT EXECUTABLE; even with "
            "--allow-real-entry-execution it returns "
            "REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED."
        ),
        epilog=(
            "TASK-014BE is a documented-only-never-authorized final "
            "pre-execution review module.  It never authorizes real "
            "execution, never opens a socket, never invokes "
            "/v5/order/create, never invokes "
    ...
    parser.add_argument(
        ...
        dest="from_latest_bd",
        help=(
            "Read the TASK-014BD disabled implementation scaffold "
            "manual authorization gate final pre-execution review "
            "manual authorization review READINESS REVIEW JSON from the "
            "default outputs/demo_trading/.../manual_authorization_"
            "review_readiness_review/latest_*.json location.  BD is the "
            "readiness review consumed by BE; BE itself is the final "
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Explicit BD artifact path override.  When set, this path "
            "is read instead of the default --from-latest-* location.  "
            "The file must be a valid TASK-014BD manual authorization "
            "review readiness review JSON artifact."
        ),
    )
    parser.add_argument(
        "--symbol",
    ...
    parser.add_argument(
        ...
        metavar="HASH",
        help=(
            "BE commit hash for the report (recorded only; this "
            "preview does not validate it as authorization)."
        ),
    )
    parser.add_argument(
        "--allow-disabled-implementation-scaffold-manual-authorization-gate-final-pre-execution-review-manual-authorization-review-final-pre-execution-review",
        dest="allow_manual_authorization_review_final_pre_execution_review",
        action="store_true",
        help=(
            "Promote envelope to manual_authorization_review_final_pre_"
            "execution_review checklist confirmation.  Even with this "
            "flag, TASK-014BE only emits a documented-only-never-"
            "authorized final-pre-execution-review artifact (conclusion "
            "remains FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE).  "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "place_order / execute.  TASK-014BE returns "
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Override the default BE output directory.  Only "
    ...
    # Resolve BD artifact path: explicit override > --from-latest > default.
    if args.bd_artifact_path:
        bd_path: Path | None = Path(args.bd_artifact_path).resolve()
    elif args.from_latest_bd:
        bd_path = _DEFAULT_BD_ARTIFACT_FILE
    else:
        bd_path = _DEFAULT_BD_ARTIFACT_FILE
    ...
    sys.exit(
        run_execute(
            ...
            allow_manual_authorization_review_final_pre_execution_review=args.allow_manual_authorization_review_final_pre_execution_review,
            allow_real_entry_execution=args.allow_real_entry_execution,
            write_report_flag=args.write_report,
            bd_artifact_path=bd_path,
            output_dir=out_dir,
        )
    )
