"""
scripts/preview_demo_tiny_guarded_entry_real_execution_manual_approval_gate.py
TASK-014AM: Guarded Tiny Entry Real Execution Manual Approval Gate CLI.

Usage (MANUAL APPROVAL GATE CHECKLIST --- default, no network,
       no implementation):
  python scripts/preview_demo_tiny_guarded_entry_real_execution_manual_approval_gate.py \\
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_real_execution_manual_approval_gate/
      {ts}_tiny_guarded_entry_real_execution_manual_approval_gate.json
      {ts}_tiny_guarded_entry_real_execution_manual_approval_gate.md
      latest_tiny_guarded_entry_real_execution_manual_approval_gate.json
      latest_tiny_guarded_entry_real_execution_manual_approval_gate.md

IMPORTANT:
  - TASK-014AM produces a STRICT MANUAL-APPROVAL-GATE-ONLY artifact.
    ...
    phrase validation, NO phrase-to-authorization mapping.  Token-
    and phrase-pattern matching is documented only.  No automatic
    git commit, no automatic git push.
  - Even with --allow-approval-gate the gate only emits a sanitized
    gate artifact.  Readiness conclusion is fixed at
    DESIGN_REVIEW_READY_NOT_EXECUTABLE.
  - Even with --allow-real-entry-execution the gate returns
    REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED (no socket opened).
  ...
  - No close-only sender / no emergency-close sender / no new-entry
    ...
    entry sender real execution path is invoked.

Exit codes:
  0  tiny_guarded_entry_real_execution_manual_approval_gate_ready /
...
_DEFAULT_OUTPUT_DIR          = (
    ROOT / "outputs" / "demo_trading" / "tiny_guarded_entry_real_execution_manual_approval_gate"
)
...
def _print_result(r: TinyGuardedEntryRealExecutionManualApprovalGateResult) -> None:
    ...
    print(f"  exact_approval_phrase (documented only)    : {r.exact_approval_phrase}")
    print(f"  entry_token_pattern                        : {r.entry_token_pattern}")
    print(f"  sample_token                               : {r.sample_token}")
    print(f"  order_link_id_prefix                       : {r.order_link_id_prefix}")
    print(f"  approval_gate_allowed                      : {r.approval_gate_allowed}")
    ...
    print(f"  guarded_entry_real_execution_manual_gate   : {r.guarded_entry_real_execution_manual_approval_gate}")
    print(f"  manual_approval_gate_only                  : {r.manual_approval_gate_only}")
    print(f"  approval_grants_execution                  : {r.approval_grants_execution}")
    print(f"  token_validation_simulated                 : {r.token_validation_simulated}")
    print(f"  token_validated                            : {r.token_validated}")
    print(f"  real_token_validated                       : {r.real_token_validated}")
    print(f"  exact_phrase_validated                     : {r.exact_phrase_validated}")
    print(f"  approval_inputs_validated                  : {r.approval_inputs_validated}")
    ...
    print(f"  upstream_entry_final_pre_execution_review_readiness: {r.upstream_entry_final_pre_execution_review_readiness_conclusion}")
    print(f"  upstream_entry_final_pre_execution_review_auth     : {r.upstream_entry_final_pre_execution_review_authorization_result}")
...
def _write_report(
    r: TinyGuardedEntryRealExecutionManualApprovalGateResult,
    output_dir: Path,
) -> None:
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Real Execution Manual Approval Gate (TASK-014AM)",
        "",
        ...
        f"| guarded_entry_real_execution_manual_approval_gate | {r.guarded_entry_real_execution_manual_approval_gate} |",
        f"| manual_approval_gate_only | {r.manual_approval_gate_only} |",
        f"| approval_grants_execution | {r.approval_grants_execution} |",
        f"| token_validation_simulated | {r.token_validation_simulated} |",
        f"| token_validated | {r.token_validated} |",
        f"| real_token_validated | {r.real_token_validated} |",
        f"| exact_phrase_validated | {r.exact_phrase_validated} |",
        f"| approval_inputs_validated | {r.approval_inputs_validated} |",
        ...
        f"| upstream_entry_final_pre_execution_review_readiness_conclusion | {r.upstream_entry_final_pre_execution_review_readiness_conclusion} |",
        f"| upstream_entry_final_pre_execution_review_authorization_result | {r.upstream_entry_final_pre_execution_review_authorization_result} |",
        f"| next_required_task | {r.next_required_task} |",
        "",
        "## Token / Phrase Patterns (documented only, NEVER validated as authorization)",
        "",
        f"- exact_approval_phrase: `{r.exact_approval_phrase}`",
        f"- entry_token_pattern:   `{r.entry_token_pattern}`",
        f"- sample_token:          `{r.sample_token}`",
        f"- order_link_id_prefix:  `{r.order_link_id_prefix}`",
        "",
        "## Manual Approval Gate Scope",
        "",
        "```json",
        json.dumps(r.manual_approval_gate_scope, indent=2),
        "```",
        "",
        "## Manual Approval Token Gate",
        "",
        "```json",
        json.dumps(r.manual_approval_token_gate, indent=2),
        "```",
        "",
        "## Required Manual Approval Inputs",
        "",
        "```json",
        json.dumps(r.required_manual_approval_inputs, indent=2),
        "```",
        "",
        "## Approval Gate Readiness Review",
        "",
        "```json",
        json.dumps(r.approval_gate_readiness_review, indent=2),
        "```",
        "",
        "## Entry Payload Approval Preview",
        "",
        "```json",
        json.dumps(r.entry_payload_approval_preview, indent=2),
        "```",
        "",
        "## Stop / Cleanup Manual Gate Review",
        "",
        "```json",
        json.dumps(r.stop_cleanup_manual_gate_review, indent=2),
        "```",
        "",
        "## Forbidden Execution Surface Review",
        "",
        "```json",
        json.dumps(r.forbidden_execution_surface_review, indent=2),
        "```",
        "",
        "## Failure and Abort Manual Gate",
        "",
        "```json",
        json.dumps(r.failure_and_abort_manual_gate, indent=2),
        "```",
        "",
        ...
        "```",
        "",
        "## Final Manual Approval Gate Verdict",
        "",
        "```json",
        json.dumps(r.final_manual_approval_gate_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AM is a STRICT MANUAL-APPROVAL-GATE-ONLY module.",
        ...
        "> a real guarded entry runner, NEVER reads environment",
        "> variables / secrets, NEVER signs any request, NEVER validates",
        "> the token or phrase as real authorization (only documents",
        "> the patterns), NEVER treats any token or phrase as",
        "> authorization, NEVER includes entry / stop-attach / cleanup",
        "> execution, NEVER lifts TASK-014L sender G20",
        "> (protected_entry_policy_missing), NEVER performs any",
        "> automatic git commit, NEVER performs any automatic git",
        "> push.  The 5 existing demo positions are NEVER touched.",
        "> Even with --allow-approval-gate the gate only emits a",
        "> sanitized gate artifact (readiness_conclusion remains",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("MANUAL APPROVAL GATE APPROVAL --- NO NETWORK --- gate-only")
    else:
        print("MANUAL APPROVAL GATE CHECKLIST --- NO NETWORK --- gate-only")
    print("TASK-014AM: Guarded Tiny Entry Real Execution Manual Approval Gate")
    print(_SEP)
    ...
    gate = DemoTinyGuardedEntryRealExecutionManualApprovalGate()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry REAL EXECUTION MANUAL APPROVAL GATE. "
            "No network, no live endpoint, no orders / positions modified, "
            "no guarded entry sender implemented, no real entry execution, "
            "no entry / stop-attach / cleanup execution included, no "
            ...
            "guarded entry sender real execution, no real token validation, "
            "no token-to-authorization mapping, no real phrase validation, "
            "no phrase-to-authorization mapping, no automatic git commit, "
            "no automatic git push.  Token- and phrase-pattern matching is "
            "documented only.  Even with --allow-approval-gate this only "
            "emits a gate artifact; even with --allow-real-entry-execution "
            "it returns REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED."
        ),
    )
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to manual-approval-gate against.  "
    ...
    parser.add_argument("--expected-commit-hash", default="",
                        ...
                        help=("Optional expected commit hash to record in the "
                              ...
                              "gate NEVER performs an automatic git commit "
                              "or push)."))
    parser.add_argument("--allow-approval-gate", action="store_true",
                        help=("Promote envelope to manual_approval_gate_approval. "
                              "Even with this flag, TASK-014AM only emits a "
                              "manual-approval-gate artifact "
                              "(readiness_conclusion stays "
                              "DESIGN_REVIEW_READY_NOT_EXECUTABLE).  "
                              "No execution, no implementation, no real "
                              "token validation, no token-to-authorization "
                              "mapping, no real phrase validation, no "
                              "phrase-to-authorization mapping, no "
                              "automatic git commit, no automatic git push."))
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AM "
                              "returns REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED "
                              "even when this flag is set (no socket opened, "
                              "no real sender implemented)."))
    ...
    sys.exit(run_execute(
        ...
        allow_approval_gate=args.allow_approval_gate,
