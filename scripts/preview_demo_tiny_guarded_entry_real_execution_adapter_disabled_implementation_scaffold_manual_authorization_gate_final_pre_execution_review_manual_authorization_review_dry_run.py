...
Preview CLI for TASK-014BC -- Guarded Entry Real Execution Adapter
...
consumes the TASK-014BB manual authorization review JSON artifact at
runtime, evaluates BC's 36 hard-fail gates, and emits a dry-run review
artifact for the future TASK-014BD readiness review.

Usage (default -- consumes BB latest artifact):
  python scripts/preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_dry_run.py \\
...
IMPORTANT:
  - TASK-014BC is a STRICT DISABLED-IMPLEMENTATION-SCAFFOLD-MANUAL-
    AUTHORIZATION-GATE-FINAL-PRE-EXECUTION-REVIEW-MANUAL-AUTHORIZATION-
    REVIEW-DRY-RUN-ONLY review. No guarded entry sender, no adapter
    implementation, no order send, no stop-attach send, no cleanup send,
    no endpoint invoked. No network, no socket, no environment-variable
    reads, no signing. NO real token validation, NO token-to-
    authorization mapping, NO real phrase validation, NO phrase-to-
    authorization mapping, NO real approval-input validation. No
    automatic git commit, no automatic git push.
  - Even with --allow-disabled-...-dry-run the module only emits a
    documented-only-never-authorized review artifact.
...
from src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_dry_run import (
    ...
    get_default_bb_artifact_path,
    get_default_bc_output_dir,
    run_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_dry_run as run_bc,
    write_report,
)

_SEP = "-" * 72

_DEFAULT_BB_ARTIFACT_DIR = ROOT / BB_DEFAULT_ARTIFACT_DIR
_DEFAULT_BB_ARTIFACT_FILE = _DEFAULT_BB_ARTIFACT_DIR / BB_DEFAULT_ARTIFACT_FILE
_DEFAULT_OUTPUT_DIR = ROOT / BC_DEFAULT_OUTPUT_DIR
...
def _print_result(r: BCResult) -> None:
    ...
    print(
        "  conclusion                                 : "
        f"{r.disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_dry_run_conclusion}"
    )
    print(
        "  authorization_result                       : "
        f"{r.disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_dry_run_authorization_result}"
    )
    ...
    print(f"  manual_authorization_review_dry_run_only   : {r.manual_authorization_review_dry_run_only}")
...
def run_execute(
    *,
    ...
    """Execute one BC dry-run review and print a stdout summary.

    Returns an exit code suitable for sys.exit().
    """
    _bb_path = bb_artifact_path or _DEFAULT_BB_ARTIFACT_FILE
    _out_dir = output_dir or _DEFAULT_OUTPUT_DIR

    _print_banner()
    print(f"  bb_artifact_path                           : {_bb_path}")
    ...
    result = run_bc(
        symbol=symbol,
        bb_artifact_path=_bb_path,
        allow_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review_dry_run=allow_manual_authorization_review_dry_run,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Review Manual Authorization Review output at runtime "
            "(BB is NOT a dry-run; BC is the dry-run) and produces a "
            "dry-run review artifact for the future TASK-014BD "
            "readiness review.  "
            "Documented-only-never-authorized: dry-run review only -- "
            ...
            "SCAFFOLD MANUAL AUTHORIZATION GATE FINAL PRE-EXECUTION "
            "REVIEW MANUAL AUTHORIZATION REVIEW DRY RUN READY NOT "
            "EXECUTABLE; even with --allow-real-entry-execution it "
            "returns REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED."
        ),
        epilog=(
            "TASK-014BC is a documented-only-never-authorized dry-run "
    ...
    parser.add_argument(
        ...
        dest="from_latest_bb",
        help=(
            "Read the TASK-014BB disabled implementation scaffold "
            "manual authorization gate final pre-execution review "
            "manual authorization review JSON from the default "
            "outputs/demo_trading/.../manual_authorization_review/"
            "latest_*.json location."
        ),
    )
    parser.add_argument(
        "--bb-artifact-path",
        ...
        metavar="PATH",
        help=(
            "Explicit BB artifact path override.  When set, this path "
            "is read instead of the default --from-latest-* location.  "
            "The file must be a valid TASK-014BB manual authorization "
    ...
    parser.add_argument(
        ...
        metavar="HASH",
        help=(
            "BC commit hash for the report (recorded only; this "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "checklist confirmation.  Even with this flag, TASK-014BC "
            "only emits a documented-only-never-authorized dry-run "
            "review artifact (conclusion remains DRY_RUN_READY_NOT_"
            "EXECUTABLE).  No implementation, no execution, no real "
            "token validation, no token-to-authorization mapping, no "
            "real phrase validation, no phrase-to-authorization "
            "mapping, no real approval-input validation, no automatic "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "place_order / execute.  TASK-014BC returns "
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Override the default BC output directory.  Only "
    ...
    # Resolve BB artifact path: explicit override > --from-latest > default.
    if args.bb_artifact_path:
        bb_path: Path | None = Path(args.bb_artifact_path).resolve()
    elif args.from_latest_bb:
        bb_path = _DEFAULT_BB_ARTIFACT_FILE
    else:
        bb_path = _DEFAULT_BB_ARTIFACT_FILE
