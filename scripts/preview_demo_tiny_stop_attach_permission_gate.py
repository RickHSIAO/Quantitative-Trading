"""
scripts/preview_demo_tiny_stop_attach_permission_gate.py
TASK-014Y: Tiny Isolated Demo Stop Attach Permission Gate / Dry-run Only CLI.

Usage (CHECKLIST --- default, no network, envelope-only):
  python scripts/preview_demo_tiny_stop_attach_permission_gate.py \\
...
Usage (REAL STOP PERMISSION DRY RUN --- envelope-only):
  ... --allow-real-stop-permission

Usage (REAL TINY STOP ATTACH GUARD --- always returns
       REAL_STOP_ATTACH_NOT_IMPLEMENTED, no socket opened):
  ... --allow-real-tiny-stop-attach
...
Writes (when --write-report):
  outputs/demo_trading/tiny_stop_attach_permission_gate/
      {ts}_tiny_stop_attach_permission_gate.json
      {ts}_tiny_stop_attach_permission_gate.md
      latest_tiny_stop_attach_permission_gate.json
      latest_tiny_stop_attach_permission_gate.md

IMPORTANT:
  - This is a PERMISSION GATE module.  No network at all.  Even with
    --allow-real-stop-permission, the gate only emits a checklist
    report and never executes anything.  Even with
    --allow-real-tiny-stop-attach, the gate returns
    REAL_STOP_ATTACH_NOT_IMPLEMENTED.  Real stop-attach execution is
...
_DEFAULT_OUTPUT_DIR          = ROOT / "outputs" / "demo_trading" / "tiny_stop_attach_permission_gate"
...
def _print_result(r: TinyStopAttachPermissionGateResult) -> None:
    print(f"  mode                                 : {r.mode}")
    ...
    print(f"  stop_price_aligned_with_tick         : {r.stop_price_aligned_with_tick}")
    ...
    print(f"  post_stop_attach_verification_plan   : {r.post_stop_attach_verification_plan}")
    print(f"  failure_response_plan                : {r.failure_response_plan}")
    print(f"  real_stop_permission_dry_run_allowed : {r.real_stop_permission_dry_run_allowed}")
    ...
    print(f"  real_stop_attach_requested           : {r.real_stop_attach_requested}")
...
def _write_report(
    r: TinyStopAttachPermissionGateResult,
    output_dir: Path,
) -> None:
    ...
    json_path   = output_dir / f"{ts_safe}_tiny_stop_attach_permission_gate.json"
    json_latest = output_dir / "latest_tiny_stop_attach_permission_gate.json"
    md_path     = output_dir / f"{ts_safe}_tiny_stop_attach_permission_gate.md"
    md_latest   = output_dir / "latest_tiny_stop_attach_permission_gate.md"
    ...
    md_lines: list[str] = [
        "# Tiny Isolated Demo Stop Attach Permission Gate (TASK-014Y)",
        "",
        ...
        f"| stop_price_aligned_with_tick | {r.stop_price_aligned_with_tick} |",
        f"| tick_size | {r.tick_size} |",
        f"| stop_attach_token_pattern | {r.stop_attach_token_pattern} |",
        f"| real_stop_permission_dry_run_allowed | {r.real_stop_permission_dry_run_allowed} |",
        ...
        f"| real_stop_attach_requested | {r.real_stop_attach_requested} |",
        ...
        "```",
        "",
        "## Post-stop-attach Required Verification Plan",
        "",
        "```json",
        json.dumps(r.post_stop_attach_verification_plan, indent=2),
        "```",
        "",
        "## Failure Response Plan",
        "",
        "```json",
        json.dumps(r.failure_response_plan, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014Y is a PERMISSION GATE.  It NEVER opens a socket,",
        "> NEVER invokes /v5/position/trading-stop, NEVER invokes",
        "> /v5/order/create, NEVER modifies any position, and NEVER",
        "> lifts TASK-014L sender G20 (protected_entry_policy_missing).",
        "> The 5 existing demo positions are NEVER touched.",
        "> Real tiny stop-attach execution is reserved for a future task;",
        "> even with --allow-real-tiny-stop-attach this task returns",
        "> REAL_STOP_ATTACH_NOT_IMPLEMENTED.",
        "",
    ]
    md_text = "\n".join(md_lines)
...
def run_execute(
    ...
    if allow_real_tiny_stop_attach:
        print("REAL TINY STOP ATTACH GUARD --- NO NETWORK --- REAL_STOP_ATTACH_NOT_IMPLEMENTED")
    elif allow_real_stop_permission:
        print("REAL STOP PERMISSION DRY RUN --- NO NETWORK --- envelope-only")
    else:
        print("CHECKLIST --- NO NETWORK --- envelope-only")
    print("TASK-014Y: Tiny Isolated Demo Stop Attach Permission Gate / Dry-run Only")
    print(_SEP)
    ...
    gate    = DemoTinyStopAttachPermissionGate()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny isolated demo stop-attach PERMISSION GATE / DRY-RUN ONLY. "
            "No network, no live endpoint, no orders / positions modified, "
            "no real trading-stop call.  Even with "
            "--allow-real-stop-permission this only emits a checklist; "
            "even with --allow-real-tiny-stop-attach it returns "
            "REAL_STOP_ATTACH_NOT_IMPLEMENTED."
        ),
    )
    ...
    parser.add_argument("--allow-real-stop-permission", action="store_true",
                        help=("Promote envelope to stop_permission_dry_run.  "
                              "Even with this flag, TASK-014Y only emits a "
                              "checklist report.  No execution."))
    parser.add_argument("--allow-real-tiny-stop-attach", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny stop attach.  TASK-014Y returns "
                              "REAL_STOP_ATTACH_NOT_IMPLEMENTED even when "
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_real_stop_permission=args.allow_real_stop_permission,
        allow_real_tiny_stop_attach=args.allow_real_tiny_stop_attach,
