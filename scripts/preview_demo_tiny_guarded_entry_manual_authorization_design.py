"""
scripts/preview_demo_tiny_guarded_entry_manual_authorization_design.py
TASK-014AJ: Guarded Tiny Entry Manual Authorization Design CLI.

Usage (AUTHORIZATION DESIGN CHECKLIST --- default, no network, no implementation):
  python scripts/preview_demo_tiny_guarded_entry_manual_authorization_design.py \\
...
Usage (AUTHORIZATION DESIGN APPROVAL --- design-only, no execution):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_manual_authorization_design/
      {ts}_tiny_guarded_entry_manual_authorization_design.json
      {ts}_tiny_guarded_entry_manual_authorization_design.md
      latest_tiny_guarded_entry_manual_authorization_design.json
      latest_tiny_guarded_entry_manual_authorization_design.md

IMPORTANT:
  - TASK-014AJ produces a STRICT MANUAL-AUTHORIZATION-DESIGN-ONLY
    ...
    NO token validation, NO token-to-authorization mapping.
  - Even with --allow-design-approval the design only emits a
    sanitized design artifact.  Readiness conclusion is fixed at
...
Exit codes:
  0  tiny_guarded_entry_manual_authorization_design_ready /
...
def _print_result(r: TinyGuardedEntryManualAuthorizationDesignResult) -> None:
    ...
    print(f"  required_human_confirmation_flag_count     : {len(r.required_human_confirmation_flags)}")
    print(f"  design_approval_allowed                    : {r.design_approval_allowed}")
    ...
    print(f"  guarded_entry_manual_authorization_design  : {r.guarded_entry_manual_authorization_design}")
    print(f"  authorization_design_only                  : {r.authorization_design_only}")
    print(f"  permission_review_only                     : {r.permission_review_only}")
    ...
    print(f"  upstream_entry_real_permission_review_readiness: {r.upstream_entry_real_permission_review_readiness_conclusion}")
...
def _write_report(
    r: TinyGuardedEntryManualAuthorizationDesignResult,
    output_dir: Path,
) -> None:
    ...
    base = "tiny_guarded_entry_manual_authorization_design"
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Manual Authorization Design (TASK-014AJ)",
        "",
        ...
        f"| guarded_entry_manual_authorization_design | {r.guarded_entry_manual_authorization_design} |",
        f"| authorization_design_only | {r.authorization_design_only} |",
        f"| permission_review_only | {r.permission_review_only} |",
        ...
        f"| upstream_entry_real_permission_review_readiness_conclusion | {r.upstream_entry_real_permission_review_readiness_conclusion} |",
        f"| next_required_task | {r.next_required_task} |",
        "",
        "## Token Pattern (documented, never validated by this task)",
        "",
        f"- entry_token_pattern: `{r.entry_token_pattern}`",
        "",
        "## Required Human Confirmation Flags (design only --- never asserted)",
        "",
    ]
    for flag in r.required_human_confirmation_flags:
    ...
    md_lines += [
        ...
        "",
        "```json",
        json.dumps(r.authorization_design_scope, indent=2),
        "```",
        "",
        "## Authorization Token Design",
        "",
        "```json",
        json.dumps(r.authorization_token_design, indent=2),
        "```",
        "",
        "## Pre-Execution Readiness Checklist Design",
        "",
        "```json",
        json.dumps(r.pre_execution_readiness_checklist_design, indent=2),
        "```",
        "",
        "## Entry Execution Request Template Design",
        "",
        "```json",
        json.dumps(r.entry_execution_request_template_design, indent=2),
        "```",
        "",
        "## Post-Authorization Stop/Cleanup Boundary Design",
        "",
        "```json",
        json.dumps(r.post_authorization_stop_cleanup_boundary_design, indent=2),
        "```",
        "",
        "## Failure and Abort Design",
        "",
        "```json",
        json.dumps(r.failure_and_abort_design, indent=2),
        "```",
        "",
        ...
        "",
        "```json",
        json.dumps(r.final_manual_authorization_design_verdict, indent=2),
        "```",
        "",
        "## Stages",
        "",
    ...
    md_lines += [
        ...
        "",
        "> TASK-014AJ is a STRICT MANUAL-AUTHORIZATION-DESIGN-ONLY",
        ...
        "> request, NEVER validates the token (only documents the",
        "> pattern), NEVER treats any token as authorization, NEVER",
        "> includes entry / stop-attach / cleanup execution, NEVER",
        "> lifts TASK-014L sender G20 (protected_entry_policy_missing).",
        "> The 5 existing demo positions are NEVER touched.",
        "> Even with --allow-design-approval the design only emits a",
        "> design artifact (readiness_conclusion remains",
        "> DESIGN_REVIEW_READY_NOT_EXECUTABLE).",
        "> Even with --allow-real-entry-execution this task returns",
        "> REAL_ENTRY_EXECUTION_NOT_IMPLEMENTED (no socket opened).",
        "",
    ]
    md_text = "\n".join(md_lines)
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("AUTHORIZATION DESIGN APPROVAL --- NO NETWORK --- design only")
    else:
        print("AUTHORIZATION DESIGN CHECKLIST --- NO NETWORK --- design only")
    print("TASK-014AJ: Guarded Tiny Entry Manual Authorization Design")
    print(_SEP)
    ...
    design = DemoTinyGuardedEntryManualAuthorizationDesign()
    result = design.run_checklist(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tiny guarded entry MANUAL AUTHORIZATION DESIGN. "
            ...
            "guarded entry sender real execution, no token validation, "
            "no token-to-authorization mapping.  Even with "
            "--allow-design-approval this only emits a design artifact; "
    ...
    parser.add_argument("--symbol", default=DEFAULT_SELECTED_SYMBOL,
                        metavar="SYMBOL",
                        help=("Symbol to design authorization against.  MUST "
                              "equal SOLUSDT (post-entry symbol).  MUST NOT "
                              "be in the 5 existing demo position symbols "
                              "(ENAUSDT / TIAUSDT / AIXBTUSDT / POLYXUSDT / "
                              "EDUUSDT)."))
    parser.add_argument("--allow-design-approval", action="store_true",
                        help=("Promote envelope to manual_authorization_design_approval. "
                              "Even with this flag, TASK-014AJ only emits a "
                              "manual-authorization-design artifact "
                              "(readiness_conclusion stays "
                              "DESIGN_REVIEW_READY_NOT_EXECUTABLE).  "
                              "No execution, no implementation, no token "
                              "validation."))
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AJ "
    ...
    sys.exit(run_execute(
        symbol=args.symbol,
        allow_design_approval=args.allow_design_approval,
