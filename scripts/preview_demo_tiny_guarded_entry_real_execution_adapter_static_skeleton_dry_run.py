...
Usage (STATIC SKELETON DRY-RUN CHECKLIST --- default, no network,
       no implementation, no execution):
  python scripts/preview_demo_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.py \\
...
Usage (STATIC SKELETON DRY-RUN APPROVAL --- dry-run-only, no execution):
...
Writes (when --write-report):
  outputs/demo_trading/tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run/
      {ts}_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.json
      {ts}_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.md
      latest_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.json
      latest_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.md

IMPORTANT:
  - TASK-014AS produces a STRICT STATIC-SKELETON-DRY-RUN-ONLY artifact.
    ...
    implementation, NO adapter `send` method.  Static skeleton
    dry-run for the FUTURE TASK-014AT adapter disabled implementation
    scaffold design
...
Exit codes:
  0  tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run_ready /
     tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run_ready_but_execution_disabled /
...
def _print_result(
    r: TinyGuardedEntryRealExecutionAdapterStaticSkeletonDryRunResult,
) -> None:
...
def _write_report(
    ...
    md_lines: list[str] = [
        "# Tiny Guarded Entry Real Execution Adapter Static Skeleton Dry-run (TASK-014AS)",
        "",
        "_TASK-014AS consumes TASK-014AR static skeleton design output at "
        "runtime and produces a static skeleton dry-run for TASK-014AT. "
        ...
        f"static_skeleton_dry_run_conclusion: `{r.implementation_design_conclusion}`  ",
        f"static_skeleton_dry_run_authorization_result: `{r.implementation_design_authorization_result}`  ",
        ...
        "",
        "## Static Skeleton Dry-run Verdict",
        "",
        "| field | value |",
        "|---|---|",
        ...
        "```",
        "",
        "## Final Static Skeleton Dry-run Verdict",
        "",
        "```json",
...
def run_execute(
    ...
    if allow_real_entry_execution:
        ...
        print("STATIC SKELETON DRY-RUN APPROVAL --- NO NETWORK --- dry-run-only")
    else:
        print("STATIC SKELETON DRY-RUN CHECKLIST --- NO NETWORK --- dry-run-only")
    print("TASK-014AS: Guarded Entry Real Execution Adapter Static Skeleton Dry-run")
    print("  consumes TASK-014AR static skeleton design output -> produces static skeleton dry-run for TASK-014AT")
    ...
    gate = DemoTinyGuardedEntryRealExecutionAdapterStaticSkeletonDryRun()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "DRY-RUN (TASK-014AS).  Consumes TASK-014AR static skeleton "
            "design output at runtime and produces a static skeleton "
            "dry-run for TASK-014AT.  No network, no live endpoint, no orders / "
            ...
            "git commit, no automatic git push.  Static skeleton "
            "dry-run for the FUTURE TASK-014AT adapter disabled "
            "implementation scaffold design.  Even with "
            "--allow-static-skeleton-dry-run "
            "this only emits a dry-run artifact; even with "
    ...
    parser.add_argument("--expected-commit-hash", default="",
                        ...
                        help=("Optional expected commit hash to record in the "
                              ...
                              "git commit or push)."))
    parser.add_argument("--allow-static-skeleton-dry-run",
                        dest="allow_implementation_design",
                        action="store_true",
                        help=("Promote envelope to static_skeleton_dry_run_approval. "
                              "Even with this flag, TASK-014AS only emits a "
                              "static-skeleton dry-run artifact "
                              "(static_skeleton_dry_run_conclusion stays "
    ...
    parser.add_argument("--allow-real-entry-execution", action="store_true",
                        help=("Guarded flag for a hypothetical future real "
                              "tiny entry sender execution.  TASK-014AS "
