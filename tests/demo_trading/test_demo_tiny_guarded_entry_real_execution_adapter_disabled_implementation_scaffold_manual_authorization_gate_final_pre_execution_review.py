...
TASK-014BA: Guarded Tiny Entry Real Execution Adapter Implementation
            Design tests.
...
from src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
...
SRC_PATH = (
    ROOT_PATH / "src"
    / "demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
)
PREVIEW_PATH = (
    ROOT_PATH / "scripts"
    / "preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
)
...
def _valid_entry_disabled_implementation_scaffold_manual_authorization_gate_dry_run() -> dict:
    ...
    *dry-run* artifact (the AY direct upstream of TASK-014BA).  AZ consumes
...
class TestAQ1ImplementationDesignReady:
    def test_checklist_solusdt(self):
        ...
        from src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import _HARD_FAIL_GATES
...
class TestAQ2ImplementationDesignApproval:
    def test_approval_yields_exec_disabled(self):
        ...
        assert r.real_execution_allowed is False
        assert r.real_entry_implemented is False
        assert r.implementation_design_conclusion == DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CONCLUSION
        assert r.g20_lifted is False
...
class TestAQ79AdapterContractIdentity:
    def test_adapter_name_and_versions(self):
        ...
        assert ADAPTER_CONTRACT_VERSION == "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_v1"
        ...
        assert ADAPTER_RESPONSE_STATUS == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_NOT_SENT"
        assert ORDER_LINK_ID_PREFIX == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_TINY_ENTRY_"


# ===========================================================================
# AQ80: next_required_task points at TASK-014BA
# ===========================================================================
...
class TestAQ80NextRequiredTask:
    def test_next_required_task(self):
        r = _run()
        assert r.next_required_task == NEXT_REQUIRED_TASK
        assert NEXT_REQUIRED_TASK == "TASK-014BB_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_manual_authorization_review"
...
class TestAQ86DesignExpectedValues:
    def test_design_expected_constants(self):
        ...
        assert ORDER_LINK_ID_PREFIX.startswith("DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_TINY_ENTRY")
...
class TestAQ94FinalVerdictCompleteness:
    def test_verdict_contains_required_fields(self):
        ...
        assert v["implementation_design_conclusion"] == DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CONCLUSION
        assert v["implementation_design_authorization_result"] == DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_AUTHORIZATION_RESULT
...
# ===========================================================================
# AQ96: Documentation sync review names next task TASK-014BA
# ===========================================================================
...
class TestAQ98OrderLinkIdPrefix:
    def test_order_link_id_prefix_starts_with_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review(self):
        assert ORDER_LINK_ID_PREFIX.startswith("DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_TINY_ENTRY")
        r = _run()
        assert r.order_link_id_prefix == ORDER_LINK_ID_PREFIX
...
class TestAQ120ImplementationDesignConclusion:
    def test_conclusion_constant_value(self):
        assert DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CONCLUSION == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE"
        r = _run()
        assert r.implementation_design_conclusion == DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CONCLUSION

    def test_authorization_result_documented_only(self):
        assert DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_AUTHORIZATION_RESULT == "DOCUMENTED_ONLY_NOT_AUTHORIZED"
        r = _run()
        assert r.implementation_design_authorization_result == DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_AUTHORIZATION_RESULT
...
# (TASK-014BA forward-declarations consumed by TASK-014BA dry-run)
# ===========================================================================
...
class TestAR124AQAcceptanceGateIdentifiersDeclared:
    ...
    static-skeleton-design module so that TASK-014BA can wire them into the
...
# ===========================================================================
# AR-FIX1: TASK-014BA runtime-consumes TASK-014AQ implementation design output
...
# ---------------------------------------------------------------------------
# These tests verify that the report-facing labels emitted by TASK-014BA
...
class TestARFIX2ModeStringIsStaticSkeleton:
    def test_default_mode_is_static_skeleton_checklist(self):
        r = _run(symbol="SOLUSDT")
        assert r.mode == "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_checklist"
        assert r.mode == MODE_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CHECKLIST

    def test_legacy_mode_alias_still_resolves(self):
        # MODE_IMPLEMENTATION_DESIGN_CHECKLIST is a back-compat alias
        # that must equal the disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_checklist string.
        assert MODE_IMPLEMENTATION_DESIGN_CHECKLIST \
            == MODE_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CHECKLIST \
            == "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_checklist"

    def test_approval_mode_is_static_skeleton_approval(self):
        r = _run(symbol="SOLUSDT", allow_implementation_design=True)
        assert r.mode == "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_approval"
        assert r.mode == MODE_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_APPROVAL
        assert MODE_IMPLEMENTATION_DESIGN_APPROVAL \
            == MODE_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_APPROVAL


class TestARFIX2StatusUnchanged:
    def test_status_remains_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_ready(self):
        r = _run(symbol="SOLUSDT")
        assert r.status \
...
class TestARFIX2StaticSkeletonConclusionAlias:
    def test_to_dict_exposes_static_skeleton_conclusion_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert d["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_conclusion"] \
        ...
        assert d["implementation_design_conclusion"] \
            == d["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_conclusion"]
...
class TestARFIX2StaticSkeletonAuthorizationResultAlias:
    def test_to_dict_exposes_static_skeleton_authorization_result_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert d["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_authorization_result"] \
            == "DOCUMENTED_ONLY_NOT_AUTHORIZED"
        assert d["implementation_design_authorization_result"] \
            == d["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_authorization_result"]
...
class TestARFIX2StaticSkeletonScopeAlias:
    def test_to_dict_exposes_static_skeleton_scope_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_scope" in d
...
class TestARFIX2FinalStaticSkeletonVerdictAlias:
    def test_to_dict_exposes_final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert "final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict" in d
        assert d["final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict"] \
            == d["final_implementation_design_verdict"]
        verdict = d["final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict"]
        assert verdict["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_conclusion"] \
            == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE"
        assert verdict["implementation_design_conclusion"] \
            == verdict["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_conclusion"]
...
class TestARFIX2AuditArtifactsHaveStaticSkeletonAliases:
    def test_audit_artifacts_contain_static_skeleton_aliases(self):
        r = _run(symbol="SOLUSDT")
        a = r.audit_artifacts
        assert "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_scope" in a
        assert "final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict" in a
        assert a["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_scope"] \
            == a["implementation_design_scope"]
        assert a["final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict"] \
...
class TestARFIX2Stage1SummaryUsesStaticSkeleton:
    def test_stage_1_summary_mentions_static_skeleton(self):
        ...
        assert "disabled implementation scaffold manual authorization gate final pre-execution review scope" in s1["summary"]
        # Legacy implementation_design_scope key still present.
        assert "implementation_design_scope" in s1
        # New alias also present.
        assert "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_scope" in s1
...
class TestARFIX2Stage13SummaryUsesStaticSkeleton:
    def test_stage_13_summary_mentions_static_skeleton(self):
        ...
        assert "disabled implementation scaffold manual authorization gate final pre-execution review verdict" in s13["summary"]
        assert "final_implementation_design_verdict" in s13
        assert "final_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_verdict" in s13
        assert s13["disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_conclusion"] \
            == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE"
...
class TestARFIX2MarkdownReportTitleAndSections:
    def test_markdown_report_uses_static_skeleton_wording(self, repo_tmp_path):
        ...
        # Title must say Disabled Implementation Scaffold Manual Authorization Gate Final Pre-Execution Review, not the
        # legacy Static Skeleton Dry-run / Implementation Design wording.
        assert "# Tiny Guarded Entry Real Execution Adapter Disabled Implementation Scaffold Manual Authorization Gate Final Pre-Execution Review (TASK-014BA)" in md
        # Section headers updated to disabled implementation scaffold manual authorization gate final pre-execution review wording.
        assert "## Disabled Implementation Scaffold Manual Authorization Gate Final Pre-Execution Review Verdict" in md
        assert "## Disabled Implementation Scaffold Manual Authorization Gate Final Pre-Execution Review Scope" in md
        assert "## Final Disabled Implementation Scaffold Manual Authorization Gate Final Pre-Execution Review Verdict" in md
        # Disabled implementation scaffold design conclusion label appears at
        # the top of the report.
        assert "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_conclusion:" in md
        # Mode shown in the report is the disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_checklist
        # string.
        assert "mode: `disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_checklist`" in md
        # TASK-014AY appears in the Markdown (AY is AZ's direct upstream).
        assert "TASK-014AY" in md
        assert "TASK-014BA" in md
        # TASK-014BA appears in the Markdown as the future consumer.
        assert "TASK-014BA" in md
    ...
    def test_markdown_report_footer_uses_readiness_review_wording(self, repo_tmp_path):
        ...
        latest_md = out_dir / f"latest_{base}.md"
        md = latest_md.read_text(encoding="utf-8")
        # Footer blockquote must say DISABLED-IMPLEMENTATION-SCAFFOLD-MANUAL-AUTHORIZATION-GATE-FINAL-PRE-EXECUTION-REVIEW-ONLY
        # (NOT ...DESIGN-ONLY - that is AX's identity, not AY's).
        assert "STRICT DISABLED-IMPLEMENTATION-SCAFFOLD-MANUAL-AUTHORIZATION-GATE-FINAL-PRE-EXECUTION-REVIEW-ONLY module" in md
...
class TestARFIX2CLIBannerSaysStaticSkeleton:
    def test_cli_help_does_not_advertise_implementation_design_only(self):
        ...
        # tokens such as DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE
        ...
        # IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE FINAL PRE-EXECUTION REVIEW (TASK-014BA),
        # consuming TASK-014AX manual authorization gate design and producing for TASK-014BA.
...
class TestASFIX2ResponseStatusLabels:
    """Gate label and stage-6 summary use DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_NOT_SENT."""

    def test_blocked_gates_contains_dry_run_response_status_gate(self):
        r = _run()
        assert "response_status_is_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_not_sent" in r.blocked_gates
    ...
    def test_stage6_summary_uses_dry_run_response_status(self):
        ...
        assert "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_NOT_SENT" in summary
    ...
    def test_markdown_report_response_status_uses_dry_run_wording(self, repo_tmp_path):
        ...
        assert "response_status=DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_NOT_SENT" in md
        assert "response_status=IMPLEMENTATION_DESIGN_NOT_SENT" not in md
        # Blocked-gates section must use the new gate label.
        assert "response_status_is_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review_not_sent" in md
...
# ============================================================================
# TASK-014BA runtime consumption of TASK-014AS (static skeleton dry-run)
...
# ============================================================================
# TASK-014BA runtime consumption of TASK-014AT
...
class TestAUATFIX1ReportProof:
    """TASK-014BA-FIX1: upstream report proof and authorization_result correctness."""
...
class TestAUATFIX2AuthorizationResultReport:
    """TASK-014BA-FIX2: upstream authorization_result propagates through all report surfaces."""
...
# ===========================================================================
# TASK-014BA: TestAXAW* — AW (DISABLED IMPLEMENTATION SCAFFOLD FINAL
...
# ===========================================================================
# TASK-014BA-FIX1: TestAXAWFIX1ReportProof —
...
# upstream and TASK-014BA (not TASK-014BA) as the forward-ref.
# ===========================================================================

class TestAXAWFIX1ReportProof:
    """TASK-014BA-FIX1: AV upstream proof surfaces in all report outputs."""
...
# ===========================================================================
# TASK-014BA-FIX1: TestAXFIX1OlderUpstreamPath —
# Regression guard: the broad rename that turned `final_pre_execution_review`
# into `manual_authorization_gate_readiness_review` in Stage 1 over-renamed the OLDER
...
class TestAXFIX1OlderUpstreamPath:
    ...
    def test_cli_help_does_not_mention_manual_authorization_gate_readiness_review_path(self):
        normalized = self._help_normalized()
        assert "tiny_guarded_entry_manual_authorization_gate_readiness_review" not in normalized

    def test_cli_flag_manual_authorization_gate_readiness_review_does_not_exist(self):
        normalized = self._help_normalized()
        assert "--from-latest-entry-manual-authorization-gate-readiness-review" not in normalized
    ...
    def test_gate_constant_is_final_pre_execution_review_not_manual_auth_gate_design(self):
        ...
        assert "manual_authorization_gate_readiness_review" not in GATE_ENTRY_FINAL_PRE_EXECUTION_REVIEW_MISSING
    ...
    def test_acceptable_statuses_frozenset_uses_final_pre_execution_review(self):
        ...
        for s in ACCEPTABLE_ENTRY_FINAL_PRE_EXECUTION_REVIEW_STATUSES:
            assert "MANUAL_AUTHORIZATION_GATE_READINESS_REVIEW" not in s

    def test_report_does_not_mention_manual_authorization_gate_readiness_review_dir(self, repo_tmp_path):
        from scripts.preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
            _write_report,
        )
        r = _run(symbol="SOLUSDT")
        ...
        assert "tiny_guarded_entry_manual_authorization_gate_readiness_review" not in json_text
        assert "tiny_guarded_entry_manual_authorization_gate_readiness_review" not in md_text


# ===========================================================================
# TASK-014BA-FIX1: AX-as-34th-upstream structural mirror + simulated approval
...
class TestAYAXFIX1CLIFlags:
    def test_preview_script_advertises_axmag_flag(self):
        path = ROOT / "scripts" / "preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
    ...
    def test_preview_script_loader_function_exposed(self):
        from scripts import preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
    ...
    def test_preview_script_help_runs_clean(self):
        script = ROOT / "scripts" / "preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
        proc = subprocess.run(
...
class TestAYAXFIX1SafetyInvariants:
    ...
    def test_src_does_not_introduce_executable_adapter_or_sender(self):
        path = ROOT / "src" / "demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
        text = path.read_text(encoding="utf-8")
...
class TestAYFIX2FailClosedEnforcement:
    ...
    def test_violation_dominates_allow_disabled_implementation_scaffold_flag(self):
        # When --allow-disabled-implementation-scaffold-manual-authorization-gate-readiness-review is set
...
class TestAYFIX3WordingCorrection:
    def test_preview_script_banner_says_dry_run_checklist(self):
        ...
        text = PREVIEW_PATH.read_text(encoding="utf-8")
        assert "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE FINAL PRE-EXECUTION REVIEW CHECKLIST" in text
    ...
    def test_preview_script_approval_banner_says_dry_run(self):
        ...
        text = PREVIEW_PATH.read_text(encoding="utf-8")
        assert "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE FINAL PRE-EXECUTION REVIEW APPROVAL" in text
    ...
    def test_stage0_summary_mentions_34_upstream_already_consumed_by_ay(self):
        ...
        (not that AZ validates 34 upstream artifacts ending at AX directly).""";
        r = _run()
        summary = r.stages[STAGE_0_ARTIFACT_PREFLIGHT]["summary"]
        assert "34 upstream artifacts AY already consumed" in summary

    def test_stage0_summary_does_not_claim_ax_as_direct_acceptance_target(self):
        """AZ does not validate AX directly; its direct upstream is AY."""
        r = _run()
        summary = r.stages[STAGE_0_ARTIFACT_PREFLIGHT]["summary"]
        assert "AX acceptance flags" not in summary

    def test_src_docstring_mentions_35_upstream_artifacts(self):
        """Src module docstring must describe 35 inputs (AY direct + 34 AY-consumed)."""
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        doc = mod.__doc__ or ""
        assert "35 upstream artifacts" in doc


# ===========================================================================
# TASK-014BA: AZ-on-AY direct-upstream proof
#
# AZ consumes AY (DRY-RUN) as the newest direct upstream. AX/AW/AV/AU/AT/AS/AR/AQ
# are chained proof through AY, not the direct artifact.
# ===========================================================================

class TestAZIdentityReadinessReviewOnly:
    """AZ identity must say FINAL-PRE-EXECUTION-REVIEW-ONLY everywhere, not DRY-RUN-ONLY or DESIGN-ONLY."""

    def test_src_docstring_identity_readiness_review(self):
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        doc = mod.__doc__ or ""
        assert "manual-authorization-gate-final-pre-execution-review-only" in doc.lower()
        assert "manual-authorization-gate-dry-run-only module" not in doc.lower()
        assert "manual-authorization-gate-design-only module" not in doc.lower()

    def test_src_status_constant_is_readiness_review(self):
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        assert mod.STATUS_IMPLEMENTATION_DESIGN_READY == (
            "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_"
            "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY"
        )

    def test_src_conclusion_constant_is_readiness_review(self):
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        assert mod.DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_CONCLUSION == (
            "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE"
        )

    def test_src_next_required_task_is_bb_final_pre_execution_review_manual_authorization_review(self):
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        assert mod.NEXT_REQUIRED_TASK == (
...
class TestAZAYDirectUpstreamLoader:
    """AZ has a loader function + CLI flag for the AY dry-run artifact."""

    def test_loader_function_exists(self):
        from scripts.preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
            load_latest_entry_disabled_implementation_scaffold_manual_authorization_gate_dry_run,
        )
        assert callable(load_latest_entry_disabled_implementation_scaffold_manual_authorization_gate_dry_run)

    def test_default_dry_run_dir_constant_points_to_ay_outputs(self):
        from scripts.preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
            _DEFAULT_ENTRY_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_DRY_RUN_DIR,
        )
        s = str(_DEFAULT_ENTRY_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_DRY_RUN_DIR)
        assert "tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_dry_run" in s

    def test_loader_reads_ay_artifact_filename(self):
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as td:
            ay_dir = _pl.Path(td) / "ay_dir"
            ay_dir.mkdir()
            target = ay_dir / "latest_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_dry_run.json"
            target.write_text(
                '{"status":"TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_DRY_RUN_READY"}',
                encoding="utf-8",
            )
            loaded = load_latest_entry_disabled_implementation_scaffold_manual_authorization_gate_dry_run(ay_dir)
            assert loaded is not None
            assert loaded["status"] == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_DRY_RUN_READY"

    def test_loader_returns_none_when_missing(self):
        import tempfile
        import pathlib as _pl
        from scripts.preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
            load_latest_entry_disabled_implementation_scaffold_manual_authorization_gate_dry_run,
        ...
        with tempfile.TemporaryDirectory() as td:
            assert load_latest_entry_disabled_implementation_scaffold_manual_authorization_gate_dry_run(_pl.Path(td)) is None

    def test_cli_help_advertises_ay_dry_run_flag(self):
        import os as _os
        env = dict(_os.environ)
        env["COLUMNS"] = "400"
        result = subprocess.run(
        ...
        assert "--from-latest-entry-disabled-implementation-scaffold-manual-authorization-gate-dry-run" in normalized
        assert "TASK-014AY" in normalized

    def test_cli_help_mentions_readiness_review(self):
        import os as _os
        env = dict(_os.environ)
        env["COLUMNS"] = "400"
        result = subprocess.run(
        ...
        out = result.stdout + result.stderr
        assert "readiness review" in out.lower() or "Final Pre-Execution Review" in out


class TestAZAYUpstreamProofWordingInReport:
    """Generated report must name AY (not AX) as direct upstream."""

    def test_markdown_intro_names_ay_dry_run_not_ax_design(self, repo_tmp_path):
        from scripts.preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
            _write_report,
        )
        r = _run(symbol="SOLUSDT")
    ...
    def test_markdown_title_says_readiness_review_not_dry_run(self, repo_tmp_path):
        from scripts.preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review import (
            _write_report,
        )
        r = _run(symbol="SOLUSDT")
        ...
        assert "Final Pre-Execution Review" in first_title
...
class TestAZSafetyInvariantsAfterRename:
    """Safety invariants must remain forbidden after the AY->AZ rename."""

    def test_real_execution_disallowed_default(self):
        r = _run()
        assert r.real_execution_allowed is False
        assert r.send_allowed is False
        assert r.order_endpoint_called is False
    ...
    def test_real_execution_disallowed_under_allow_real_entry_execution(self):
        r = _run(allow_real_entry_execution=True)
        assert r.real_execution_allowed is False
        assert r.send_allowed is False
        assert r.order_endpoint_called is False
...
class TestAZForbiddenChangesToCoreModules:
    """main.py / src/risk.py / BybitExecutor are untouched."""

    def test_main_py_not_imported_by_az_src(self):
        import re as _re
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        src_text = open(mod.__file__, "r", encoding="utf-8").read()
        code_lines = [
            ln for ln in src_text.splitlines()
            if _re.match(r"^\s*(import|from)\s+\S", ln)
        ]
        joined = "\n".join(code_lines)
        assert not _re.search(r"^\s*import\s+main\b", joined, _re.M)
        assert not _re.search(r"^\s*from\s+main\b", joined, _re.M)
        assert not _re.search(r"^\s*(import|from)\s+src\.risk\b", joined, _re.M)
        assert not _re.search(r"^\s*(import|from)\s+\S*BybitExecutor", joined, _re.M)
        assert "BybitExecutor" not in joined

    def test_no_network_imports_in_az_src(self):
        import re as _re
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as mod
        src_text = open(mod.__file__, "r", encoding="utf-8").read()
        code_lines = [
        ...
        for forbidden in ("urllib", "requests", "httpx",
                          "socket", "http.client", "hmac",
                          "dotenv", "pybit"):
            assert not _re.search(
                rf"^\s*(import\s+{_re.escape(forbidden)}\b|from\s+{_re.escape(forbidden)}\b)",
                joined, _re.M,
            ), f"forbidden network import found: {forbidden}"


class TestAZAYFIX2HardFailGateMembershipProof:
    """Static-source proof that AY's FIX2 wired the AX-upstream + simulated-approval gates into _HARD_FAIL_GATES."""

    def _ay_src_text(self) -> str:
        ay_path = (
            ROOT_PATH / "src" /
            "demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_dry_run.py"
        )
        return ay_path.read_text(encoding="utf-8")

    def test_ay_src_declares_hard_fail_gates_frozenset(self):
        assert "_HARD_FAIL_GATES" in self._ay_src_text()

    def test_ay_src_includes_ax_upstream_gates_in_hard_fail_set(self):
        txt = self._ay_src_text()
        for tail in (
            "MISSING",
            "STATUS_UNACCEPTABLE",
            "MODE_UNACCEPTABLE",
            "REAL_EXECUTION_ALLOWED_TRUE",
            "SEND_ALLOWED_TRUE",
            "ADAPTER_IMPLEMENTATION_INCLUDED_TRUE",
            "ADAPTER_EXECUTION_INCLUDED_TRUE",
            "ORDER_ENDPOINT_CALLED_TRUE",
            "STOP_ENDPOINT_CALLED_TRUE",
            "NO_POSITION_MODIFIED_FALSE",
            "NO_SECRETS_LOADED_FALSE",
            "G20_LIFTED_TRUE",
            "CONCLUSION_MISMATCH",
            "RESPONSE_STATUS_UNACCEPTABLE",
            "NEXT_TASK_MISMATCH",
        ):
            gate = f"GATE_ENTRY_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_DESIGN_{tail}"
            assert gate in txt, f"AY src must declare gate {gate}"

    def test_ay_src_includes_simulated_approval_gates_in_hard_fail_set(self):
        txt = self._ay_src_text()
        for tail in (
            "MISSING",
            "AMBIGUOUS",
            ...
            "CONTAINS_SIGNATURE_LIKE_VALUE",
            "AUTO_TRIGGERS_SENDER",
            "GRANTS_EXECUTION",
        ):
            gate = f"GATE_SIMULATED_APPROVAL_{tail}"
            assert gate in txt, f"AY src must declare simulated-approval gate {gate}"

    def test_ay_tests_assert_status_fail_closed_for_fix2_gates(self):
        ay_test_path = (
            ROOT_PATH / "tests" / "demo_trading" /
            "test_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_dry_run.py"
        )
        txt = ay_test_path.read_text(encoding="utf-8")
        assert "TestAYFIX2FailClosedEnforcement" in txt
        assert "STATUS_FAIL_CLOSED" in txt


class TestAZAYFIX3WordingProof:
    """Static-source proof that AY's FIX3 wording corrections are present."""

    def _ay_scripts_text(self) -> str:
        ay_scripts_path = (
            ROOT_PATH / "scripts" /
        ...
        return ay_scripts_path.read_text(encoding="utf-8")
    ...
    def test_ay_preview_banner_says_dry_run_checklist(self):
        assert "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE DRY-RUN CHECKLIST" in self._ay_scripts_text()

    def test_ay_preview_banner_does_not_say_design_checklist(self):
        assert "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE DESIGN CHECKLIST" not in self._ay_scripts_text()

    def test_ay_preview_approval_banner_says_dry_run_approval(self):
        assert "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE DRY-RUN APPROVAL" in self._ay_scripts_text()

    def test_ay_preview_approval_banner_does_not_say_design_approval(self):
        assert "DISABLED IMPLEMENTATION SCAFFOLD MANUAL AUTHORIZATION GATE DESIGN APPROVAL" not in self._ay_scripts_text()

    def test_ay_src_docstring_says_34_upstream(self):
        assert "34 upstream artifacts" in self._ay_src_text()


class TestAZChainedProofThroughAYStillIntact:
    """The chained AX/AW/AV/AU/AT/AS/AR/AQ proof fields remain populated through AY."""

    def test_chained_ax_design_fields_populated(self):
        r = _run()
        assert r.upstream_entry_disabled_implementation_scaffold_manual_authorization_gate_design_status \
            == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_DESIGN_READY"
        assert r.upstream_entry_disabled_implementation_scaffold_manual_authorization_gate_design_authorization_result \
    ...
    def test_chained_aw_final_pre_execution_review_fields_populated(self):
        r = _run()
        assert r.upstream_entry_disabled_implementation_scaffold_final_pre_execution_review_status \
            == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_DISABLED_IMPLEMENTATION_SCAFFOLD_FINAL_PRE_EXECUTION_REVIEW_READY"

    def test_chained_implementation_readiness_review_fields_populated(self):
        r = _run()
        assert r.upstream_entry_implementation_readiness_review_status \
            == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_IMPLEMENTATION_READINESS_REVIEW_READY"

    def test_chained_au_dry_run_fields_populated(self):
        r = _run()
        assert r.upstream_entry_disabled_implementation_scaffold_dry_run_status \
            == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_DISABLED_IMPLEMENTATION_SCAFFOLD_DRY_RUN_READY"

    def test_chained_at_design_fields_populated(self):
        r = _run()
        assert r.upstream_entry_disabled_implementation_scaffold_design_status \
            == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_DISABLED_IMPLEMENTATION_SCAFFOLD_DESIGN_READY"

    def test_chained_aq_implementation_design_fields_populated(self):
        r = _run()
        assert r.upstream_entry_implementation_design_status \
            == "TINY_GUARDED_ENTRY_REAL_EXECUTION_ADAPTER_IMPLEMENTATION_DESIGN_READY"


class TestAZNoApprovalTokenValidatedAsAuthorization:
    """AZ must never validate any phrase/token/input as executable authorization."""

    def test_response_status_is_readiness_review_not_sent(self):
        import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as M
        assert M.ADAPTER_RESPONSE_STATUS == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_NOT_SENT"

    def test_authorization_result_is_documented_only_not_authorized(self):
        r = _run()
        assert r.implementation_design_authorization_result == "DOCUMENTED_ONLY_NOT_AUTHORIZED"

    def test_conclusion_is_readiness_review_ready_not_executable(self):
        r = _run()
        assert r.implementation_design_conclusion == "DISABLED_IMPLEMENTATION_SCAFFOLD_MANUAL_AUTHORIZATION_GATE_FINAL_PRE_EXECUTION_REVIEW_READY_NOT_EXECUTABLE"


# ===========================================================================
# TASK-014BA-FIX1: AY (manual-authorization-gate-dry-run) direct upstream
...
# ===========================================================================
import src.demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as _AZM
...
class TestAZFIX1ScriptsLoaderWiring:
    ...
    def test_loader_function_exists(self):
        from scripts import preview_demo_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review as _S
...
class TestAZFIX1ReportFieldsExposed:
    ...
    def test_write_report_emits_aydr_proof_rows(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            md = (out / "latest_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.md").read_text(encoding="utf-8")
            ...
            jp = out / "latest_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.json"
...
# ===========================================================================
# TASK-014BA-FIX2: AX-design next_required_task expectation must be the
...
class TestAZFIX2ReportHappyPath:
    ...
    def _write_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            md = (out / "latest_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.md").read_text(encoding="utf-8")
            payload = _json.loads(
                (out / "latest_tiny_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.json").read_text(encoding="utf-8")
            )
            return md, payload
...
class TestAZFIX2SourceLevelChainLiterals:
    ...
    def test_ba_src_forward_ref_is_bb_final_pre_execution_review_manual_authorization_review(self):
        """BA final-pre-exec-review's own NEXT_REQUIRED_TASK literal must be the BB manual-authorization-review task."""
        s = self._src(
            "demo_tiny_guarded_entry_real_execution_adapter_"
            "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
        )
        ba_expected = (
            "TASK-014BB_guarded_entry_real_execution_adapter_"
            "disabled_implementation_scaffold_manual_authorization_gate_"
    ...
    def test_az_src_expects_ax_next_task_as_ay_dry_run_literal(self):
        ...
        # (the AZ-FIX2 fix replaced the readiness_review literal with dry_run).
        _AX_AZ_WRONG_FOR_AZ_SRC = (
            "TASK-014AY_guarded_entry_real_execution_adapter_"
            "disabled_implementation_scaffold_manual_authorization_gate_readiness_review"
        )
        assert _AX_AZ_WRONG_FOR_AZ_SRC not in s

    def test_ba_src_expects_ay_next_task_as_az_readiness_review_literal(self):
        """BA's chained AY-dry-run NEXT_TASK_MISMATCH check must compare AY's
        next_required_task against the AZ readiness-review literal."""
        s = self._src(
            "demo_tiny_guarded_entry_real_execution_adapter_"
            "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review.py"
        )
        ...
        assert ay_expected_in_ba in s
        # The bulk-rename contamination literal (BA self) must NOT be the AY expectation in BA src
        _AY_BA_WRONG_FROM_BULK_RENAME = (
            "TASK-014BA_guarded_entry_real_execution_adapter_"
            "disabled_implementation_scaffold_manual_authorization_gate_final_pre_execution_review"
        )
        # That literal IS allowed elsewhere in BA src (e.g. AY chained-fixtures, banner labels),
        # but it must not appear as the comparison RHS for `entry_aydr_next_required_task !=`.
        import re
        bad_pattern = re.compile(
            r'entry_aydr_next_required_task\s*!=\s*"' + re.escape(_AY_BA_WRONG_FROM_BULK_RENAME) + r'"'
        )
