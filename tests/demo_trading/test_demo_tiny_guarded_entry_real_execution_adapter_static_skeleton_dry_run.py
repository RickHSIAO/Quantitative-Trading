...
TASK-014AS: Guarded Tiny Entry Real Execution Adapter Implementation
            Design tests.
...
from src.demo_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run import (
    ...
    DemoTinyGuardedEntryRealExecutionAdapterStaticSkeletonDryRun,
    ENDPOINT_PATH_REF,
    ...
    STATIC_SKELETON_DRY_RUN_AUTHORIZATION_RESULT,
    ...
    MODE_STATIC_SKELETON_DRY_RUN_APPROVAL,
    MODE_STATIC_SKELETON_DRY_RUN_CHECKLIST,
    ...
    TinyGuardedEntryRealExecutionAdapterStaticSkeletonDryRunResult,
...
SRC_PATH = (
    ROOT_PATH / "src"
    / "demo_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.py"
)
PREVIEW_PATH = (
    ROOT_PATH / "scripts"
    / "preview_demo_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run.py"
)
...
def _valid_entry_implementation_readiness_review() -> dict:
    ...
    return {
        ...
        "next_required_task":           "TASK-014AS_guarded_entry_real_execution_adapter_implementation_design",
    }
...
def _valid_entry_static_skeleton_design() -> dict:
    """The 28th (newest) upstream artifact: TASK-014AR's output."""
    return {
...
def _design() -> DemoTinyGuardedEntryRealExecutionAdapterStaticSkeletonDryRun:
    return DemoTinyGuardedEntryRealExecutionAdapterStaticSkeletonDryRun()
...
class TestAQ1ImplementationDesignReady:
    def test_checklist_solusdt(self):
        ...
        from src.demo_tiny_guarded_entry_real_execution_adapter_static_skeleton_dry_run import _HARD_FAIL_GATES
...
class TestAQ2ImplementationDesignApproval:
    def test_approval_yields_exec_disabled(self):
        ...
        assert r.real_execution_allowed is False
        assert r.real_entry_implemented is False
        assert r.implementation_design_conclusion == STATIC_SKELETON_DRY_RUN_CONCLUSION
        assert r.g20_lifted is False
...
class TestAQ79AdapterContractIdentity:
    def test_adapter_name_and_versions(self):
        ...
        assert ADAPTER_CONTRACT_VERSION == "static_skeleton_dry_run_v1"
        ...
        assert ADAPTER_RESPONSE_STATUS == "STATIC_SKELETON_DRY_RUN_NOT_SENT"
        assert ORDER_LINK_ID_PREFIX == "STATIC_SKELETON_DRY_RUN_TINY_ENTRY_"


# ===========================================================================
# AQ80: next_required_task points at TASK-014AS
# ===========================================================================
...
class TestAQ80NextRequiredTask:
    def test_next_required_task(self):
        r = _run()
        assert r.next_required_task == NEXT_REQUIRED_TASK
        assert NEXT_REQUIRED_TASK == "TASK-014AT_guarded_entry_real_execution_adapter_disabled_implementation_scaffold_design"
...
class TestAQ86DesignExpectedValues:
    def test_design_expected_constants(self):
        ...
        assert ORDER_LINK_ID_PREFIX.startswith("STATIC_SKELETON_DRY_RUN_TINY_ENTRY")
...
class TestAQ94FinalVerdictCompleteness:
    def test_verdict_contains_required_fields(self):
        ...
        assert v["implementation_design_conclusion"] == STATIC_SKELETON_DRY_RUN_CONCLUSION
        assert v["implementation_design_authorization_result"] == STATIC_SKELETON_DRY_RUN_AUTHORIZATION_RESULT
...
# ===========================================================================
# AQ96: Documentation sync review names next task TASK-014AS
# ===========================================================================
...
class TestAQ98OrderLinkIdPrefix:
    def test_order_link_id_prefix_starts_with_static_skeleton_dry_run(self):
        assert ORDER_LINK_ID_PREFIX.startswith("STATIC_SKELETON_DRY_RUN_TINY_ENTRY")
        r = _run()
        assert r.order_link_id_prefix == ORDER_LINK_ID_PREFIX
...
class TestAQ120ImplementationDesignConclusion:
    def test_conclusion_constant_value(self):
        assert STATIC_SKELETON_DRY_RUN_CONCLUSION == "STATIC_SKELETON_DRY_RUN_READY_NOT_EXECUTABLE"
        r = _run()
        assert r.implementation_design_conclusion == STATIC_SKELETON_DRY_RUN_CONCLUSION

    def test_authorization_result_documented_only(self):
        assert STATIC_SKELETON_DRY_RUN_AUTHORIZATION_RESULT == "DOCUMENTED_ONLY_NOT_AUTHORIZED"
        r = _run()
        assert r.implementation_design_authorization_result == STATIC_SKELETON_DRY_RUN_AUTHORIZATION_RESULT
...
# (TASK-014AS forward-declarations consumed by TASK-014AS dry-run)
# ===========================================================================
...
# ===========================================================================
# AR-FIX1: TASK-014AS runtime-consumes TASK-014AQ implementation design output
...
# ---------------------------------------------------------------------------
# These tests verify that the report-facing labels emitted by TASK-014AS
# now use STATIC SKELETON DRY-RUN terminology, that the new aliases are
...
class TestARFIX2ModeStringIsStaticSkeleton:
    def test_default_mode_is_static_skeleton_checklist(self):
        r = _run(symbol="SOLUSDT")
        assert r.mode == "static_skeleton_dry_run_checklist"
        assert r.mode == MODE_STATIC_SKELETON_DRY_RUN_CHECKLIST

    def test_legacy_mode_alias_still_resolves(self):
        # MODE_IMPLEMENTATION_DESIGN_CHECKLIST is a back-compat alias
        # that must equal the static_skeleton_dry_run_checklist string.
        assert MODE_IMPLEMENTATION_DESIGN_CHECKLIST \
            == MODE_STATIC_SKELETON_DRY_RUN_CHECKLIST \
            == "static_skeleton_dry_run_checklist"

    def test_approval_mode_is_static_skeleton_approval(self):
        r = _run(symbol="SOLUSDT", allow_implementation_design=True)
        assert r.mode == "static_skeleton_dry_run_approval"
        assert r.mode == MODE_STATIC_SKELETON_DRY_RUN_APPROVAL
        assert MODE_IMPLEMENTATION_DESIGN_APPROVAL \
            == MODE_STATIC_SKELETON_DRY_RUN_APPROVAL


class TestARFIX2StatusUnchanged:
    def test_status_remains_static_skeleton_dry_run_ready(self):
        r = _run(symbol="SOLUSDT")
        assert r.status \
...
class TestARFIX2StaticSkeletonConclusionAlias:
    def test_to_dict_exposes_static_skeleton_conclusion_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert d["static_skeleton_dry_run_conclusion"] \
        ...
        assert d["implementation_design_conclusion"] \
            == d["static_skeleton_dry_run_conclusion"]
...
class TestARFIX2StaticSkeletonAuthorizationResultAlias:
    def test_to_dict_exposes_static_skeleton_authorization_result_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert d["static_skeleton_dry_run_authorization_result"] \
            == "DOCUMENTED_ONLY_NOT_AUTHORIZED"
        assert d["implementation_design_authorization_result"] \
            == d["static_skeleton_dry_run_authorization_result"]
...
class TestARFIX2StaticSkeletonScopeAlias:
    def test_to_dict_exposes_static_skeleton_scope_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert "static_skeleton_dry_run_scope" in d
        # Alias content must equal the legacy implementation_design_scope.
        assert d["static_skeleton_dry_run_scope"] \
            == d["implementation_design_scope"]
        # Scope summary text now says AS consumes AR output and produces
        # static skeleton dry-run for AT.
        summary = d["static_skeleton_dry_run_scope"]["scope_summary"]
        assert "TASK-014AR" in summary
...
class TestARFIX2FinalStaticSkeletonVerdictAlias:
    def test_to_dict_exposes_final_static_skeleton_dry_run_verdict_alias(self):
        r = _run(symbol="SOLUSDT")
        d = r.to_dict()
        assert "final_static_skeleton_dry_run_verdict" in d
        assert d["final_static_skeleton_dry_run_verdict"] \
            == d["final_implementation_design_verdict"]
        verdict = d["final_static_skeleton_dry_run_verdict"]
        assert verdict["static_skeleton_dry_run_conclusion"] \
            == "STATIC_SKELETON_DRY_RUN_READY_NOT_EXECUTABLE"
        assert verdict["implementation_design_conclusion"] \
            == verdict["static_skeleton_dry_run_conclusion"]
...
class TestARFIX2AuditArtifactsHaveStaticSkeletonAliases:
    def test_audit_artifacts_contain_static_skeleton_aliases(self):
        r = _run(symbol="SOLUSDT")
        a = r.audit_artifacts
        assert "static_skeleton_dry_run_scope" in a
        assert "final_static_skeleton_dry_run_verdict" in a
        assert a["static_skeleton_dry_run_scope"] \
            == a["implementation_design_scope"]
        assert a["final_static_skeleton_dry_run_verdict"] \
...
class TestARFIX2Stage13SummaryUsesStaticSkeleton:
    def test_stage_13_summary_mentions_static_skeleton(self):
        ...
        assert "static skeleton dry-run verdict" in s13["summary"]
        assert "final_implementation_design_verdict" in s13
        assert "final_static_skeleton_dry_run_verdict" in s13
        assert s13["static_skeleton_dry_run_conclusion"] \
            == "STATIC_SKELETON_DRY_RUN_READY_NOT_EXECUTABLE"
...
class TestARFIX2MarkdownReportTitleAndSections:
    def test_markdown_report_uses_static_skeleton_wording(self, repo_tmp_path):
        ...
        # Title must say Static Skeleton Dry-run, not Implementation Design.
        assert "# Tiny Guarded Entry Real Execution Adapter Static Skeleton Dry-run (TASK-014AS)" in md
        # Section headers updated to static skeleton dry-run wording.
        assert "## Static Skeleton Dry-run Verdict" in md
        assert "## Static Skeleton Dry-run Scope" in md
        assert "## Final Static Skeleton Dry-run Verdict" in md
        # Static skeleton conclusion label appears at the top of the report.
        assert "static_skeleton_dry_run_conclusion:" in md
        # Mode shown in the report is the static_skeleton_dry_run_checklist
        # string.
        assert "mode: `static_skeleton_dry_run_checklist`" in md
        # The narrative summary mentions that AS consumes AR output and
        # produces static skeleton dry-run for AT.
        assert "TASK-014AR" in md
...
class TestARFIX2CLIBannerSaysStaticSkeleton:
    def test_cli_help_does_not_advertise_implementation_design_only(self):
        ...
        # DRY-RUN (TASK-014AS), not the legacy IMPLEMENTATION DESIGN
        ...
        assert "TASK-014AR" in normalized
